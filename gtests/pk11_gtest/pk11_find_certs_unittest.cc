...
// issuer:test cert
// subject:test cert (byte-identical to kTestCert1DER / kTestCert2DER)
// issuerKey:secp256r1
// subjectKey:secp256r1
// serialNumber:5
// subjectAltName:email:a@example.com
const std::vector<uint8_t> kTestCertWithEmailSANDER = {
    0x30, 0x82, 0x01, 0x3B, 0x30, 0x81, 0xE0, 0xA0, 0x03, 0x02, 0x01, 0x02,
    0x02, 0x01, 0x05, 0x30, 0x0D, 0x06, 0x09, 0x2A, 0x86, 0x48, 0x86, 0xF7,
    0x0D, 0x01, 0x01, 0x0B, 0x05, 0x00, 0x30, 0x14, 0x31, 0x12, 0x30, 0x10,
    0x06, 0x03, 0x55, 0x04, 0x03, 0x0C, 0x09, 0x74, 0x65, 0x73, 0x74, 0x20,
    0x63, 0x65, 0x72, 0x74, 0x30, 0x22, 0x18, 0x0F, 0x32, 0x30, 0x31, 0x37,
    0x31, 0x31, 0x32, 0x37, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x5A, 0x18,
    ...
    ,
};

class PK11FindCertsByEmailTest : public PK11FindCertsTestBase {};

// bug 2029462:
TEST_F(PK11FindCertsByEmailTest, EmailCacheCleanupAfterSameSubjectCerts) {
  // Import the cert with the email SAN first. This creates the subject list and
  // registers an entry in the email cache.
  SECItem certA_item = {
      siBuffer, const_cast<unsigned char*>(kTestCertWithEmailSANDER.data()),
      (unsigned int)kTestCertWithEmailSANDER.size()};
  ScopedCERTCertificate certA(CERT_NewTempCertificate(
      CERT_GetDefaultCertDB(), &certA_item, nullptr, false, true));
  ASSERT_NE(nullptr, certA);
  ASSERT_EQ(SECSuccess, PK11_ImportCert(m_slot, certA.get(), CK_INVALID_HANDLE,
                                        "cert_with_email_san", false));

  // Import a cert with the same subject but no email SAN. This appends to the
  // existing subject list.
  SECItem certB_item = {siBuffer,
                        const_cast<unsigned char*>(kTestCert2DER.data()),
                        (unsigned int)kTestCert2DER.size()};
  ScopedCERTCertificate certB(CERT_NewTempCertificate(
      CERT_GetDefaultCertDB(), &certB_item, nullptr, false, true));
  ASSERT_NE(nullptr, certB);
  ASSERT_EQ(SECSuccess, PK11_ImportCert(m_slot, certB.get(), CK_INVALID_HANDLE,
                                        "cert_without_email_san", false));

  // Drop certA first: subject list still contains certB, so the email hash
  // entry remains valid.
  certA = nullptr;

  // Drop certB last: subject list drains to zero. Without the fix,
  // remove_email_entry sees certB->email == NULL, skips cleaning up the
  // email cache, and then nssArena_Destroy frees the arena backing the subject
  // list. This leaves a dangling pointer in the email cache.
  certB = nullptr;

  // Lookup by the now-gone email address must not dereference freed memory.
  ScopedCERTCertList result(
      PK11_FindCertsFromNickname("a@example.com", nullptr));
  EXPECT_TRUE(!result || CERT_LIST_EMPTY(result.get()));
}
