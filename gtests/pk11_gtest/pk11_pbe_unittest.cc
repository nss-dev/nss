 ...
 private:
  bool DeriveBytes(std::string& pass, std::string& salt,
    ...
    ScopedSECAlgorithmID alg_id(
        PK11_CreatePBEAlgorithmID(SEC_OID_PKCS12_V2_PBE_WITH_SHA1_AND_2KEY_TRIPLE_DES_CBC,
                                  kIterations, &salt_item));
    ...
    return key_data->len == derived.len() &&
           !memcmp(&derived[0], key_data->data, key_data->len);
  }
};

TEST_F(Pkcs11PbeTest, DeriveKnown) {
  std::vector<uint8_t> derived = {0x86, 0x6b, 0xce, 0xef, 0x26, 0xa4, 0x4f,
                                  0x02, 0x4a, 0x26, 0x86, 0x6b, 0xce, 0xef,
                                  0x26, 0x01};
