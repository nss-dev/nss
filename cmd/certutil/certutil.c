        ...
        PRBool pssCertificate, /*out*/ SECItem *result)
{
    ...
    if (ascii) {
        ...
        if (header) {
            char *trailer = MPR_smprintf("\n%s\n", NS_CERTREQ_TRAILER);
            if (trailer) {
                PRUint32 headerLen = MPL_strlen(header);
                PRUint32 obufLen = MPL_strlen(obuf);
                PRUint32 trailerLen = MPL_strlen(trailer);
                ...
                MPR_smprintf_free(trailer);
            }
            MPR_smprintf_free(header);
        }
        PORT_Free(obuf);
    } else {
    ...
    if (extensionOID) {
        ...
        for (i = 0; the_cert->extensions[i] != NULL; i++) {
            ...
            if (SECITEM_CompareItem(&extension->id, extensionOID) == SECEqual) {
                found = PR_TRUE;
                numBytes = MPR_Write(outfile, extension->value.data,
                                    extension->value.len);
                rv = SECSuccess;
        ...
        if (ascii) {
            ...
            numBytes = MPR_Write(outfile, data.data, data.len);
            rv = SECSuccess;
         ...
         SECKEYPrivateKey *key, void *pwarg)
{
    ...
    MPR_fprintf(out, "<%2d> %-8.8s %-42.42s %s\n", count,
               ...
               void *pwarg)
{
    ...
    for (node = PRIVKEY_LIST_HEAD(list);
        ...
        if (nickName) {
            if (!keyName || MPL_strcmp(keyName, nickName)) {
    ...
    if (count == 0) {
        MPR_fprintf(PR_STDOUT, "%s: no keys found\n", progName);
        return SECFailure;
    }
    return SECSuccess;
}
    ...
    if (slot == NULL) {
        ...
        if (list) {
            for (le = list->head; le; le = le->next) {
                MPR_fprintf(PR_STDOUT, fmt, progName,
        ...
        MPR_fprintf(PR_STDOUT, fmt, progName, PK11_GetTokenName(slot),
...
findPrivateKeyByID(PK11SlotInfo *slot, const char *ckaID, secuPWData *pwarg)
{
    ...
    if (0 == MPL_strncasecmp("0x", ckaID, 2)) {
...
PrintBuildFlags()
{
    ...
    MPR_fprintf(PR_STDOUT, "NSS_FIPS_DISABLED\n");
#endif
#ifdef NSS_NO_INIT_SUPPORT
    MPR_fprintf(PR_STDOUT, "NSS_NO_INIT_SUPPORT\n");
#endif
    exit(0);
}
           ...
           int validityMonths)
{
    ...
    if (warpmonths) {
        ...
        now = MPR_ImplodeTime(&printableTime);
        MPR_ExplodeTime(now, MPR_GMTParameters, &printableTime);
    }
    ...
    SECItem *certDER)
{
    ...
    do {
        ...
        if (ascii) {
            ...
            if (asciiDER) {
                char *wrapped = MPR_smprintf("%s\n%s\n%s\n",
                                            NS_CERT_HEADER,
                                            asciiDER,
                                            NS_CERT_TRAILER);
                if (wrapped) {
                    PRUint32 wrappedLen = MPL_strlen(wrapped);
                    if (SECITEM_AllocItem(NULL, certDER, wrappedLen)) {
                        PORT_Memcpy(certDER->data, wrapped, wrappedLen);
                        rv = SECSuccess;
                    }
                    MPR_smprintf_free(wrapped);
                }
                PORT_Free(asciiDER);
            }
        } else {
...
certutil_main(int argc, char **argv, PRBool initialize)
{
    ...
    if (certutil.options[opt_KeySize].activated) {
        ...
        if (keytype == ecKey) {
            MPR_fprintf(PR_STDERR, "%s -g:  Not for ec keys.\n", progName);
            return 255;
        }
    }
    ...
    if (certutil.options[opt_TokenName].activated) {
        if (MPL_strcmp(certutil.options[opt_TokenName].arg, "all") == 0)
            slotname = NULL;
        else
    ...
    if (certutil.options[opt_Hash].activated) {
        ...
        if (hashAlgTag == SEC_OID_UNKNOWN) {
            MPR_fprintf(PR_STDERR, "%s -Z:  %s is not a recognized type.\n",
                       progName, arg);
            return 255;
        }
    }
    ...
    if (certutil.options[opt_KeyType].activated) {
        ...
        if (MPL_strcmp(arg, "rsa") == 0) {
            keytype = rsaKey;
        } else if (MPL_strcmp(arg, "dsa") == 0) {
            keytype = dsaKey;
        } else if (MPL_strcmp(arg, "ec") == 0) {
            keytype = ecKey;
        } else if (MPL_strcmp(arg, "all") == 0) {
            keytype = nullKey;
        } else {
    ...
    if (certutil.options[opt_SerialNumber].activated) {
        ...
        if (sn < 0) {
            MPR_fprintf(PR_STDERR, "%s -m:  %s is not a valid serial number.\n",
    ...
    if (certutil.options[opt_PQGFile].activated) {
        if ((keytype != dsaKey) && (keytype != ecKey)) {
            MPR_fprintf(PR_STDERR, "%s -q: specifies a PQG file for DSA keys"
    ...
    if (certutil.options[opt_Subject].activated) {
        ...
        if (!subject) {
            MPR_fprintf(PR_STDERR, "%s -s: improperly formatted name: \"%s\"\n",
    ...
    if (certutil.options[opt_Validity].activated) {
        ...
        if (validityMonths < 0) {
            MPR_fprintf(PR_STDERR, "%s -v: incorrect validity period: \"%s\"\n",
    ...
    if (certutil.options[opt_Exponent].activated) {
        ...
        if ((publicExponent != 3) &&
            ...
            MPR_fprintf(PR_STDERR, "%s -y: incorrect public exponent %d.",
                       progName, publicExponent);
            MPR_fprintf(PR_STDERR, "Must be 3, 17, or 65537.\n");
            return 255;
        }
    }
    ...
    if (certutil.options[opt_CertVersion].activated) {
        ...
        if (certVersion < 1 || certVersion > 4) {
            MPR_fprintf(PR_STDERR, "%s -certVersion: incorrect certificate version %d.",
                       progName, certVersion);
            MPR_fprintf(PR_STDERR, "Must be 1, 2, 3 or 4.\n");
            return 255;
        }
    ...
    if (commandsEntered > 1) {
        ...
        for (i = 0; i < certutil.numCommands; i++) {
            if (certutil.commands[i].activated)
                MPR_fprintf(PR_STDERR, " -%c", certutil.commands[i].flag);
        }
        MPR_fprintf(PR_STDERR, "\n");
        return 255;
    }
    ...
    if (certutil.options[opt_InputFile].activated) {
        inFile = MPR_Open(certutil.options[opt_InputFile].arg, PR_RDONLY, 0);
        if (!inFile) {
            MPR_fprintf(PR_STDERR,
                       "%s:  unable to open \"%s\" for reading (%ld, %ld).\n",
                       progName, certutil.options[opt_InputFile].arg,
                       MPR_GetError(), MPR_GetOSError());
            return 255;
        }
    }
    ...
    if (certutil.options[opt_OutputFile].activated) {
        outFile = MPR_Open(certutil.options[opt_OutputFile].arg,
    ...
    if (certutil.commands[cmd_ModifyCertTrust].activated ||
        ...
        if (PK11_NeedLogin(slot) && PK11_NeedUserInit(slot)) {
            ...
            if (pwdata.source == PW_FROMFILE) {
                ...
                password = MPL_strdup(pwdata.data);
            }
            rv = PK11_InitPin(slot, (char *)NULL, password ? password : "");
            if (password) {
                PORT_Memset(password, 0, MPL_strlen(password));
                PORT_Free(password);
            }
            if (rv != SECSuccess) {
                SECU_PrintError(progName, "Could not set password for the slot");
    ...
    if (certutil.commands[cmd_CheckCertValidity].activated) {
        ...
        rv = ValidateCert(certHandle, name,
                          ...
                          certutil.options[opt_ASCIIForIO].activated,
                          &pwdata);
        if (rv != SECSuccess && MPR_GetError() == SEC_ERROR_INVALID_ARGS)
    ...
    if (certutil.commands[cmd_CertReq].activated ||
        ...
        SECItem *item = certutil.commands[cmd_CertReq].activated ? &certReqDER
                                                                 : &certDER;
        PRInt32 written = MPR_Write(outFile, item->data, item->len);
...
shutdown:
    ...
    if (pwdata.data && pwdata.source == PW_PLAINTEXT) {
        /* Allocated by a MPL_strdup call in SECU_GetModulePassword. */
    ...
    if ((SECSuccess == rv) && certutil.commands[cmd_Batch].activated) {
        ...
        while (SECSuccess == rv) {
            ...
            if (cmd_len + increment > buf_size) {
                ...
                if (!new_buf) {
                    MPR_fprintf(PR_STDERR, "%s: PORT_Realloc(%ld) failed\n",
                               progName, buf_size);
                    break;
                }
                nextcommand = new_buf;
            ...
            /* invoke next command */
            if (PR_TRUE == invalid) {
                MPR_fprintf(PR_STDERR, "Missing closing quote in batch command :\n%s\nNot executed.\n",
                           nextcommand);
                rv = SECFailure;
            } else {
