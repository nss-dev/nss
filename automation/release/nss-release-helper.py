...
def check_files_exist():
    if (
        not os.path.exists(nssutil_h)
        or not os.path.exists(softkver_h)
        or not os.path.exists(nss_h)
        or not os.path.exists(nssckbi_h)
    ):
        exit_with_failure(
            "cannot find expected header files, must run from inside NSS hg directory"
        )
    ...
    if is_beta:
        ...
        inplace_replace(
            ...
            replacements=[
                ...
                Replacement(
                    regex=r"^(#define *NSSUTIL_BETA *)PR_FALSE *$", repl=r"\g<1>PR_TRUE"
                ),
            ],
        )
        inplace_replace(
        ...
        inplace_replace(
            ...
            replacements=[
                ...
                Replacement(
                    regex=r"^(#define *NSS_BETA *)PR_FALSE *$", repl=r"\g<1>PR_TRUE"
                ),
            ],
        )
    else:
        ...
        inplace_replace(
            ...
            replacements=[
                ...
                Replacement(
                    regex=r"^(#define *NSSUTIL_BETA *)PR_TRUE *$", repl=r"\g<1>PR_FALSE"
                ),
            ],
        )
        inplace_replace(
        ...
        inplace_replace(
            ...
            replacements=[
                ...
                Replacement(
                    regex=r"^(#define *NSS_BETA *)PR_TRUE *$", repl=r"\g<1>PR_FALSE"
                ),
            ],
        )
...
def print_beta_versions():
    check_call_noisy(
        ["egrep", "#define *NSSUTIL_VERSION|#define *NSSUTIL_BETA", nssutil_h]
    )
    check_call_noisy(
        ["egrep", "#define *SOFTOKEN_VERSION|#define *SOFTOKEN_BETA", softkver_h]
    )
...
def print_library_versions():
    ...
    check_call_noisy(
        [
            "egrep",
            "#define *NSSUTIL_VERSION|#define NSSUTIL_VMAJOR|#define *NSSUTIL_VMINOR|#define *NSSUTIL_VPATCH|#define *NSSUTIL_VBUILD|#define *NSSUTIL_BETA",
            nssutil_h,
        ]
    )
    check_call_noisy(
        [
            "egrep",
            "#define *SOFTOKEN_VERSION|#define SOFTOKEN_VMAJOR|#define *SOFTOKEN_VMINOR|#define *SOFTOKEN_VPATCH|#define *SOFTOKEN_VBUILD|#define *SOFTOKEN_BETA",
            softkver_h,
        ]
    )
    check_call_noisy(
        [
            "egrep",
            "#define *NSS_VERSION|#define NSS_VMAJOR|#define *NSS_VMINOR|#define *NSS_VPATCH|#define *NSS_VBUILD|#define *NSS_BETA",
            nss_h,
        ]
    )
...
def set_all_lib_versions(version, major, minor, patch, build):
    ...
    if old_major < new_major or (old_major == new_major and old_minor < new_minor):
        ...
        print(
            "- setting reference branch to the branch of the previous version: "
            + new_branch
        )
...
def set_4_digit_release_number(args):
    ensure_arguments_count(
        args, 4, "major_version  minor_version  patch_release  4th_digit_release_number"
    )
    major = args[0].strip()
    minor = args[1].strip()
    patch = args[2].strip()
...
def make_release_branch(args):
    ...
    if patch is not None:
        exit_with_failure(
            "make_release_branch expects a minor version (e.g., '3.117'), not a patch version."
        )
    ...
    if hg_status:
        ...
        print()
        exit_with_failure(
            "Please commit or revert changes then run this command again. You can reset your working directory with 'hg update -C' and 'hg purge if you want to discard all local changes."
        )
    ...
    if hg_status:
        ...
        print()
        exit_with_failure(
            "Please check the correct version to freeze, or update the version numbers then run this command again."
        )
...
def generate_release_note(args):
    ensure_arguments_count(
        args,
        3,
        "this_release_version_string revision_or_tag previous_release_version_string ",
    )
...
def release_nss(args):
    ...
    recent_log = check_output(
        ["hg", "log", "-l", "5", "--template", "{desc|firstline}\\n"]
    ).decode("utf-8")
    ...
    generate_doc_script = os.path.join(
        os.path.dirname(__file__), "generate_release_doc.py"
    )
    check_call_noisy([sys.executable, generate_doc_script, version])
    print_separator()
    ...
    # Get the commit hash
    docs_commit = (
        check_output(["hg", "log", "-r", ".", "--template", "{node|short}"])
        .decode("utf-8")
        .strip()
    )
    ...
    if "y" in response.lower():
        check_call_noisy(
            ["hg", "outgoing", "--graph", "-b", "default", "-b", branch_name, remote]
        )
    print_separator()
...
def create_nss_release_archive(args):
    ...
    if not os.path.exists(nsprtar_with_path):
        ...
        check_call_noisy(
            [
                "wget",
                f"{nspr_releases_url}/v{nsprrel}/src/nspr-{nsprrel}.tar.gz",
    ...
    if not os.path.exists(nsprtar_with_path):
        exit_with_failure(
            "cannot find nspr archive at expected location " + nsprtar_with_path
        )
    ...
    check_call_noisy(
        [
            ...
            "-r",
            nssreltag,
            "--prefix=nss-" + nssrel + "/nss",
            stagedir + "/" + nssreltag + "/src/" + nss_tar,
            "-X",
            ".hgtags",
        ]
    )
    ...
    check_call(
        "sha256sum " + nss_tar + " " + nss_nspr_tar + " > SHA256SUMS", shell=True
    )
