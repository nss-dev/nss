...
Display "*******************************************************************************"
    ...
    while read -r testPgm args; do
        ...
        if [[ ${checkmem} -eq 1 ]]; then
            /net/zhadum/export/tools/sparc/on10/SUNWspro/SOS8/bin/dbx -C -c "runargs ${arenaCmd} $args; check -all;run;exit" ${testPgm} > ${testOut} 2>&1
        else
            ${testPgm} ${args} ${arenaText} > ${testOut} 2>&1
        fi
