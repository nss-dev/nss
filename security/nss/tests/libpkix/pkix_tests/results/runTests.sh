...
function RunTests
{
    ...
    #
    Display "*******************************************************************************"
    Display "START OF TESTS FOR PKIX RESULTS${memText}"
Display "*******************************************************************************"
    Display ""
    ...
    while read -r testPgm; do
        ...
        if [[ ${checkmem} -eq 1 ]]; then
            /net/zhadum/export/tools/sparc/on10/SUNWspro/SOS8/bin/dbx -C -c "check -all;run;exit" ${testPgm} ${arenaCmd} > ${testOut} 2>&1
        else
            ${testPgm} ${arenaCmd} > ${testOut} 2>&1
        fi
...
EOF
    ...
    if [[ ${errors} -eq 0 ]]; then
        if [[ ${memErrors} -eq 0 ]]; then
            Display "\n************************************************************"
            Display "END OF TESTS FOR PKIX RESULTS: ALL TESTS COMPLETED SUCCESSFULLY"
            Display "************************************************************"
            return 0
        fi
    fi
    ...
    Display "\n*******************************************************************************"
    Display "END OF TESTS FOR PKIX RESULTS: ${errors} UNIT TEST${plural} FAILED: ${failedpgms}"
    if [[ ${checkmem} -eq 1 ]]; then
