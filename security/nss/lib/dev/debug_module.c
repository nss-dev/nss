...
static void print_hex_data(void *buf, CK_ULONG len)
{
    CK_ULONG i, j;
    char str[41];
    ...
    for (i=0; i<len; i++) {
	j = (i % 20) * 2;
	PR_snprintf(&str[j], 3, "%02x", data[i]);
	if (j == 38) PR_LOG(modlog, 5, ("%s", str));
    }
    if (j < 38) PR_LOG(modlog, 5, ("%s", str));
}
...
static void print_attr_value(CK_ATTRIBUTE_PTR attr)
{
    ...
    case CKA_KEY_TYPE:
	if (attr->ulValueLen > 0 && attr->pValue) {
	    CK_KEY_TYPE kt = *((CK_KEY_TYPE *)attr->pValue);
	    /*get_obj_class(class, valstr, sizeof valstr);
	    PR_LOG(modlog, 4, ("    %s = %s [%d]", 
	           atype, valstr, attr->ulValueLen));*/
	    PR_LOG(modlog, 4, ("    %s = %x [%d]", 
	           atype, kt, attr->ulValueLen));
	    break;
	}
    case CKA_VALUE_LEN:
	if (attr->ulValueLen > 0 && attr->pValue) {
	    CK_ULONG num = *((CK_ULONG *)attr->pValue);
	    PR_LOG(modlog, 4, ("    %s = %d [%d]", 
	           atype, num, attr->ulValueLen));
	    break;
	}
...
)
{
    ...
    print_hex_data(pEncryptedPart, *pulEncryptedPartLen);
    PR_LOG(modlog, 1, ("  rv = 0x%x\n", rv));
    return rv;
}
...
)
{
    ...
    print_hex_data(pEncryptedPart, ulEncryptedPartLen);
    PR_LOG(modlog, 3, ("  pPart = 0x%p", pPart));
    ...
    print_hex_data(pPart, *pulPartLen);
    PR_LOG(modlog, 1, ("  rv = 0x%x\n", rv));
    return rv;
}
...
)
{
    ...
    PR_LOG(modlog, 4, ("  *pulDigestLen = 0x%x", *pulDigestLen));
    print_hex_data(pDigest, *pulDigestLen);
    PR_LOG(modlog, 1, ("  rv = 0x%x\n", rv));
    return rv;
}
...
)
{
    ...
    PR_LOG(modlog, 4, ("  *pulSignatureLen = 0x%x", *pulSignatureLen));
    print_hex_data(pSignature, *pulSignatureLen);
    PR_LOG(modlog, 1, ("  rv = 0x%x\n", rv));
    return rv;
}
...
)
{
    ...
    print_hex_data(pWrappedKey, *pulWrappedKeyLen);
    PR_LOG(modlog, 1, ("  rv = 0x%x\n", rv));
    return rv;
}
...
)
{
    ...
    print_hex_data(pWrappedKey, ulWrappedKeyLen);
    PR_LOG(modlog, 3, ("  pTemplate = 0x%p", pTemplate));
