...
ARCFOUR:
	...
	movq	%rsp,%rbp
	pushq	%rbx
#	movq	%rdi,		%rdi	# key = ARG(key)
	movq	%rsi,		%rbx	# rbx = ARG(len)
	movq	%rdx,		%rsi	# in = ARG(in)
	movq	%rcx,		%r10	# out = ARG(out)
	movq	(%rdi),		%rcx	# x = key->x
	movq	8(%rdi),	%rdx	# y = key->y
	addq	$16,		%rdi	# d = key->data
	incq	%rcx			# x++
	andq	$255,		%rcx	# x &= 0xff
	leaq	-8(%rbx,%rsi),	%rbx	# rbx = in+len-8
	movq	%rbx,		%r9	# tmp = in+len-8
	movq	0(%rdi,%rcx,8),	%rax	# tx = d[x]
	cmpq	%rsi,		%rbx	# cmp in with in+len-8
	jl	.Lend			# jump if (in+len-8 < in)

.Lstart:
	addq	$8,		%rsi		# increment in
	addq	$8,		%r10		# increment out

	# generate the next 8 bytes of the rc4 stream into %r8
	movq	$8,		%r11		# byte counter
1:	addb	%al,		%dl		# y += tx
	movl	0(%rdi,%rdx,8),	%ebx		# ty = d[y]
	movl	%ebx,		0(%rdi,%rcx,8)	# d[x] = ty
	addb	%al,		%bl		# val = ty + tx
	movl	%eax,		0(%rdi,%rdx,8)	# d[y] = tx
	incb	%cl				# x++		(NEXT ROUND)
	movl	0(%rdi,%rcx,8),	%eax		# tx = d[x]	(NEXT ROUND)
	movb	0(%rdi,%rbx,8),	%r8b		# val = d[val]
	decb	%r11b
...
.Lfinished:
	decq	%rcx				# x--
	movb	%dl,		-8(%rdi)	# key->y = y
	movb	%cl,		-16(%rdi)	# key->x = x
	popq	%rbx
	leave
	ret
.L_ARCFOUR_end:
