...
SECStatus 
SEC_DestroyCrl(PCERTSignedCrl *crl);

PCERTSignedCrl *
SEC_NewCrl(PCERTCertDBHandle *handle, char *url, SECItem *derCrl, int type);

PCERTSignedCrl *
cert_DBInsertCRL
   (PCERTCertDBHandle *handle, char *url,
    PCERTSignedCrl *newCrl, SECItem *derCrl, int type);
...
SECStatus
SEC_DeletePermCRL(PCERTSignedCrl *crl);


SECStatus
SEC_LookupCrls(PCERTCertDBHandle *handle, PCERTCrlHeadNode **nodes, int type);
