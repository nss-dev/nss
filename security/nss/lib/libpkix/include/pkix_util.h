...
/* PKIX_CertChain
 *
 * A PKIX_CertChain is an ordered list of X.509 certificates.
 *
 * A CertChain and the certificates in it should be considered read-only. In
 * a valid chain, the end-entity certificate is index 0.
 */

/*
 * FUNCTION: PKIX_CertChain_Create
 * DESCRIPTION:
 *
 *  Creates a new CertChain using the List of Certs pointed to by "certs" and
 *  stores it at "pChain". Once created, a CertChain is immutable.
 *
 * PARAMETERS:
 *  "certs"
 *      Address of List of Certs to be used to create CertChain.
 *      Must be non-NULL.
 *  "pChain"
 *      Address where object pointer will be stored. Must be non-NULL.
 *  "plContext"
 ...
 * RETURNS:
 *  Returns NULL if the function succeeds.
 *  Returns a CertChain Error if the function fails in a non-fatal way.
 *  Returns a Fatal Error if the function fails in an unrecoverable way.
 */
PKIX_Error *
...
/*
 * XXX May want to add cert chain encoding/decoding functions at some point */

/*
 * FUNCTION: PKIX_CertChain_GetCertificates
 * DESCRIPTION:
 *
 *  Retrieves a pointer to the List of Certs associated with the CertChain
 *  pointed to by "chain" and stores it at "pCerts". If the function succeeds,
 *  the pointer to the List is guaranteed to be non-NULL.
 *
 *  Note that the List returned by this function is immutable.
 *
 * PARAMETERS:
 *  "chain"
 *      Address of CertChain whose certificates are to be stored.
 *      Must be non-NULL.
 *  "pCerts"
 *      Address where object pointer will be stored. Must be non-NULL.
 *  "plContext"
