...
/* semctl() takes union semun via its variadic argument; this is intentional. */
...
PR_IMPLEMENT(PRLock*) MPR_NewLock(void) {
  PRIntn rv;
  PRLock* lock;
...
PR_IMPLEMENT(void) MPR_DestroyLock(PRLock* lock) {
  PRIntn rv;
  PR_ASSERT(NULL != lock);
...
PR_IMPLEMENT(void) MPR_Lock(PRLock* lock) {
  /* Nb: MPR_Lock must not call MPR_GetCurrentThread to access the |id| or
   * |tid| field of the current thread's PRThread structure because
   * _pt_root calls MPR_Lock before setting thred->id and thred->tid. */
  PRIntn rv;
  PR_ASSERT(lock != NULL);
   ...
   * the correctness of MPR_AssertCurrentThreadOwnsLock(), but
...
PR_IMPLEMENT(PRStatus) MPR_Unlock(PRLock* lock) {
...
PR_IMPLEMENT(void) MPR_AssertCurrentThreadOwnsLock(PRLock* lock) {
  /* Nb: the order of the |locked| and |owner==me| checks is not critical
   * to the correctness of MPR_AssertCurrentThreadOwnsLock(), but
   * this particular order makes the assertion more likely to
   * catch errors. */
...
static PRIntn pt_TimedWait(pthread_cond_t* cv, pthread_mutex_t* ml,
  ...
  struct timeval now;
  struct timespec tmo;
  PRUint32 ticks = MPR_TicksPerSecond();

  tmo.tv_sec = (PRInt32)(timeout / ticks);
  tmo.tv_nsec = (PRInt32)(timeout - (tmo.tv_sec * ticks));
  tmo.tv_nsec =
      (PRInt32)MPR_IntervalToMicroseconds(PT_NANOPERMICRO * tmo.tv_nsec);
...
PR_IMPLEMENT(PRCondVar*) MPR_NewCondVar(PRLock* lock) {
...
PR_IMPLEMENT(void) MPR_DestroyCondVar(PRCondVar* cvar) {
...
PR_IMPLEMENT(PRStatus) MPR_WaitCondVar(PRCondVar* cvar, PRIntervalTime timeout) {
  PRIntn rv;
  PRThread* thred = MPR_GetCurrentThread();
  ...
  /*
   * The thread waiting is used for MPR_Interrupt
   */
...
PR_IMPLEMENT(PRStatus) MPR_NotifyCondVar(PRCondVar* cvar) {
  PR_ASSERT(cvar != NULL);
  pt_PostNotifyToCvar(cvar, PR_FALSE);
  return PR_SUCCESS;
} /* MPR_NotifyCondVar */

PR_IMPLEMENT(PRStatus) MPR_NotifyAllCondVar(PRCondVar* cvar) {
  PR_ASSERT(cvar != NULL);
  pt_PostNotifyToCvar(cvar, PR_TRUE);
  return PR_SUCCESS;
} /* MPR_NotifyAllCondVar */
...
PR_IMPLEMENT(PRMonitor*) MPR_NewMonitor(void) {
  PRMonitor* mon;
  int rv;
...
PR_IMPLEMENT(PRMonitor*) MPR_NewNamedMonitor(const char* name) {
  PRMonitor* mon = MPR_NewMonitor();
  if (mon) {
    mon->name = name;
  }
  return mon;
...
PR_IMPLEMENT(void) MPR_DestroyMonitor(PRMonitor* mon) {
  int rv;
 ...
 */
PR_IMPLEMENT(PRIntn) MPR_GetMonitorEntryCount(PRMonitor* mon) {
  pthread_t self = pthread_self();
  PRIntn rv;
  PRIntn count = 0;
...
PR_IMPLEMENT(void) MPR_AssertCurrentThreadInMonitor(PRMonitor* mon) {
...
PR_IMPLEMENT(void) MPR_EnterMonitor(PRMonitor* mon) {
  pthread_t self = pthread_self();
  PRIntn rv;
...
PR_IMPLEMENT(PRStatus) MPR_ExitMonitor(PRMonitor* mon) {
  pthread_t self = pthread_self();
  PRIntn rv;
  ...
  if (notifyEntryWaiter) {
     ...
     * reference. */
    MPR_DestroyMonitor(mon);
  }
  return PR_SUCCESS;
} /* MPR_ExitMonitor */

PR_IMPLEMENT(PRStatus) MPR_Wait(PRMonitor* mon, PRIntervalTime timeout) {
  PRStatus rv;
...
PR_IMPLEMENT(PRStatus) MPR_Notify(PRMonitor* mon) {
  pt_PostNotifyToMonitor(mon, PR_FALSE);
  return PR_SUCCESS;
} /* MPR_Notify */

PR_IMPLEMENT(PRStatus) MPR_NotifyAll(PRMonitor* mon) {
...
/**************************************************************/
/**************************************************************/
PR_IMPLEMENT(void) MPR_PostSem(PRSemaphore* semaphore) {
  static PRBool unwarned = PR_TRUE;
  if (unwarned)
    unwarned = _PR_Obsolete("MPR_PostSem", "locks & condition variables");
  MPR_Lock(semaphore->cvar->lock);
  MPR_NotifyCondVar(semaphore->cvar);
  semaphore->count += 1;
  MPR_Unlock(semaphore->cvar->lock);
} /* MPR_PostSem */

PR_IMPLEMENT(PRStatus) MPR_WaitSem(PRSemaphore* semaphore) {
  PRStatus status = PR_SUCCESS;
  static PRBool unwarned = PR_TRUE;
  if (unwarned)
    unwarned = _PR_Obsolete("MPR_WaitSem", "locks & condition variables");
  MPR_Lock(semaphore->cvar->lock);
  while ((semaphore->count == 0) && (PR_SUCCESS == status)) {
    status = MPR_WaitCondVar(semaphore->cvar, PR_INTERVAL_NO_TIMEOUT);
  }
...
PR_IMPLEMENT(void) MPR_DestroySem(PRSemaphore* semaphore) {
  static PRBool unwarned = PR_TRUE;
  if (unwarned)
    unwarned = _PR_Obsolete("MPR_DestroySem", "locks & condition variables");
  MPR_DestroyLock(semaphore->cvar->lock);
  MPR_DestroyCondVar(semaphore->cvar);
  MPR_Free(semaphore);
} /* MPR_DestroySem */

PR_IMPLEMENT(PRSemaphore*) MPR_NewSem(PRUintn value) {
  ...
  if (unwarned)
    unwarned = _PR_Obsolete("MPR_NewSem", "locks & condition variables");

  semaphore = PR_NEWZAP(PRSemaphore);
  if (NULL != semaphore) {
    PRLock* lock = MPR_NewLock();
    if (NULL != lock) {
      semaphore->cvar = MPR_NewCondVar(lock);
...
PR_IMPLEMENT(PRSem*)
MPR_OpenSemaphore(const char* name, PRIntn flags, PRIntn mode, PRUintn value) {
  PRSem* sem;
...
PR_IMPLEMENT(PRStatus) MPR_WaitSemaphore(PRSem* sem) {
  int rv;
  rv = sem_wait(sem->sem);
  if (0 != rv) {
    _PR_MD_MAP_DEFAULT_ERROR(errno);
...
PR_IMPLEMENT(PRStatus) MPR_PostSemaphore(PRSem* sem) {
  int rv;
  rv = sem_post(sem->sem);
  if (0 != rv) {
    _PR_MD_MAP_DEFAULT_ERROR(errno);
...
PR_IMPLEMENT(PRStatus) MPR_CloseSemaphore(PRSem* sem) {
  int rv;
...
PR_IMPLEMENT(PRStatus) MPR_DeleteSemaphore(const char* name) {
  int rv;
  char osname[PR_IPC_NAME_SIZE];
...
PR_IMPLEMENT(PRStatus) MPRP_TryLock(PRLock* lock) {
...
PR_IMPLEMENT(PRCondVar*) MPRP_NewNakedCondVar(void) {
  PRCondVar* cv;
  ...
  return cv;
} /* MPRP_NewNakedCondVar */

PR_IMPLEMENT(void) MPRP_DestroyNakedCondVar(PRCondVar* cvar) {
  int rv;
...
#  endif
  MPR_Free(cvar);
} /* MPRP_DestroyNakedCondVar */

PR_IMPLEMENT(PRStatus)
MPRP_NakedWait(PRCondVar* cvar, PRLock* ml, PRIntervalTime timeout) {
  PRIntn rv;
  PR_ASSERT(cvar != NULL);
...
PR_IMPLEMENT(PRStatus) MPRP_NakedNotify(PRCondVar* cvar) {
  int rv;
  PR_ASSERT(cvar != NULL);
...
PR_IMPLEMENT(PRStatus) MPRP_NakedBroadcast(PRCondVar* cvar) {
  int rv;
  PR_ASSERT(cvar != NULL);
