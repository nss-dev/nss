 ...
 * we can use the faster MPR_SendFile/MPR_TransmitFile implementation based on
...
static PRLock* _pr_flock_lock;  /* For MPR_LockFile() etc. */
static PRCondVar* _pr_flock_cv; /* For MPR_LockFile() etc. */
static PRLock* _pr_rename_lock; /* For MPR_Rename() */
...
PR_IMPLEMENT(void) MPT_FPrintStats(PRFileDesc* debug_out, const char* msg) {
  PTDebug stats;
  char buffer[100];
  PRExplodedTime tod;
  PRInt64 elapsed, aMil;
  stats = pt_debug; /* a copy */
  MPR_ExplodeTime(stats.timeStarted, MPR_LocalTimeParameters, &tod);
  (void)MPR_FormatTime(buffer, sizeof(buffer), "%T", &tod);

  LL_SUB(elapsed, MPR_Now(), stats.timeStarted);
  LL_I2L(aMil, 1000000);
  LL_DIV(elapsed, elapsed, aMil);

  if (NULL != msg) {
    MPR_fprintf(debug_out, "%s", msg);
  }
  MPR_fprintf(debug_out, "\tstarted: %s[%lld]\n", buffer, elapsed);
  MPR_fprintf(debug_out, "\tlocks [created: %u, destroyed: %u]\n",
             stats.locks_created, stats.locks_destroyed);
  MPR_fprintf(debug_out, "\tlocks [acquired: %u, released: %u]\n",
             stats.locks_acquired, stats.locks_released);
  MPR_fprintf(debug_out, "\tcvars [created: %u, destroyed: %u]\n",
             stats.cvars_created, stats.cvars_destroyed);
  MPR_fprintf(debug_out, "\tcvars [notified: %u, delayed_delete: %u]\n",
...
static void pt_poll_now(pt_Continuation* op) {
  ...
  PRThread* self = MPR_GetCurrentThread();
  ...
  switch (op->timeout) {
    ...
    default:
      ...
      do {
        ...
        msecs = (PRInt32)MPR_IntervalToMilliseconds(remaining);
        ...
        if (rv > 0) {
          ...
          if (rv == 0) /* poll timed out */
          {
            if (wait_for_remaining) {
              ...
              now += MPR_MillisecondsToInterval(msecs);
            }
          } else {
            now = MPR_IntervalNow();
          }
...
#  if defined(DEBUG)
  ...
  pt_debug.timeStarted = MPR_Now();
#  endif

  _pr_flock_lock = MPR_NewLock();
  PR_ASSERT(NULL != _pr_flock_lock);
  _pr_flock_cv = MPR_NewCondVar(_pr_flock_lock);
  PR_ASSERT(NULL != _pr_flock_cv);
  _pr_rename_lock = MPR_NewLock();
...
void _PR_CleanupIO(void) {
  ...
  if (_pr_flock_cv) {
    MPR_DestroyCondVar(_pr_flock_cv);
    _pr_flock_cv = NULL;
  }
  if (_pr_flock_lock) {
    MPR_DestroyLock(_pr_flock_lock);
    _pr_flock_lock = NULL;
  }
  if (_pr_rename_lock) {
    MPR_DestroyLock(_pr_rename_lock);
    _pr_rename_lock = NULL;
  }
} /* _PR_CleanupIO */

PR_IMPLEMENT(PRFileDesc*) MPR_GetSpecialFD(PRSpecialFD osfd) {
  ...
  switch (osfd) {
    ...
    case PR_StandardError:
      ...
      break;
    default:
      (void)MPR_SetError(PR_INVALID_ARGUMENT_ERROR, 0);
  }
  return result;
} /* MPR_GetSpecialFD */
...
static PRBool pt_TestAbort(void) {
  ...
  if (_PT_THREAD_INTERRUPTED(me)) {
    MPR_SetError(PR_PENDING_INTERRUPT_ERROR, 0);
...
static void pt_MapError(void (*mapper)(PRIntn), PRIntn syserrno) {
  switch (syserrno) {
    case EINTR:
      ...
      break;
    case ETIMEDOUT:
      MPR_SetError(PR_IO_TIMEOUT_ERROR, 0);
      break;
    default:
      mapper(syserrno);
  }
...
static PRStatus pt_Close(PRFileDesc* fd) {
  if ((NULL == fd) || (NULL == fd->secret) ||
    ...
    MPR_SetError(PR_BAD_DESCRIPTOR_ERROR, 0);
    return PR_FAILURE;
  }
  if (pt_TestAbort()) {
    return PR_FAILURE;
...
static PRStatus pt_ConnectContinue(PRFileDesc* fd, PRInt16 out_flags) {
  ...
  if ((out_flags &
    ...
    MPR_SetError(PR_IN_PROGRESS_ERROR, 0);
    return PR_FAILURE;
  }
...
PR_IMPLEMENT(PRStatus) MPR_GetConnectStatus(const PRPollDesc* pd) {
  /* Find the NSPR layer and invoke its connectcontinue method */
  PRFileDesc* bottom = MPR_GetIdentitiesLayer(pd->fd, PR_NSPR_IO_LAYER);
...
static PRInt32 pt_AIXDispatchSendFile(PRFileDesc* sd, PRSendFileData* sfd,
  ...
  if (pt_aix_sendfile_fptr) {
    ...
    return MPR_EmulateSendFile(sd, sfd, flags, timeout);
  }
}
#    endif /* !HAVE_SEND_FILE */
...
static PRInt32 pt_LinuxSendFile(PRFileDesc* sd, PRSendFileData* sfd,
  ...
  if ((sfd->hlen != 0 || sfd->tlen != 0) && sd->secret->md.tcp_nodelay == 0) {
    ...
    if (setsockopt(sd->secret->md.osfd, SOL_TCP, TCP_CORK, &tcp_cork,
       ...
       * MPR_SetSocketOption).  This is not fatal, so we keep
       * on going.
       */
  ...
  if (sfd->hlen != 0) {
    count = MPR_Send(sd, sfd->header, sfd->hlen, 0, timeout);
    if (count == -1) {
      goto failed;
    }
  }
  ...
  if (sfd->tlen != 0) {
    rv = MPR_Send(sd, sfd->trailer, sfd->tlen, 0, timeout);
    if (rv == -1) {
      count = -1;
      goto failed;
    }
...
static PRInt32 pt_AcceptRead(PRFileDesc* sd, PRFileDesc** nd, PRNetAddr** raddr,
  ...
  rv = MPR_EmulateAcceptRead(sd, nd, raddr, buf, amount, timeout);
  return rv;
} /* pt_AcceptRead */
  ...
  if (PR_SUCCESS == rv) {
    switch (data->option) {
      case PR_SockOpt_Linger: {
        ...
        data->value.linger.linger = MPR_SecondsToInterval(linger.l_linger);
        break;
      }
...
#  if !defined(DARWIN) && !defined(LINUX) && !defined(ANDROID)
        MPR_SetError(PR_OPERATION_NOT_SUPPORTED_ERROR, 0);
        rv = PR_FAILURE;
#  else
        PRIntn value;
        length = sizeof(value);
  ...
  if (PR_SUCCESS == rv) {
    switch (data->option) {
      case PR_SockOpt_Linger: {
        ...
        linger.l_linger = MPR_IntervalToSeconds(data->value.linger.linger);
...
static PRFileDesc* pt_SetMethods(PRIntn osfd, PRDescType type,
  ...
  if (fd == NULL) {
    MPR_SetError(PR_OUT_OF_MEMORY_ERROR, 0);
  } else {
...
#  endif
    ...
    switch (type) {
      case PR_DESC_FILE:
        fd->methods = MPR_GetFileMethods();
        break;
      case PR_DESC_SOCKET_TCP:
        fd->methods = MPR_GetTCPMethods();
...
#  endif
      ...
      case PR_DESC_SOCKET_UDP:
        fd->methods = MPR_GetUDPMethods();
        pt_MakeFdNonblock(osfd);
        break;
      case PR_DESC_PIPE:
        fd->methods = MPR_GetPipeMethods();
        pt_MakeFdNonblock(osfd);
        break;
      default:
        break;
...
PR_IMPLEMENT(const PRIOMethods*) MPR_GetFileMethods(void) {
  return &_pr_file_methods;
} /* MPR_GetFileMethods */

PR_IMPLEMENT(const PRIOMethods*) MPR_GetPipeMethods(void) {
  return &_pr_pipe_methods;
} /* MPR_GetPipeMethods */

PR_IMPLEMENT(const PRIOMethods*) MPR_GetTCPMethods(void) {
  return &_pr_tcp_methods;
} /* MPR_GetTCPMethods */

PR_IMPLEMENT(const PRIOMethods*) MPR_GetUDPMethods(void) {
...
MPR_AllocFileDesc(PRInt32 osfd, const PRIOMethods* methods) {
  PRFileDesc* fd = _PR_Getfd();
...
PR_IMPLEMENT(PRFileDesc*)
MPR_Socket(PRInt32 domain, PRInt32 type, PRInt32 proto) {
  PRIntn osfd;
  PRDescType ftype;
  PRFileDesc* fd = NULL;
...
#  endif   /* _PR_HAVE_SDP */
      && PF_UNIX != domain) {
    MPR_SetError(PR_ADDRESS_NOT_SUPPORTED_ERROR, 0);
    return fd;
  }
  if (type == SOCK_STREAM) {
    ...
    (void)MPR_SetError(PR_ADDRESS_NOT_SUPPORTED_ERROR, 0);
    return fd;
  }
#  if defined(_PR_HAVE_SDP)
#    if defined(LINUX)
...
PR_IMPLEMENT(PRFileDesc*)
MPR_OpenFile(const char* name, PRIntn flags, PRIntn mode) {
  PRFileDesc* fd = NULL;
...
#  endif
  ...
  ** enforce the sematics of MPR_Rename(). (see the latter for
  ** more details)
  */
  if (flags & PR_CREATE_FILE) {
    osflags |= O_CREAT;
    if (NULL != _pr_rename_lock) {
      MPR_Lock(_pr_rename_lock);
    }
  }
...
PR_IMPLEMENT(PRFileDesc*) MPR_Open(const char* name, PRIntn flags, PRIntn mode) {
  return MPR_OpenFile(name, flags, mode);
} /* MPR_Open */

PR_IMPLEMENT(PRStatus) MPR_Delete(const char* name) {
  PRIntn rv = -1;
...
PR_IMPLEMENT(PRStatus) MPR_Access(const char* name, PRAccessHow how) {
  PRIntn rv;
...
PR_IMPLEMENT(PRStatus) MPR_GetFileInfo(const char* fn, PRFileInfo* info) {
  PRInt32 rv = _PR_MD_GETFILEINFO(fn, info);
  return (0 == rv) ? PR_SUCCESS : PR_FAILURE;
} /* MPR_GetFileInfo */

PR_IMPLEMENT(PRStatus) MPR_GetFileInfo64(const char* fn, PRFileInfo64* info) {
  PRInt32 rv;
...
PR_IMPLEMENT(PRStatus) MPR_Rename(const char* from, const char* to) {
  PRIntn rv = -1;
  ...
  ** where the lock is held is in MPR_Open() when possibly creating a
  ** new file.
  */
  ...
  rv = access(to, F_OK);
  if (0 == rv) {
    MPR_SetError(PR_FILE_EXISTS_ERROR, 0);
    rv = -1;
  } else {
    rv = rename(from, to);
    if (rv == -1) {
...
PR_IMPLEMENT(PRStatus) MPR_CloseDir(PRDir* dir) {
  if (pt_TestAbort()) {
    return PR_FAILURE;
  }
...
PR_IMPLEMENT(PRStatus) MPR_MakeDir(const char* name, PRIntn mode) {
  PRInt32 rv = -1;
...
PR_IMPLEMENT(PRStatus) MPR_MkDir(const char* name, PRIntn mode) {
  return MPR_MakeDir(name, mode);
} /* PR_Mkdir */

PR_IMPLEMENT(PRStatus) MPR_RmDir(const char* name) {
  PRInt32 rv;
...
PR_IMPLEMENT(PRDir*) MPR_OpenDir(const char* name) {
  DIR* osdir;
  PRDir* dir = NULL;
    ...
    for (index = 0; index < npds; ++index) {
      ...
      if ((NULL != pds[index].fd) && (0 != pds[index].in_flags)) {
        ...
        if ((0 != (in_flags_read & out_flags_read)) ||
          ...
          PRFileDesc* bottom =
              MPR_GetIdentitiesLayer(pds[index].fd, PR_NSPR_IO_LAYER);
    ...
    if (0 == ready) {
      switch (timeout) {
        ...
        case PR_INTERVAL_NO_TIMEOUT:
          ...
          break;
        default:
          msecs = MPR_IntervalToMilliseconds(timeout);
      ...
      if (-1 == ready) {
        ...
        if (EINTR == oserror) {
          if (timeout == PR_INTERVAL_NO_TIMEOUT) {
            ...
            elapsed = (PRIntervalTime)(MPR_IntervalNow() - start);
            if (elapsed > timeout) {
              ready = 0; /* timed out */
            } else {
              remaining = timeout - elapsed;
              msecs = MPR_IntervalToMilliseconds(remaining);
              goto retry;
            }
          }
        } else {
...
MPR_Poll(PRPollDesc* pds, PRIntn npds, PRIntervalTime timeout) {
  return (_pr_poll_with_poll(pds, npds, timeout));
}

PR_IMPLEMENT(PRDirEntry*) MPR_ReadDir(PRDir* dir, PRDirFlags flags) {
  struct dirent* dp;
...
PR_IMPLEMENT(PRFileDesc*) MPR_NewUDPSocket(void) {
  PRIntn domain = PF_INET;

  return MPR_Socket(domain, SOCK_DGRAM, 0);
} /* MPR_NewUDPSocket */

PR_IMPLEMENT(PRFileDesc*) MPR_NewTCPSocket(void) {
  PRIntn domain = PF_INET;

  return MPR_Socket(domain, SOCK_STREAM, 0);
} /* MPR_NewTCPSocket */

PR_IMPLEMENT(PRFileDesc*) MPR_OpenUDPSocket(PRIntn af) {
  return MPR_Socket(af, SOCK_DGRAM, 0);
} /* MPR_NewUDPSocket */

PR_IMPLEMENT(PRFileDesc*) MPR_OpenTCPSocket(PRIntn af) {
  return MPR_Socket(af, SOCK_STREAM, 0);
} /* MPR_NewTCPSocket */

PR_IMPLEMENT(PRStatus) MPR_NewTCPSocketPair(PRFileDesc* fds[2]) {
  PRInt32 osfd[2];
  ...
  return PR_SUCCESS;
} /* MPR_NewTCPSocketPair */

PR_IMPLEMENT(PRStatus)
MPR_CreatePipe(PRFileDesc** readPipe, PRFileDesc** writePipe) {
  int pipefd[2];
  ...
  if (pipe(pipefd) == -1) {
    /* XXX map pipe error */
    MPR_SetError(PR_UNKNOWN_ERROR, errno);
    return PR_FAILURE;
  }
...
*/
PR_IMPLEMENT(PRStatus) MPR_SetFDInheritable(PRFileDesc* fd, PRBool inheritable) {
  /*
...
PR_IMPLEMENT(PRFileDesc*) MPR_ImportFile(PRInt32 osfd) {
  PRFileDesc* fd;
...
PR_IMPLEMENT(PRFileDesc*) MPR_ImportPipe(PRInt32 osfd) {
  PRFileDesc* fd;
...
PR_IMPLEMENT(PRFileDesc*) MPR_ImportTCPSocket(PRInt32 osfd) {
  PRFileDesc* fd;
...
PR_IMPLEMENT(PRFileDesc*) MPR_ImportUDPSocket(PRInt32 osfd) {
  PRFileDesc* fd;
...
PR_IMPLEMENT(PRFileDesc*) MPR_CreateSocketPollFd(PRInt32 osfd) {
  PRFileDesc* fd;
...
PR_IMPLEMENT(PRStatus) MPR_DestroySocketPollFd(PRFileDesc* fd) {
  if (NULL == fd) {
    MPR_SetError(PR_BAD_DESCRIPTOR_ERROR, 0);
    return PR_FAILURE;
  }
  ...
  _PR_Putfd(fd);
  return PR_SUCCESS;
} /* MPR_DestroySocketPollFd */

PR_IMPLEMENT(PRInt32) MPR_FileDesc2NativeHandle(PRFileDesc* bottom) {
  PRInt32 osfd = -1;
  bottom =
      (NULL == bottom) ? NULL : MPR_GetIdentitiesLayer(bottom, PR_NSPR_IO_LAYER);
  if (NULL == bottom) {
    MPR_SetError(PR_INVALID_ARGUMENT_ERROR, 0);
  } else {
  ...
  return osfd;
} /* MPR_FileDesc2NativeHandle */

PR_IMPLEMENT(void)
MPR_ChangeFileDescNativeHandle(PRFileDesc* fd, PRInt32 handle) {
  if (fd) {
    fd->secret->md.osfd = handle;
  }
} /*  MPR_ChangeFileDescNativeHandle*/

PR_IMPLEMENT(PRStatus) MPR_LockFile(PRFileDesc* fd) {
...
PR_IMPLEMENT(PRStatus) MPR_TLockFile(PRFileDesc* fd) {
  PRStatus status = PR_SUCCESS;
...
PR_IMPLEMENT(PRStatus) MPR_UnlockFile(PRFileDesc* fd) {
  PRStatus status = PR_SUCCESS;
...
PR_IMPLEMENT(PRInt32) MPR_GetSysfdTableMax(void) {
#  if defined(AIX)
...
PR_IMPLEMENT(PRInt32) MPR_SetSysfdTableSize(PRIntn table_size) {
#  if defined(AIX)
  return -1;
#  else
  struct rlimit rlim;
  PRInt32 tableMax = MPR_GetSysfdTableMax();
...
/*
 * MPR_Stat is supported for backward compatibility; some existing Java
 * code uses it.  New code should use MPR_GetFileInfo.
 */

#  ifndef NO_NSPR_10_SUPPORT
PR_IMPLEMENT(PRInt32) MPR_Stat(const char* name, struct stat* buf) {
  static PRBool unwarned = PR_TRUE;
  if (unwarned) {
    unwarned = _PR_Obsolete("MPR_Stat", "MPR_GetFileInfo");
  }
...
PR_IMPLEMENT(void) MPR_FD_ZERO(PR_fd_set* set) {
  static PRBool unwarned = PR_TRUE;
  if (unwarned) {
    unwarned = _PR_Obsolete("MPR_FD_ZERO (MPR_Select)", "MPR_Poll");
  }
  memset(set, 0, sizeof(PR_fd_set));
}

PR_IMPLEMENT(void) MPR_FD_SET(PRFileDesc* fh, PR_fd_set* set) {
  static PRBool unwarned = PR_TRUE;
  if (unwarned) {
    unwarned = _PR_Obsolete("MPR_FD_SET (MPR_Select)", "MPR_Poll");
  }
...
PR_IMPLEMENT(void) MPR_FD_CLR(PRFileDesc* fh, PR_fd_set* set) {
  PRUint32 index, index2;
  static PRBool unwarned = PR_TRUE;
  if (unwarned) {
    unwarned = _PR_Obsolete("MPR_FD_CLR (MPR_Select)", "MPR_Poll");
  }
...
PR_IMPLEMENT(PRInt32) MPR_FD_ISSET(PRFileDesc* fh, PR_fd_set* set) {
  PRUint32 index;
  static PRBool unwarned = PR_TRUE;
  if (unwarned) {
    unwarned = _PR_Obsolete("MPR_FD_ISSET (MPR_Select)", "MPR_Poll");
  }
  for (index = 0; index < set->hsize; index++)
    if (set->harray[index] == fh) {
      return 1;
...
PR_IMPLEMENT(void) MPR_FD_NSET(PRInt32 fd, PR_fd_set* set) {
  static PRBool unwarned = PR_TRUE;
  if (unwarned) {
    unwarned = _PR_Obsolete("MPR_FD_NSET (MPR_Select)", "MPR_Poll");
  }
...
PR_IMPLEMENT(void) MPR_FD_NCLR(PRInt32 fd, PR_fd_set* set) {
  PRUint32 index, index2;
  static PRBool unwarned = PR_TRUE;
  if (unwarned) {
    unwarned = _PR_Obsolete("MPR_FD_NCLR (MPR_Select)", "MPR_Poll");
  }
...
PR_IMPLEMENT(PRInt32) MPR_FD_NISSET(PRInt32 fd, PR_fd_set* set) {
  PRUint32 index;
  static PRBool unwarned = PR_TRUE;
  if (unwarned) {
    unwarned = _PR_Obsolete("MPR_FD_NISSET (MPR_Select)", "MPR_Poll");
  }
  for (index = 0; index < set->nsize; index++)
    if (set->narray[index] == fd) {
      return 1;
...
PR_IMPLEMENT(PRInt32)
MPR_Select(PRInt32 unused, PR_fd_set* pr_rd, PR_fd_set* pr_wr, PR_fd_set* pr_ex,
          PRIntervalTime timeout) {
  fd_set rd, wr, ex;
  ...
  static PRBool unwarned = PR_TRUE;
  if (unwarned) {
    unwarned = _PR_Obsolete("MPR_Select", "MPR_Poll");
  }
  ...
  if (timeout == PR_INTERVAL_NO_TIMEOUT) {
    ...
    tv.tv_sec = (PRInt32)MPR_IntervalToSeconds(timeout);
    tv.tv_usec = (PRInt32)MPR_IntervalToMicroseconds(
        timeout - MPR_SecondsToInterval(tv.tv_sec));
    tvp = &tv;
    start = MPR_IntervalNow();
  }
  ...
  if (rv == -1 && errno == EINTR) {
    if (timeout == PR_INTERVAL_NO_TIMEOUT) {
      ...
      if (elapsed > timeout) {
        ...
        remaining = timeout - elapsed;
        tv.tv_sec = (PRInt32)MPR_IntervalToSeconds(remaining);
        tv.tv_usec = (PRInt32)MPR_IntervalToMicroseconds(
            remaining - MPR_SecondsToInterval(tv.tv_sec));
        goto retry;
      }
    }
  }
