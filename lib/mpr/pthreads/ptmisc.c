...
PR_IMPLEMENT(void) MPR_SetConcurrency(PRUintn numCPUs) {
#  ifdef SOLARIS
  thr_setconcurrency(numCPUs);
#  else
  PT_LOG("MPR_SetConcurrency");
#  endif
}

PR_IMPLEMENT(void) MPR_SetThreadRecycleMode(PRUint32 flag) {
  PT_LOG("MPR_SetThreadRecycleMode")
}
