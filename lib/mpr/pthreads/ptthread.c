...
#  endif
   ...
   * that thred->id is set before MPR_CreateThread() returns.
...
#  ifdef _PR_NICE_PRIORITY_SCHEDULING
  thred->tid = tid;
  MPR_NotifyAllCondVar(pt_book.cv);
#  endif
  ...
  MPR_Lock(pt_book.ml);
  /*
   * At this moment, MPR_CreateThread() may not have set thred->id yet.
   * It is safe for a detached thread to free thred only after
   * MPR_CreateThread() has accessed thred->id and thred->idSet.
   */
  if (detached) {
    while (!thred->okToDelete) {
      MPR_WaitCondVar(pt_book.cv, PR_INTERVAL_NO_TIMEOUT);
    }
  }
...
static PRThread* pt_AttachThread(void) {
  ...
  /*
   * NSPR must have been initialized when MPR_AttachThread is called.
   * We cannot have MPR_AttachThread call implicit initialization
   * because if multiple threads call MPR_AttachThread simultaneously,
   * NSPR may be initialized more than once.
   * We can't call any function that calls MPR_GetCurrentThread()
   * either (e.g., MPR_SetError()) as that will result in infinite
   * recursion.
   */
  if (!_pr_initialized) {
    return NULL;
  ...
  /* PR_NEWZAP must not call MPR_GetCurrentThread() */
...
#  endif
  ...
  thred = PR_NEWZAP(PRThread);
  if (NULL == thred) {
    MPR_SetError(PR_OUT_OF_MEMORY_ERROR, errno);
    goto done;
  } else {
    pthread_t id;
...
#  endif
    ...
    if (thred->stack == NULL) {
      PRIntn oserr = errno;
      MPR_Free(thred); /* all that work ... poof! */
      MPR_SetError(PR_OUT_OF_MEMORY_ERROR, oserr);
...
#  endif
    ...
    if (0 != rv) {
      ...
      MPR_Free(thred->stack);
      MPR_Free(thred); /* all that work ... poof! */
      MPR_SetError(PR_INSUFFICIENT_RESOURCES_ERROR, oserr);
      thred = NULL; /* and for what? */
      goto done;
    }
     ...
     * MPR_CreateThread() returns.  (See comments in _pt_root().)
     */
    if (!thred->idSet) {
      thred->id = id;
      thred->idSet = PR_TRUE;
    ...
    /*
     * If the new thread is detached, tell it that MPR_CreateThread() has
...
MPR_CreateThread(PRThreadType type, void (*start)(void* arg), void* arg,
...
PR_IMPLEMENT(PRThread*)
MPR_CreateThreadGCAble(PRThreadType type, void (*start)(void* arg), void* arg,
                      PRThreadPriority priority, PRThreadScope scope,
                      PRThreadState state, PRUint32 stackSize) {
  return _PR_CreateThread(type, start, arg, priority, scope, state, stackSize,
                          PR_TRUE);
} /* MPR_CreateThreadGCAble */

PR_IMPLEMENT(void*) MPR_GetExecutionEnvironment(PRThread* thred) {
  return thred->environment;
} /* MPR_GetExecutionEnvironment */

PR_IMPLEMENT(void) MPR_SetExecutionEnvironment(PRThread* thred, void* env) {
  thred->environment = env;
} /* MPR_SetExecutionEnvironment */

PR_IMPLEMENT(PRThread*)
MPR_AttachThread(PRThreadType type, PRThreadPriority priority,
                PRThreadStack* stack) {
  return MPR_GetCurrentThread();
} /* MPR_AttachThread */

PR_IMPLEMENT(PRStatus) MPR_JoinThread(PRThread* thred) {
  int rv = -1;
  void* result = NULL;
  ...
  if ((0xafafafaf == thred->state) ||
    ...
    MPR_LogPrint("MPR_JoinThread: %p not joinable | already smashed\n", thred);
  } else {
    ...
    if (0 == rv) {
      ...
      MPR_SetError(prerror, rv);
    }
  }
  return (0 == rv) ? PR_SUCCESS : PR_FAILURE;
} /* MPR_JoinThread */

PR_IMPLEMENT(void) MPR_DetachThread(void) {
  void* thred;
  int rv;
...
PR_IMPLEMENT(PRThread*) MPR_GetCurrentThread(void) {
  void* thred;
...
PR_IMPLEMENT(PRThreadScope) MPR_GetThreadScope(const PRThread* thred) {
  return (thred->state & PT_THREAD_BOUND) ? PR_GLOBAL_BOUND_THREAD
                                          : PR_GLOBAL_THREAD;
} /* MPR_GetThreadScope() */

PR_IMPLEMENT(PRThreadType) MPR_GetThreadType(const PRThread* thred) {
  return (thred->state & PT_THREAD_SYSTEM) ? PR_SYSTEM_THREAD : PR_USER_THREAD;
}

PR_IMPLEMENT(PRThreadState) MPR_GetThreadState(const PRThread* thred) {
  return (thred->state & PT_THREAD_DETACHED) ? PR_UNJOINABLE_THREAD
                                             : PR_JOINABLE_THREAD;
} /* MPR_GetThreadState */

PR_IMPLEMENT(PRThreadPriority) MPR_GetThreadPriority(const PRThread* thred) {
  PR_ASSERT(thred != NULL);
  return thred->priority;
} /* MPR_GetThreadPriority */

PR_IMPLEMENT(void)
MPR_SetThreadPriority(PRThread* thred, PRThreadPriority newPri) {
  PRIntn rv;
  ...
  if (EPERM != pt_schedpriv) {
    ...
    if (0 == rv) {
      ...
      if (EPERM == rv) {
        pt_schedpriv = EPERM;
        PR_LOG(_pr_thread_lm, PR_LOG_MIN,
               ("MPR_SetThreadPriority: no thread scheduling privilege"));
      }
    }
    if (rv != 0) {
      rv = -1;
...
#  elif defined(_PR_NICE_PRIORITY_SCHEDULING)
  ...
  if (errno == 0) {
    ...
    if (rv == -1) {
      ...
      PR_LOG(_pr_thread_lm, PR_LOG_MIN,
             ("MPR_SetThreadPriority: setpriority failed with error %d", errno));
    }
  }
#  else
...
} /* MPR_SetThreadPriority */

PR_IMPLEMENT(PRStatus) MPR_Interrupt(PRThread* thred) {
  /*
  ...
  */
  ...
  if ((NULL != cv) && !thred->interrupt_blocked) {
    ...
    if (0 > PR_ATOMIC_DECREMENT(&cv->notify_pending)) {
      MPR_DestroyCondVar(cv);
    }
  }
  return PR_SUCCESS;
} /* MPR_Interrupt */

PR_IMPLEMENT(void) MPR_ClearInterrupt(void) {
  PRThread* me = MPR_GetCurrentThread();
  me->state &= ~PT_THREAD_ABORTED;
} /* MPR_ClearInterrupt */

PR_IMPLEMENT(void) MPR_BlockInterrupt(void) {
  PRThread* me = MPR_GetCurrentThread();
  _PT_THREAD_BLOCK_INTERRUPT(me);
} /* MPR_BlockInterrupt */

PR_IMPLEMENT(void) MPR_UnblockInterrupt(void) {
  PRThread* me = MPR_GetCurrentThread();
  _PT_THREAD_UNBLOCK_INTERRUPT(me);
} /* MPR_UnblockInterrupt */

PR_IMPLEMENT(PRStatus) MPR_Yield(void) {
  static PRBool warning = PR_TRUE;
  if (warning)
    warning = _PR_Obsolete("MPR_Yield()", "MPR_Sleep(PR_INTERVAL_NO_WAIT)");
  return MPR_Sleep(PR_INTERVAL_NO_WAIT);
}

PR_IMPLEMENT(PRStatus) MPR_Sleep(PRIntervalTime ticks) {
  ...
  if (PR_INTERVAL_NO_WAIT == ticks) {
    ...
    timein = MPR_IntervalNow();
    cv = MPR_NewCondVar(_pr_sleeplock);
    PR_ASSERT(cv != NULL);
    MPR_Lock(_pr_sleeplock);
    do {
      ...
      rv = MPR_WaitCondVar(cv, ticks - delta);
    } while (PR_SUCCESS == rv);
    MPR_Unlock(_pr_sleeplock);
    MPR_DestroyCondVar(cv);
  }
  return rv;
} /* MPR_Sleep */
...
static void _pt_thread_death(void* arg) {
  ...
  if (NULL == thred) {
    /*
     * Have MPR_GetCurrentThread return the expected value to the
     * destructors.
     */
...
static void _pt_thread_death_internal(void* arg, PRBool callDestructors) {
  ...
  MPR_Free(thred->privateData);
  if (NULL != thred->errorString) {
    MPR_Free(thred->errorString);
  }
  if (NULL != thred->name) {
    MPR_Free(thred->name);
  }
...
#  endif
  ...
  pt_book.ml = MPR_NewLock();
  PR_ASSERT(NULL != pt_book.ml);
  pt_book.cv = MPR_NewCondVar(pt_book.ml);
...
#  endif
  ...
  if (0 != rv) {
    MPR_Assert("0 == rv", __FILE__, __LINE__);
  }
...
void _PR_Fini(void) {
  ...
  if (!_pr_initialized) {
     ...
     * MPR_Cleanup has been called already. */
...
PR_IMPLEMENT(PRStatus) MPR_Cleanup(void) {
  PRThread* me = MPR_GetCurrentThread();
  int rv;
  PR_LOG(_pr_thread_lm, PR_LOG_MIN, ("MPR_Cleanup: shutting down NSPR"));
  ...
  if (me->state & PT_THREAD_PRIMORD) {
     ...
     */
    if (0 == pt_book.system) {
      MPR_DestroyCondVar(pt_book.cv);
      pt_book.cv = NULL;
      MPR_DestroyLock(pt_book.ml);
      pt_book.ml = NULL;
    }
    MPR_DestroyLock(_pr_sleeplock);
    _pr_sleeplock = NULL;
...
PR_IMPLEMENT(void) MPR_ProcessExit(PRIntn status) { _exit(status); }

PR_IMPLEMENT(PRUint32) MPR_GetThreadID(PRThread* thred) {
...
MPR_GetThreadAffinityMask(PRThread* thread, PRUint32* mask) {
  return 0; /* not implemented */
}

PR_IMPLEMENT(PRInt32)
MPR_SetThreadAffinityMask(PRThread* thread, PRUint32 mask) {
  return 0; /* not implemented */
}

PR_IMPLEMENT(void)
MPR_SetThreadDumpProc(PRThread* thread, PRThreadDumpProc dump, void* arg) {
  thread->dump = dump;
  thread->dumpArg = arg;
}
...
PR_IMPLEMENT(void) MPR_SetThreadGCAble(void) {
  MPR_Lock(pt_book.ml);
  MPR_GetCurrentThread()->state |= PT_THREAD_GCABLE;
  MPR_Unlock(pt_book.ml);
}

PR_IMPLEMENT(void) MPR_ClearThreadGCAble(void) {
  MPR_Lock(pt_book.ml);
  MPR_GetCurrentThread()->state &= (~PT_THREAD_GCABLE);
  MPR_Unlock(pt_book.ml);
}
...
PR_IMPLEMENT(PRStatus) MPR_EnumerateThreads(PREnumerator func, void* arg) {
  PRIntn count = 0;
  PRStatus rv = PR_SUCCESS;
...
#  endif
  ...
  PR_LOG(_pr_gc_lm, PR_LOG_ALWAYS, ("Begin MPR_EnumerateThreads\n"));
  /*
   * $$$
  ...
  while (thred != NULL) {
    ...
    if (_PT_IS_GCABLE_THREAD(thred)) {
      ...
      PR_LOG(_pr_gc_lm, PR_LOG_ALWAYS,
             ("In MPR_EnumerateThreads callback thread %p thid = %X\n", thred,
              thred->id));
  ...
  PR_LOG(_pr_gc_lm, PR_LOG_ALWAYS,
         ("End MPR_EnumerateThreads count = %d \n", count));
  return rv;
} /* MPR_EnumerateThreads */

/*
 * MPR_SuspendAll and MPR_ResumeAll are called during garbage collection.  The
...
#  endif
   ...
   * So the signal handler sets a flag which MPR_ResumeAll will reset.
   * The MPR_ResumeAll must reset this flag ...
   */
...
PR_IMPLEMENT(void) MPR_SuspendAll(void) {
#  ifdef DEBUG
...
#  endif
  ...
  PR_ASSERT(0 == rv);
  PR_LOG(_pr_gc_lm, PR_LOG_ALWAYS, ("Begin MPR_SuspendAll\n"));
  /*
...
#  ifdef DEBUG
  etime = MPR_IntervalNow();
  PR_LOG(_pr_gc_lm, PR_LOG_ALWAYS,
         ("End MPR_SuspendAll (time %dms)\n",
          MPR_IntervalToMilliseconds(etime - stime)));
#  endif
} /* MPR_SuspendAll */

PR_IMPLEMENT(void) MPR_ResumeAll(void) {
#  ifdef DEBUG
  PRIntervalTime stime, etime;
#  endif
  PRThread* thred = pt_book.first;
  PRThread* me = MPR_GetCurrentThread();
  PR_LOG(_pr_gc_lm, PR_LOG_ALWAYS, ("Begin MPR_ResumeAll\n"));
  /*
...
#  ifdef DEBUG
  ...
  etime = MPR_IntervalNow();
  PR_LOG(_pr_gc_lm, PR_LOG_ALWAYS,
         ("End MPR_ResumeAll (time %dms)\n",
          MPR_IntervalToMilliseconds(etime - stime)));
#  endif
} /* MPR_ResumeAll */

/* Return the stack pointer for the given thread- used by the GC */
PR_IMPLEMENT(void*) MPR_GetSP(PRThread* thred) {
  PR_LOG(_pr_gc_lm, PR_LOG_ALWAYS,
         ("in MPR_GetSP thred %p thid = %X, sp = %p\n", thred, thred->id,
          thred->sp));
  return thred->sp;
} /* MPR_GetSP */

PR_IMPLEMENT(PRStatus) MPR_SetCurrentThreadName(const char* name) {
  PRThread* thread;
  size_t nameLen;
  int result = 0;
  ...
  thread = MPR_GetCurrentThread();
  if (!thread) {
    return PR_FAILURE;
  }

  MPR_Free(thread->name);
  nameLen = strlen(name);
  thread->name = (char*)MPR_Malloc(nameLen + 1);
  if (!thread->name) {
    return PR_FAILURE;
  }
...
#  endif /* not BSD */
  ...
  if (result) {
    MPR_SetError(PR_UNKNOWN_ERROR, result);
    return PR_FAILURE;
  }
  return PR_SUCCESS;
}

PR_IMPLEMENT(const char*) MPR_GetThreadName(const PRThread* thread) {
  if (!thread) {
    return NULL;
  }
  return thread->name;
