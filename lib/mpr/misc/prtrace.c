...
static void _PR_InitializeTrace(void) {
  ...
  logLock = MPR_NewLock();
  logCVar = MPR_NewCondVar(logLock);

  MPR_Unlock(traceLock);
  return;
...
MPR_CreateTrace(const char* qName,      /* QName for this trace handle */
...
*/
PR_IMPLEMENT(void)
MPR_DestroyTrace(PRTraceHandle handle /* Handle to be destroyed */
) {
  ...
  */
  ...
  MPR_Unlock(traceLock);
  return;
} /* end MPR_DestroyTrace()  */

/*
** Create a TraceEntry in the trace buffer
*/
PR_IMPLEMENT(void)
MPR_Trace(PRTraceHandle handle, /* use this trace handle */
  ...
  */
  tep->thread = MPR_GetCurrentThread();
  tep->handle = handle;
  tep->time = MPR_Now();
  ...
  if ((mark % logEntriesPerSegment) == 0) {
    MPR_Lock(logLock);
    logCount++;
    MPR_NotifyCondVar(logCVar);
    MPR_Unlock(logLock);
    /*
    ** Gh0D! This is awful!
    ** Anyway, to minimize lost trace data segments,
    ** I inserted the MPR_Sleep(0) to cause a context switch
...
*/
PR_IMPLEMENT(void)
MPR_SetTraceOption(PRTraceOption command, /* One of the enumerated values */
  ...
  return;
} /* end  MPR_SetTraceOption() */

/*
**
*/
PR_IMPLEMENT(void)
MPR_GetTraceOption(PRTraceOption command, /* One of the enumerated values */
                  void* value            /* command value or NULL */
) {
  switch (command) {
    case PRTraceBufSize:
  ...
  return;
} /* end MPR_GetTraceOption() */

/*
**
*/
PR_IMPLEMENT(PRTraceHandle)
MPR_GetTraceHandleFromName(const char* qName, /* QName search argument */
  ...
  qh = MPR_FindNextTraceQname(NULL);
  while (qh != NULL) {
    rh = MPR_FindNextTraceRname(NULL, qh);
    while (rh != NULL) {
      MPR_GetTraceNameFromHandle(rh, &qn, &rn, &desc);
      if ((strcmp(qName, qn) == 0) && (strcmp(rName, rn) == 0)) {
        rnp = (RName*)rh;
        goto foundIt;
      }
      rh = MPR_FindNextTraceRname(rh, qh);
    }
    qh = MPR_FindNextTraceQname(NULL);
  }
...
foundIt:
  ...
  return (rh);
} /* end MPR_GetTraceHandleFromName() */
...
MPR_GetTraceNameFromHandle(
  ...
  *description = rnp->desc;
  ...
  return;
} /* end MPR_GetTraceNameFromHandle() */

/*
**
*/
PR_IMPLEMENT(PRTraceHandle)
MPR_FindNextTraceQname(PRTraceHandle handle) {
...
} /* end MPR_FindNextTraceQname() */

/*
**
*/
PR_IMPLEMENT(PRTraceHandle)
MPR_FindNextTraceRname(PRTraceHandle rhandle, PRTraceHandle qhandle) {
...
static PRFileDesc* InitializeRecording(void) {
  ...
  PR_LOG(lm, PR_LOG_DEBUG, ("MPR_RecordTraceEntries: begins"));

  logLostData = 0; /* reset at entry */
  logState = LogReset;

  /* Get the filename for the logfile from the environment */
  logFileName = MPR_GetEnvSecure("NSPR_TRACE_LOG");
  if (logFileName == NULL) {
    PR_LOG(lm, PR_LOG_ERROR,
  ...
  /* Open the logfile */
  logFile = MPR_Open(logFileName, PR_WRONLY | PR_CREATE_FILE, 0666);
  if (logFile == NULL) {
    PR_LOG(
        lm, PR_LOG_ERROR,
        ("RecordTraceEntries: Cannot open %s as trace log file. OS error: %ld",
         logFileName, MPR_GetOSError()));
    return NULL;
  }
  return logFile;
...
static void WriteTraceSegment(PRFileDesc* logFile, void* buf, PRInt32 amount) {
  ...
  rc = MPR_Write(logFile, buf, amount);
  if (rc == -1)
    PR_LOG(
        lm, PR_LOG_ERROR,
        ("RecordTraceEntries: MPR_Write() failed. Error: %ld", MPR_GetError()));
  else if (rc != amount)
    PR_LOG(lm, PR_LOG_ERROR,
           ("RecordTraceEntries: MPR_Write() Tried to write: %ld, Wrote: %ld",
            amount, rc));
  else
    PR_LOG(lm, PR_LOG_DEBUG,
           ("RecordTraceEntries: MPR_Write(): Buffer: %p, bytes: %ld", buf,
            amount));
...
MPR_RecordTraceEntries(void) {
  ...
  if (logFile == NULL) {
    PR_LOG(lm, PR_LOG_DEBUG, ("MPR_RecordTraceEntries: Failed to initialize"));
    return;
  }
  ...
  while (logState != LogStop) {
    ...
    while ((logCount == 0) && (logOrder == logState)) {
      MPR_WaitCondVar(logCVar, PR_INTERVAL_NO_TIMEOUT);
    }
    ...
    if (logCount) {
      ...
      if (lostSegments > 0) {
        ...
        PR_LOG(lm, PR_LOG_DEBUG,
               ("MPR_RecordTraceEntries: LostData segments: %ld", logLostData));
      } else {
        logCount--;
      }
    ...
    if (doWrite == PR_TRUE) {
      if (localState != LogSuspend) {
        ...
        PR_LOG(lm, PR_LOG_DEBUG,
               ("RecordTraceEntries: MPR_Write(): is suspended"));
    }
  ...
  return;
} /* end  MPR_RecordTraceEntries() */

/*
**
*/
PR_IMPLEMENT(PRIntn)
MPR_GetTraceEntries(PRTraceEntry* buffer, /* where to write output */
  ...
  */
  PR_LOG(lm, PR_LOG_ERROR,
         ("MPR_GetTraceEntries: Next: %ld, LastSeen: %ld", next, fetchLastSeen));
  ...
  if (fetchLastSeen <= next) {
    ...
    PR_LOG(lm, PR_LOG_ERROR,
           ("MPR_GetTraceEntries: Copied: %ld, LastSeen: %ld", copied,
            fetchLastSeen));
