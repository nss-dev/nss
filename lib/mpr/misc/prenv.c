...
#  define _PR_NEW_LOCK_ENV() \
    { _pr_envLock = MPR_NewLock(); }
#  define _PR_DELETE_LOCK_ENV()      \
    {                                \
      if (_pr_envLock) {             \
        MPR_DestroyLock(_pr_envLock); \
        _pr_envLock = NULL;          \
      }                              \
    }
#  define _PR_LOCK_ENV()                     \
    {                                        \
      if (_pr_envLock) MPR_Lock(_pr_envLock); \
    }
#  define _PR_UNLOCK_ENV()                     \
    {                                          \
      if (_pr_envLock) MPR_Unlock(_pr_envLock); \
    }
#endif
...
PR_IMPLEMENT(char*) MPR_GetEnv(const char* var) {
  char* ev;
...
PR_IMPLEMENT(char*) MPR_GetEnvSecure(const char* var) {
...
#  endif /* XP_UNIX */
  return MPR_GetEnv(var);
#endif   /* HAVE_SECURE_GETENV */
}

PR_IMPLEMENT(PRStatus) MPR_SetEnv(const char* string) {
  PRIntn result;
...
PR_IMPLEMENT(char**) MPR_DuplicateEnvironment(void) {
...
#  endif
  ...
  result = (char**)MPR_Malloc(sizeof(char*) * (end - the_environ + 1));
  if (result != NULL) {
...
PR_IMPLEMENT(char**) MPR_DuplicateEnvironment(void) { return NULL; }
#endif
