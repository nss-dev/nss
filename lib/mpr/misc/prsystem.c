...
PR_IMPLEMENT(char) MPR_GetDirectorySeparator(void) {
  return PR_DIRECTORY_SEPARATOR;
} /* MPR_GetDirectorySeparator */

/*
** OBSOLETE -- the function name is misspelled.
*/
PR_IMPLEMENT(char) MPR_GetDirectorySepartor(void) {
#if defined(DEBUG)
  static PRBool warn = PR_TRUE;
  if (warn) {
    warn =
        _PR_Obsolete("MPR_GetDirectorySepartor()", "MPR_GetDirectorySeparator()");
  }
#endif
  return MPR_GetDirectorySeparator();
} /* MPR_GetDirectorySepartor */

PR_IMPLEMENT(char) MPR_GetPathSeparator(void) {
  return PR_PATH_SEPARATOR;
} /* MPR_GetPathSeparator */

PR_IMPLEMENT(PRStatus)
MPR_GetSystemInfo(PRSysInfo cmd, char* buf, PRUint32 buflen) {
  PRUintn len = 0;
  ...
  switch (cmd) {
...
#else
      (void)MPR_snprintf(buf, buflen, _PR_SI_SYSNAME);
#endif
      break;
    ...
    case PR_SI_ARCHITECTURE:
      ...
      (void)MPR_snprintf(buf, buflen, _PR_SI_ARCHITECTURE);
      break;
    default:
...
**
*/
PR_IMPLEMENT(PRInt32) MPR_GetNumberOfProcessors(void) {
  PRInt32 numCpus;
#if defined(WIN32)
  SYSTEM_INFO info;
...
#endif
  return (numCpus);
} /* end MPR_GetNumberOfProcessors() */

/*
** MPR_GetPhysicalMemorySize()
**
** Implementation notes:
**   Every platform does it a bit different.
...
**
*/
PR_IMPLEMENT(PRUint64) MPR_GetPhysicalMemorySize(void) {
  PRUint64 bytes = 0;
