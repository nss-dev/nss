...
#  define LOCK_DNS() MPR_Lock(_pr_dnsLock)
#  define UNLOCK_DNS() MPR_Unlock(_pr_dnsLock)
...
static void _pr_PrintIfreq(struct ifreq* ifr) {
  ...
  MPR_NetAddrToString(&addr, addrstr, sizeof(addrstr));
...
/*
 * Emulate the code in NSPR 4.2 or older.  MPR_GetIPNodeByName behaves
...
#endif
#if !defined(_PR_NO_DNS_LOCK)
  _pr_dnsLock = MPR_NewLock();
#endif
#if !defined(_PR_HAVE_GETPROTO_R)
  _getproto_lock = MPR_NewLock();
#endif
#if defined(_PR_INET6) && defined(_PR_HAVE_GETHOSTBYNAME2)
  _pr_query_ifs_lock = MPR_NewLock();
#endif
}
...
#if !defined(_PR_NO_DNS_LOCK)
  if (_pr_dnsLock) {
    MPR_DestroyLock(_pr_dnsLock);
    _pr_dnsLock = NULL;
  }
#endif
#if !defined(_PR_HAVE_GETPROTO_R)
  if (_getproto_lock) {
    MPR_DestroyLock(_getproto_lock);
    _getproto_lock = NULL;
  }
#endif
#if defined(_PR_INET6) && defined(_PR_HAVE_GETHOSTBYNAME2)
  if (_pr_query_ifs_lock) {
    MPR_DestroyLock(_pr_query_ifs_lock);
 ...
 * or arguments of MPR_GetHostByName, MPR_GetIPNodeByName, and
 * MPR_GetHostByAddr.  DO NOT CHANGE THE NAMES OF THESE LOCAL
...
MPR_GetHostByName(const char* name, char* buf, PRIntn bufsize, PRHostEnt* hp) {
  struct hostent* h;
  ...
  if (bufsize > sizeof(localbuf)) {
    tmpbuf = (char*)MPR_Malloc(bufsize);
    if (NULL == tmpbuf) {
  ...
  if (NULL == h) {
    MPR_SetError(PR_DIRECTORY_LOOKUP_ERROR, _MD_GETHOST_ERRNO());
  } else {
    _PRIPAddrConversion conversion = _PRIPAddrNoConversion;
    rv = CopyHostent(h, &buf, &bufsize, conversion, hp);
    if (PR_SUCCESS != rv) {
      MPR_SetError(PR_INSUFFICIENT_RESOURCES_ERROR, 0);
    }
  }
  UNLOCK_DNS();
#if defined(_PR_HAVE_GETHOST_R)
...
#  define FREEHOSTENT "freehostent"
  ...
  _pr_getipnodebyname_fp = MPR_FindSymbolAndLibrary(GETIPNODEBYNAME, &lib);
  if (NULL != _pr_getipnodebyname_fp) {
    _pr_freehostent_fp = MPR_FindSymbol(lib, FREEHOSTENT);
    if (NULL != _pr_freehostent_fp) {
      _pr_getipnodebyaddr_fp = MPR_FindSymbol(lib, GETIPNODEBYADDR);
...
PR_IMPLEMENT(PRStatus)
MPR_GetIPNodeByName(const char* name, PRUint16 af, PRIntn flags, char* buf,
...
#  endif
  ...
  if (NULL == h) {
    ...
    MPR_SetError(PR_DIRECTORY_LOOKUP_ERROR, error_num);
#elif defined(_PR_INET6_PROBE) && defined(_PR_HAVE_GETIPNODEBYNAME)
    if (_pr_ipv6_is_present()) {
      MPR_SetError(PR_DIRECTORY_LOOKUP_ERROR, error_num);
    } else {
...
PR_IMPLEMENT(PRStatus)
MPR_GetHostByAddr(const PRNetAddr* hostaddr, char* buf, PRIntn bufsize,
                 PRHostEnt* hostentry) {
  struct hostent* h;
  PRStatus rv = PR_FAILURE;
  const void* addr;
...
PR_IMPLEMENT(PRStatus)
MPR_GetProtoByName(const char* name, char* buffer, PRInt32 buflen,
                  PRProtoEnt* result) {
  ...
  if (NULL == getprotobyname_r(name, res, buffer, buflen)) {
    MPR_SetError(PR_DIRECTORY_LOOKUP_ERROR, _MD_ERRNO());
    return PR_FAILURE;
  }
...
PR_IMPLEMENT(PRStatus)
MPR_GetProtoByNumber(PRInt32 number, char* buffer, PRInt32 buflen,
                    PRProtoEnt* result) {
  PRStatus rv = PR_SUCCESS;
#if defined(_PR_HAVE_GETPROTO_R)
  struct protoent* res = (struct protoent*)result;
...
PR_IMPLEMENT(PRIntn)
MPR_EnumerateHostEnt(PRIntn enumIndex, const PRHostEnt* hostEnt, PRUint16 port,
                    PRNetAddr* address) {
  ...
  return enumIndex;
} /* MPR_EnumerateHostEnt */

PR_IMPLEMENT(PRStatus)
MPR_InitializeNetAddr(PRNetAddrValue val, PRUint16 port, PRNetAddr* addr) {
  PRStatus rv = PR_SUCCESS;
  if (!_pr_initialized) {
    _PR_ImplicitInitialization();
  }
  ...
  return rv;
} /* MPR_InitializeNetAddr */

PR_IMPLEMENT(PRStatus)
MPR_SetNetAddr(PRNetAddrValue val, PRUint16 af, PRUint16 port, PRNetAddr* addr) {
  PRStatus rv = PR_SUCCESS;
  if (!_pr_initialized) {
    _PR_ImplicitInitialization();
  }
...
PR_IMPLEMENT(PRBool)
MPR_IsNetAddrType(const PRNetAddr* addr, PRNetAddrValue val) {
 ...
 */
PR_IMPLEMENT(void)
MPR_ConvertIPv4AddrToIPv6(PRUint32 v4addr, PRIPv6Addr* v6addr) {
  PRUint8* dstp;
...
PR_IMPLEMENT(PRUint16) MPR_ntohs(PRUint16 n) { return ntohs(n); }
PR_IMPLEMENT(PRUint32) MPR_ntohl(PRUint32 n) { return ntohl(n); }
PR_IMPLEMENT(PRUint16) MPR_htons(PRUint16 n) { return htons(n); }
PR_IMPLEMENT(PRUint32) MPR_htonl(PRUint32 n) { return htonl(n); }
PR_IMPLEMENT(PRUint64) MPR_ntohll(PRUint64 n) {
#ifdef IS_BIG_ENDIAN
  return n;
#else
  PRUint32 hi, lo;
...
PR_IMPLEMENT(PRUint64) MPR_htonll(PRUint64 n) {
#ifdef IS_BIG_ENDIAN
  return n;
#else
  PRUint32 hi, lo;
...
/*
 * Implementation of MPR_GetAddrInfoByName and friends
 *
 ...
 *                        getaddrinfo, falling back to MPR_GetHostByName if
...
#    ifdef WIN32
  ...
  for (i = 0; i < sizeof(libname) / sizeof(libname[0]); i++) {
    lib = MPR_LoadLibrary(libname[i]);
    if (!lib) {
      continue;
    }
    _pr_getaddrinfo =
        (FN_GETADDRINFO)MPR_FindFunctionSymbol(lib, GETADDRINFO_SYMBOL);
    if (!_pr_getaddrinfo) {
      MPR_UnloadLibrary(lib);
      continue;
    }
    _pr_freeaddrinfo =
        (FN_FREEADDRINFO)MPR_FindFunctionSymbol(lib, FREEADDRINFO_SYMBOL);
    _pr_getnameinfo =
        (FN_GETNAMEINFO)MPR_FindFunctionSymbol(lib, GETNAMEINFO_SYMBOL);
...
#    else
   ...
   */
  _pr_getaddrinfo =
      (FN_GETADDRINFO)MPR_FindFunctionSymbolAndLibrary(GETADDRINFO_SYMBOL, &lib);
  if (!_pr_getaddrinfo) {
    return PR_FAILURE;
  }
  _pr_freeaddrinfo =
 ...
 * MPR_GetHostByName, which requires that we allocate a buffer for the
...
static PRAddrInfo* pr_GetAddrInfoByNameFB(const char* hostname, PRUint16 af,
  ...
  PRStatus rv;
  PRAddrInfoFB* ai;
  /* fallback on MPR_GetHostByName */
  ai = PR_NEW(PRAddrInfoFB);
  if (!ai) {
    MPR_SetError(PR_OUT_OF_MEMORY_ERROR, 0);
    return NULL;
  }
  rv = MPR_GetHostByName(hostname, ai->buf, sizeof ai->buf, &ai->hostent);
  if (rv == PR_FAILURE) {
    MPR_Free(ai);
    return NULL;
  }
...
MPR_GetAddrInfoByName(const char* hostname, PRUint16 af, PRIntn flags) {
...
#  endif
    ...
    MPR_SetError(PR_DIRECTORY_LOOKUP_ERROR, rv);
  }
  return NULL;
#endif
}
...
PR_IMPLEMENT(PRStatus)
MPR_GetPrefLoopbackAddrInfo(PRNetAddr* result, PRUint16 port) {
  char tmpBuf[40];
  ...
  MPR_snprintf(tmpBuf, tmpBufSize, "%u", port);
...
PR_IMPLEMENT(void) MPR_FreeAddrInfo(PRAddrInfo* ai) {
#if defined(_PR_HAVE_GETADDRINFO)
#  if defined(_PR_INET6_PROBE)
  if (!_pr_ipv6_is_present()) {
    MPR_Free((PRAddrInfoFB*)ai);
  } else
#  endif
...
PR_IMPLEMENT(void*)
MPR_EnumerateAddrInfo(void* iterPtr, const PRAddrInfo* base, PRUint16 port,
                     PRNetAddr* result) {
#if defined(_PR_HAVE_GETADDRINFO)
  PRADDRINFO* ai;
#  if defined(_PR_INET6_PROBE)
  if (!_pr_ipv6_is_present()) {
    ...
    iter = MPR_EnumerateHostEnt(iter, &((PRAddrInfoFB*)base)->hostent, port,
                               result);
    if (iter < 0) {
      iter = 0;
    }
...
#  endif
  ...
  iter =
      MPR_EnumerateHostEnt(iter, &((PRAddrInfoFB*)base)->hostent, port, result);
  if (iter < 0) {
    iter = 0;
  }
  return (void*)iter;
...
PR_IMPLEMENT(const char*) MPR_GetCanonNameFromAddrInfo(const PRAddrInfo* ai) {
#if defined(_PR_HAVE_GETADDRINFO)
#  if defined(_PR_INET6_PROBE)
  if (!_pr_ipv6_is_present()) {
...
static PRStatus pr_StringToNetAddrGAI(const char* string, PRNetAddr* addr) {
  ...
  if (rv != 0) {
    MPR_SetError(PR_INVALID_ARGUMENT_ERROR, rv);
    return PR_FAILURE;
  }
...
PR_IMPLEMENT(PRStatus) MPR_StringToNetAddr(const char* string, PRNetAddr* addr) {
  if (!_pr_initialized) {
    _PR_ImplicitInitialization();
  }
...
#  endif
    ...
    if (AF_INET != addr->raw.family) {
      ...
      MPR_snprintf(string, size, "%u.%u.%u.%u", byte[0], byte[1], byte[2],
                  byte[3]);
    }
  }
...
PR_IMPLEMENT(PRStatus)
MPR_NetAddrToString(const PRNetAddr* addr, char* string, PRUint32 size) {
  if (!_pr_initialized) {
    _PR_ImplicitInitialization();
  }
