    ...
    while (why == scan) {
      ...
      } else if (why == scan) /* the dominant case */
      {
        ...
        if (id == NULL) {                /* there are no alarms set */
          (void)MPR_WaitCondVar(alarm->cond, PR_INTERVAL_NO_TIMEOUT);
        } else {
          pause = id->nextNotify - (MPR_IntervalNow() - id->epoch);
          if ((PRInt32)pause <= 0) /* is this one's time up? */
          {
            why = notify;        /* set up to do our thing */
            alarm->current = id; /* id we're about to schedule */
          } else {
            (void)MPR_WaitCondVar(alarm->cond, pause); /* dally */
          }
        }
      }
    }
    MPR_Unlock(alarm->lock);
...
PR_IMPLEMENT(PRAlarm*) MPR_CreateAlarm(void) {
  PRAlarm* alarm = PR_NEWZAP(PRAlarm);
  if (alarm != NULL) {
    if ((alarm->lock = MPR_NewLock()) == NULL) {
      goto done;
    }
    if ((alarm->cond = MPR_NewCondVar(alarm->lock)) == NULL) {
      goto done;
    }
    alarm->state = alarm_active;
    PR_INIT_CLIST(&alarm->timers);
    alarm->notifier =
        MPR_CreateThread(PR_USER_THREAD, pr_alarmNotifier, alarm,
                        MPR_GetThreadPriority(MPR_GetCurrentThread()),
  ...
  if (alarm->cond != NULL) {
    MPR_DestroyCondVar(alarm->cond);
  }
  if (alarm->lock != NULL) {
    MPR_DestroyLock(alarm->lock);
  }
  PR_DELETE(alarm);
  return NULL;
} /* CreateAlarm */

PR_IMPLEMENT(PRStatus) MPR_DestroyAlarm(PRAlarm* alarm) {
  PRStatus rv;

  MPR_Lock(alarm->lock);
  alarm->state = alarm_inactive;
  rv = MPR_NotifyCondVar(alarm->cond);
  MPR_Unlock(alarm->lock);

  if (rv == PR_SUCCESS) {
    rv = MPR_JoinThread(alarm->notifier);
  }
  if (rv == PR_SUCCESS) {
    MPR_DestroyCondVar(alarm->cond);
    MPR_DestroyLock(alarm->lock);
...
MPR_SetAlarm(PRAlarm* alarm, PRIntervalTime period, PRUint32 rate,
  ...
  id->period = period;
  id->rate = rate;
  id->epoch = id->nextNotify = MPR_IntervalNow();
  (void)pr_PredictNextNotifyTime(id);

  MPR_Lock(alarm->lock);
  PR_INSERT_BEFORE(&id->list, &alarm->timers);
  MPR_NotifyCondVar(alarm->cond);
  MPR_Unlock(alarm->lock);
...
MPR_ResetAlarm(PRAlarmID* id, PRIntervalTime period, PRUint32 rate) {
  /*
