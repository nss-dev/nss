...
PR_IMPLEMENT(void)
MPR_ExplodeTime(PRTime usecs, PRTimeParamFn params, PRExplodedTime* exploded) {
 ...
 */
PR_IMPLEMENT(PRTime)
MPR_ImplodeTime(const PRExplodedTime* exploded) {
  PRExplodedTime copy;
  PRTime retVal;
  ...
  copy = *exploded;
  MPR_NormalizeTime(&copy, MPR_GMTParameters);
...
PR_IMPLEMENT(void)
MPR_NormalizeTime(PRExplodedTime* time, PRTimeParamFn params) {
  int daysInMonth;
  PRInt32 numDays;
...
static struct tm* MT_safe_localtime(const time_t* clock, struct tm* result) {
  struct tm* tmPtr;
  int needLock = MPR_Initialized(); /* We need to use a lock to protect
...
MPR_LocalTimeParameters(const PRExplodedTime* gmt) {
  ...
  secs64 = MPR_ImplodeTime(gmt); /* This is still in microseconds */
...
PR_IMPLEMENT(PRTimeParameters)
MPR_USPacificTimeParameters(const PRExplodedTime* gmt) {
  const DSTParams* dst;
  PRTimeParameters retVal;
  PRExplodedTime st;
...
PR_IMPLEMENT(PRTimeParameters)
MPR_GMTParameters(const PRExplodedTime* gmt) {
  PRTimeParameters retVal = {0, 0};
  return retVal;
}

/*
 * The following code implements MPR_ParseTimeString().  It is based on
...
MPR_ParseTimeStringToExplodedTime(const char* string, PRBool default_to_gmt,
   ...
   */
  MPR_NormalizeTime(result, MPR_GMTParameters);
  ...
  if (zone_offset == -1) {
    ...
    if (result->tm_year >= 1970) {
      ...
      if (secs != (time_t)-1) {
        ...
        MPR_ExplodeTime(usecs64, MPR_LocalTimeParameters, result);
        return PR_SUCCESS;
      }
    }
...
PR_IMPLEMENT(PRStatus)
MPR_ParseTimeString(const char* string, PRBool default_to_gmt, PRTime* result) {
  PRExplodedTime tm;
  PRStatus rv;

  rv = MPR_ParseTimeStringToExplodedTime(string, default_to_gmt, &tm);
  if (rv != PR_SUCCESS) {
    return rv;
  }

  *result = MPR_ImplodeTime(&tm);
...
MPR_FormatTime(char* buf, int buflen, const char* fmt,
...
/*
 * The following string arrays and macros are used by MPR_FormatTimeUSEnglish().
 */
...
/* Needed by MPR_FormatTimeUSEnglish() */
...
PR_IMPLEMENT(PRUint32)
MPR_FormatTimeUSEnglish(char* buf, PRUint32 bufSize, const char* format,
                       const PRExplodedTime* time) {
  char* bufPtr = buf;
  const char* fmtPtr;
  char tmpBuf[40];
  ...
  for (fmtPtr = format; *fmtPtr != '\0'; fmtPtr++) {
    if (*fmtPtr != '%') {
      ...
      switch (*(++fmtPtr)) {
        ...
        case 'c':
          /* Date and time. */
          MPR_FormatTimeUSEnglish(tmpBuf, tmpBufSize, "%a %b %d %H:%M:%S %Y",
                                 time);
        ...
        case 'd':
          /* day of month ( 01 - 31 ) */
          MPR_snprintf(tmpBuf, tmpBufSize, "%.2ld", time->tm_mday);
          ADDSTR(bufPtr, bufSize, tmpBuf);
          break;
        ...
        case 'H':
          /* hour ( 00 - 23 ) */
          MPR_snprintf(tmpBuf, tmpBufSize, "%.2ld", time->tm_hour);
          ADDSTR(bufPtr, bufSize, tmpBuf);
          break;

        case 'I':
          /* hour ( 01 - 12 ) */
          MPR_snprintf(tmpBuf, tmpBufSize, "%.2ld",
        ...
        case 'j':
          ...
          MPR_snprintf(tmpBuf, tmpBufSize, "%.3d", time->tm_yday + 1);
          ADDSTR(bufPtr, bufSize, tmpBuf);
          break;

        case 'm':
          /* month number ( 01 - 12 ) */
          MPR_snprintf(tmpBuf, tmpBufSize, "%.2ld", time->tm_month + 1);
          ADDSTR(bufPtr, bufSize, tmpBuf);
          break;

        case 'M':
          /* minute ( 00 - 59 ) */
          MPR_snprintf(tmpBuf, tmpBufSize, "%.2ld", time->tm_min);
          ADDSTR(bufPtr, bufSize, tmpBuf);
          break;
        ...
        case 'S':
          ...
          MPR_snprintf(tmpBuf, tmpBufSize, "%.2ld", time->tm_sec);
          ADDSTR(bufPtr, bufSize, tmpBuf);
          break;
        ...
        case 'U':
           ...
           * week 1 */
          MPR_snprintf(tmpBuf, tmpBufSize, "%.2d", pr_WeekOfYear(time, 0));
          ADDSTR(bufPtr, bufSize, tmpBuf);
          break;

        case 'w':
          /* weekday number ( 0 - 6 ), Sunday = 0 */
          MPR_snprintf(tmpBuf, tmpBufSize, "%d", time->tm_wday);
          ADDSTR(bufPtr, bufSize, tmpBuf);
          break;
        ...
        case 'W':
           ...
           * week 1 */
          MPR_snprintf(tmpBuf, tmpBufSize, "%.2d", pr_WeekOfYear(time, 1));
          ADDSTR(bufPtr, bufSize, tmpBuf);
          break;

        case 'x':
          /* Date representation */
          MPR_FormatTimeUSEnglish(tmpBuf, tmpBufSize, "%m/%d/%y", time);
          ADDSTR(bufPtr, bufSize, tmpBuf);
          break;

        case 'X':
          /* Time representation. */
          MPR_FormatTimeUSEnglish(tmpBuf, tmpBufSize, "%H:%M:%S", time);
          ADDSTR(bufPtr, bufSize, tmpBuf);
          break;

        case 'y':
          /* year within century ( 00 - 99 ) */
          MPR_snprintf(tmpBuf, tmpBufSize, "%.2d", time->tm_year % 100);
          ADDSTR(bufPtr, bufSize, tmpBuf);
          break;

        case 'Y':
          /* year as ccyy ( for example 1986 ) */
          MPR_snprintf(tmpBuf, tmpBufSize, "%.4d", time->tm_year);
          ADDSTR(bufPtr, bufSize, tmpBuf);
          break;
        ...
        case 'Z':
           ...
           * defer to MPR_FormatTime() for this option.
           */
          MPR_FormatTime(tmpBuf, tmpBufSize, "%Z", time);
          ADDSTR(bufPtr, bufSize, tmpBuf);
          break;
