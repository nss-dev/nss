...
/*
 * MPR_AtomicDecrement() is used in NSPR's thread-specific data
 * destructor.  Because thread-specific data destructors may be
 * invoked after a MPR_Cleanup() call, we need an implementation
...
void _PR_MD_INIT_ATOMIC() {
  ...
  PR_ASSERT(MPR_FloorLog2(MAX_ATOMIC_LOCKS) == MPR_CeilingLog2(MAX_ATOMIC_LOCKS));

  PR_ASSERT(MPR_FloorLog2(DEFAULT_ATOMIC_LOCKS) ==
            MPR_CeilingLog2(DEFAULT_ATOMIC_LOCKS));
  ...
  if (((eval = getenv("NSPR_ATOMIC_HASH_LOCKS")) != NULL) &&
    ...
    if (num_atomic_locks > MAX_ATOMIC_LOCKS) {
      ...
      num_atomic_locks = MPR_FloorLog2(num_atomic_locks);
      num_atomic_locks = 1L << num_atomic_locks;
    }
    atomic_locks =
        (pthread_mutex_t*)MPR_Malloc(sizeof(pthread_mutex_t) * num_atomic_locks);
    if (atomic_locks) {
...
#    endif
  ...
  PR_ASSERT(MPR_FloorLog2(num_atomic_locks) == MPR_CeilingLog2(num_atomic_locks));
}
...
PRInt32 _PR_MD_ATOMIC_INCREMENT(PRInt32* val) {
  ...
  MPR_Lock(atomic_lock);
  rv = ++(*val);
  MPR_Unlock(atomic_lock);
  return rv;
}
...
MPR_AtomicIncrement(PRInt32* val) { return _PR_MD_ATOMIC_INCREMENT(val); }

PR_IMPLEMENT(PRInt32)
MPR_AtomicDecrement(PRInt32* val) { return _PR_MD_ATOMIC_DECREMENT(val); }

PR_IMPLEMENT(PRInt32)
MPR_AtomicSet(PRInt32* val, PRInt32 newval) {
  return _PR_MD_ATOMIC_SET(val, newval);
}

PR_IMPLEMENT(PRInt32)
MPR_AtomicAdd(PRInt32* ptr, PRInt32 val) { return _PR_MD_ATOMIC_ADD(ptr, val); }
/*
...
MPR_CreateStack(const char* stack_name) {
  PRStack* stack;
  ...
  if (stack_name) {
    stack->prstk_name = (char*)MPR_Malloc(strlen(stack_name) + 1);
  ...
  stack->prstk_lock = MPR_NewLock();
  if (stack->prstk_lock == NULL) {
    MPR_Free(stack->prstk_name);
    PR_DELETE(stack);
    return NULL;
  }
...
MPR_DestroyStack(PRStack* stack) {
...
#ifndef _PR_HAVE_ATOMIC_CAS
  MPR_DestroyLock(stack->prstk_lock);
#endif /* !_PR_HAVE_ATOMIC_CAS */
  PR_DELETE(stack);
...
PR_IMPLEMENT(void)
MPR_StackPush(PRStack* stack, PRStackElem* stack_elem) {
  MPR_Lock(stack->prstk_lock);
  stack_elem->prstk_elem_next = stack->prstk_head.prstk_elem_next;
  stack->prstk_head.prstk_elem_next = stack_elem;
  MPR_Unlock(stack->prstk_lock);
  return;
}

PR_IMPLEMENT(PRStackElem*)
MPR_StackPop(PRStack* stack) {
  PRStackElem* element;
