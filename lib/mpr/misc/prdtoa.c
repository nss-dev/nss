...
#define ACQUIRE_DTOA_LOCK(n) MPR_Lock(dtoa_lock[n])
#define FREE_DTOA_LOCK(n) MPR_Unlock(dtoa_lock[n])

static PRLock* dtoa_lock[2];

void _PR_InitDtoa(void) {
  dtoa_lock[0] = MPR_NewLock();
  dtoa_lock[1] = MPR_NewLock();
}

void _PR_CleanupDtoa(void) {
  MPR_DestroyLock(dtoa_lock[0]);
  dtoa_lock[0] = NULL;
  MPR_DestroyLock(dtoa_lock[1]);
  dtoa_lock[1] = NULL;
...
MPR_dtoa(PRFloat64 d, PRIntn mode, PRIntn ndigits, PRIntn* decpt, PRIntn* sign,
...
*/
PR_IMPLEMENT(void)
MPR_cnvtf(char* buf, int bufsz, int prcsn, double dfval) {
  ...
  /* XXX Why use mode 1? */
  if (MPR_dtoa(dval(fval), 1, prcsn, &decpt, &sign, &endnum, num, bufsz) ==
      PR_FAILURE) {
    buf[0] = '\0';
    goto done;
  }
  ...
  if (decpt > (prcsn + 1) || decpt < -(prcsn - 1) || decpt < -5) {
    ...
    *bufp++ = 'e';
    MPR_snprintf(bufp, bufsz - (bufp - buf), "%+d", decpt - 1);
  } else if (decpt >= 0) {
    if (decpt == 0) {
      *bufp++ = '0';
    } else {
