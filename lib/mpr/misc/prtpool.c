...
#define JOIN_NOTIFY(_jobp)            \
  PR_BEGIN_MACRO                      \
  MPR_Lock(_jobp->tpool->join_lock);   \
  _jobp->join_wait = PR_FALSE;        \
  MPR_NotifyCondVar(_jobp->join_cv);   \
  MPR_Unlock(_jobp->tpool->join_lock); \
  PR_END_MACRO
...
#define CANCEL_IO_JOB(jobp)              \
  ...
  tp->ioq.cnt--;                         \
  MPR_NotifyCondVar(jobp->cancel_cv);     \
  PR_END_MACRO
...
static void wstart(void* arg) {
  ...
  while (!tp->shutdown) {
    ...
    MPR_Lock(tp->jobq.lock);
    tp->idle_threads++;
    MPR_Unlock(tp->jobq.lock);
    ...
    while (PR_CLIST_IS_EMPTY(&tp->jobq.list) && (!tp->shutdown)) {
      tp->idle_threads++;
      MPR_WaitCondVar(tp->jobq.cv, PR_INTERVAL_NO_TIMEOUT);
      tp->idle_threads--;
    }
    if (tp->shutdown) {
      MPR_Unlock(tp->jobq.lock);
...
static void add_to_jobq(PRThreadPool* tp, PRJob* jobp) {
  ...
  if ((tp->idle_threads < tp->jobq.cnt) &&
    ...
    if (wthrp) {
      wthrp->thread =
          MPR_CreateThread(PR_USER_THREAD, wstart, tp, PR_PRIORITY_NORMAL,
  ...
  while (!tp->shutdown) {
    ...
    if (pollfd_cnt > tp->ioq.npollfds) {
      ...
      if (NULL != tp->ioq.pollfds) {
        MPR_Free(tp->ioq.pollfds);
      }
      tp->ioq.pollfds = (PRPollDesc*)MPR_Malloc(
    ...
    if (!PR_CLIST_IS_EMPTY(&tp->ioq.list)) {
      ...
      if (PR_INTERVAL_NO_TIMEOUT == jobp->timeout) {
        ...
        poll_timeout = jobp->absolute - MPR_IntervalNow();
    ...
    rv = MPR_Poll(tp->ioq.pollfds, pollfds_used, poll_timeout);
    ...
    if (rv > 0) {
      ...
      if (pollfds[0].out_flags & PR_POLL_READ) {
        rval_status = MPR_WaitForPollableEvent(tp->ioq.notify_fd);
      ...
      for (index = 1; index < (pollfds_used); index++) {
        ...
        if ((revents & PR_POLL_NVAL) || /* busted in all cases */
          ...
          if (jobp->io_op == JOB_IO_CONNECT) {
            if (MPR_GetConnectStatus(&pollfds[index]) == PR_SUCCESS) {
              jobp->iod->error = 0;
            } else {
              jobp->iod->error = MPR_GetError();
            }
          } else {
            jobp->iod->error = 0;
          }
...
static void timer_wstart(void* arg) {
  ...
  /*
   * call MPR_WaitCondVar with minimum value of all timeouts
   */
  while (!tp->shutdown) {
    PRJob* jobp;

    MPR_Lock(tp->timerq.lock);
    if (PR_CLIST_IS_EMPTY(&tp->timerq.list)) {
      ...
      timeout = jobp->absolute - MPR_IntervalNow();
      if (timeout <= 0) {
        timeout = PR_INTERVAL_NO_WAIT; /* already timed out */
      }
    }
    if (PR_INTERVAL_NO_WAIT != timeout) {
      MPR_WaitCondVar(tp->timerq.cv, timeout);
    }
    if (tp->shutdown) {
      MPR_Unlock(tp->timerq.lock);
      break;
    }
    /*
...
static void delete_threadpool(PRThreadPool* tp) {
  if (NULL != tp) {
    if (NULL != tp->shutdown_cv) {
      MPR_DestroyCondVar(tp->shutdown_cv);
    }
    if (NULL != tp->jobq.cv) {
      MPR_DestroyCondVar(tp->jobq.cv);
    }
    if (NULL != tp->jobq.lock) {
      MPR_DestroyLock(tp->jobq.lock);
    }
    if (NULL != tp->join_lock) {
      MPR_DestroyLock(tp->join_lock);
    }
#ifdef OPT_WINNT
    ...
    if (NULL != tp->timerq.cv) {
      MPR_DestroyCondVar(tp->timerq.cv);
    }
    if (NULL != tp->timerq.lock) {
      MPR_DestroyLock(tp->timerq.lock);
    }

    if (NULL != tp->ioq.lock) {
      MPR_DestroyLock(tp->ioq.lock);
    }
    if (NULL != tp->ioq.pollfds) {
      MPR_Free(tp->ioq.pollfds);
    }
    if (NULL != tp->ioq.notify_fd) {
      MPR_DestroyPollableEvent(tp->ioq.notify_fd);
    }
    MPR_Free(tp);
  }
  return;
...
static PRThreadPool* alloc_threadpool(void) {
  ...
  tp->jobq.lock = MPR_NewLock();
  if (NULL == tp->jobq.lock) {
    goto failed;
  }
  tp->jobq.cv = MPR_NewCondVar(tp->jobq.lock);
  if (NULL == tp->jobq.cv) {
    goto failed;
  }
  tp->join_lock = MPR_NewLock();
  if (NULL == tp->join_lock) {
    goto failed;
  }
#ifdef OPT_WINNT
  ...
  tp->timerq.lock = MPR_NewLock();
  if (NULL == tp->timerq.lock) {
    goto failed;
  }
  tp->timerq.cv = MPR_NewCondVar(tp->timerq.lock);
  if (NULL == tp->timerq.cv) {
    goto failed;
  }

  tp->shutdown_cv = MPR_NewCondVar(tp->jobq.lock);
  if (NULL == tp->shutdown_cv) {
    goto failed;
  }
  tp->ioq.notify_fd = MPR_NewPollableEvent();
  if (NULL == tp->ioq.notify_fd) {
    goto failed;
  }
  return tp;
...
MPR_CreateThreadPool(PRInt32 initial_threads, PRInt32 max_threads,
  ...
  for (i = 0; i < initial_threads; ++i) {
    thr = MPR_CreateThread(PR_USER_THREAD, wstart, tp, PR_PRIORITY_NORMAL,
  ...
  thr = MPR_CreateThread(PR_USER_THREAD, io_wstart, tp, PR_PRIORITY_NORMAL,
                        PR_GLOBAL_THREAD, PR_JOINABLE_THREAD, stacksize);
  PR_ASSERT(thr);
  wthrp = PR_NEWZAP(wthread);
  PR_ASSERT(wthrp);
  ...
  thr = MPR_CreateThread(PR_USER_THREAD, timer_wstart, tp, PR_PRIORITY_NORMAL,
                        PR_GLOBAL_THREAD, PR_JOINABLE_THREAD, stacksize);
  PR_ASSERT(thr);
  wthrp = PR_NEWZAP(wthread);
  PR_ASSERT(wthrp);
...
static void delete_job(PRJob* jobp) {
  if (NULL != jobp) {
    if (NULL != jobp->join_cv) {
      MPR_DestroyCondVar(jobp->join_cv);
      jobp->join_cv = NULL;
    }
    if (NULL != jobp->cancel_cv) {
      MPR_DestroyCondVar(jobp->cancel_cv);
      jobp->cancel_cv = NULL;
    }
    PR_DELETE(jobp);
  }
...
static PRJob* alloc_job(PRBool joinable, PRThreadPool* tp) {
  ...
  if (joinable) {
    jobp->join_cv = MPR_NewCondVar(tp->join_lock);
...
PR_IMPLEMENT(PRJob*)
MPR_QueueJob_Read(PRThreadPool* tpool, PRJobIoDesc* iod, PRJobFn fn, void* arg,
                 PRBool joinable) {
...
PR_IMPLEMENT(PRJob*)
MPR_QueueJob_Write(PRThreadPool* tpool, PRJobIoDesc* iod, PRJobFn fn, void* arg,
                  PRBool joinable) {
...
PR_IMPLEMENT(PRJob*)
MPR_QueueJob_Accept(PRThreadPool* tpool, PRJobIoDesc* iod, PRJobFn fn, void* arg,
                   PRBool joinable) {
...
MPR_QueueJob_Connect(PRThreadPool* tpool, PRJobIoDesc* iod,
  ...
  rv = MPR_Connect(iod->socket, addr, PR_INTERVAL_NO_WAIT);
  if ((rv == PR_FAILURE) && ((err = MPR_GetError()) == PR_IN_PROGRESS_ERROR)) {
    /* connection pending */
  ...
  return (MPR_QueueJob(tpool, fn, arg, joinable));
}

/* queue a job, when a timer expires */
PR_IMPLEMENT(PRJob*)
MPR_QueueJob_Timer(PRThreadPool* tpool, PRIntervalTime timeout, PRJobFn fn,
...
static void notify_timerq(PRThreadPool* tp) {
   ...
   */
  MPR_NotifyCondVar(tp->timerq.cv);
}
...
static void notify_ioq(PRThreadPool* tp) {
   ...
   */
  rval_status = MPR_SetPollableEvent(tp->ioq.notify_fd);
  PR_ASSERT(PR_SUCCESS == rval_status);
}
...
MPR_CancelJob(PRJob* jobp) {
  ...
  if (jobp->on_timerq) {
    ...
    MPR_Lock(tp->ioq.lock);
    if (jobp->on_ioq) {
      jobp->cancel_cv = MPR_NewCondVar(tp->ioq.lock);
      if (NULL == jobp->cancel_cv) {
        MPR_Unlock(tp->ioq.lock);
      ...
      MPR_Unlock(tp->ioq.lock); /* release, reacquire ioq lock */
      notify_ioq(tp);
      MPR_Lock(tp->ioq.lock);
      while (jobp->cancel_io) {
        MPR_WaitCondVar(jobp->cancel_cv, PR_INTERVAL_NO_TIMEOUT);
      }
      MPR_Unlock(tp->ioq.lock);
...
PR_IMPLEMENT(PRStatus)
MPR_JoinJob(PRJob* jobp) {
  if (!JOINABLE_JOB(jobp)) {
    MPR_SetError(PR_INVALID_ARGUMENT_ERROR, 0);
    return PR_FAILURE;
  }
  MPR_Lock(jobp->tpool->join_lock);
  while (jobp->join_wait) {
    MPR_WaitCondVar(jobp->join_cv, PR_INTERVAL_NO_TIMEOUT);
  }
  MPR_Unlock(jobp->tpool->join_lock);
  delete_job(jobp);
  return PR_SUCCESS;
}
...
MPR_ShutdownThreadPool(PRThreadPool* tpool) {
  ...
  MPR_Lock(tpool->jobq.lock);
  tpool->shutdown = PR_TRUE;
  MPR_NotifyAllCondVar(tpool->shutdown_cv);
  MPR_Unlock(tpool->jobq.lock);
...
MPR_JoinThreadPool(PRThreadPool* tpool) {
  ...
  while (!tpool->shutdown) {
    MPR_WaitCondVar(tpool->shutdown_cv, PR_INTERVAL_NO_TIMEOUT);
  }
...
#else
  MPR_NotifyAllCondVar(tpool->jobq.cv);
#endif
  ...
  while (!PR_CLIST_IS_EMPTY(&tpool->jobq.wthreads)) {
    ...
    rval_status = MPR_JoinThread(wthrp->thread);
    PR_ASSERT(PR_SUCCESS == rval_status);
    PR_DELETE(wthrp);
    MPR_Lock(tpool->jobq.lock);
  }
