...
MPR_ErrorToString(PRErrorCode code, PRLanguageCode language) {
...
PR_IMPLEMENT(const char*)
MPR_ErrorToName(PRErrorCode code) {
  struct PRErrorTableList* et;
...
MPR_ErrorLanguages(void) { return callback_languages; }

PR_IMPLEMENT(PRErrorCode)
MPR_ErrorInstallTable(const struct PRErrorTable* table) {
  struct PRErrorTableList* new_et;

  new_et = (struct PRErrorTableList*)MPR_Malloc(sizeof(struct PRErrorTableList));
  if (!new_et) {
    return errno; /* oops */
  }
  new_et->table = table;
...
PR_IMPLEMENT(void)
MPR_ErrorInstallCallback(const char* const* languages,
