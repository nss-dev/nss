...
PR_IMPLEMENT(PRErrorCode) MPR_GetError(void) {
  PRThread* thread = MPR_GetCurrentThread();
  return thread->errorCode;
}

PR_IMPLEMENT(PRInt32) MPR_GetOSError(void) {
  PRThread* thread = MPR_GetCurrentThread();
  return thread->osErrorCode;
}

PR_IMPLEMENT(void) MPR_SetError(PRErrorCode code, PRInt32 osErr) {
  PRThread* thread = MPR_GetCurrentThread();
...
PR_IMPLEMENT(void) MPR_SetErrorText(PRIntn textLength, const char* text) {
  PRThread* thread = MPR_GetCurrentThread();
...
PR_IMPLEMENT(PRInt32) MPR_GetErrorTextLength(void) {
  PRThread* thread = MPR_GetCurrentThread();
  return thread->errorStringLength;
} /* MPR_GetErrorTextLength */

PR_IMPLEMENT(PRInt32) MPR_GetErrorText(char* text) {
  PRThread* thread = MPR_GetCurrentThread();
