...
PRLock* _pr_sleeplock; /* used in MPR_Sleep(), classic and pthreads */

static void _PR_InitCallOnce(void);

PRBool _pr_initialized = PR_FALSE;

PR_IMPLEMENT(PRBool) MPR_VersionCheck(const char* importedVersion) {
  /*
...
PR_IMPLEMENT(const char*) MPR_GetVersion(void) { return PR_VERSION; }

PR_IMPLEMENT(PRBool) MPR_Initialized(void) { return _pr_initialized; }
...
static void _PR_InitStuff(void) {
  ...
  (void)MPR_GetPageSize();

  _pr_clock_lm = MPR_NewLogModule("clock");
  _pr_cmon_lm = MPR_NewLogModule("cmon");
  _pr_io_lm = MPR_NewLogModule("io");
  _pr_mon_lm = MPR_NewLogModule("mon");
  _pr_linker_lm = MPR_NewLogModule("linker");
  _pr_cvar_lm = MPR_NewLogModule("cvar");
  _pr_sched_lm = MPR_NewLogModule("sched");
  _pr_thread_lm = MPR_NewLogModule("thread");
  _pr_gc_lm = MPR_NewLogModule("gc");
  _pr_shm_lm = MPR_NewLogModule("shm");
  _pr_shma_lm = MPR_NewLogModule("shma");
...
PR_IMPLEMENT(void) MPR_DisableClockInterrupts(void) {
#if !defined(_PR_PTHREADS) && !defined(_PR_BTHREADS)
  if (!_pr_initialized) {
    _PR_InitStuff();
  } else {
...
PR_IMPLEMENT(void) MPR_EnableClockInterrupts(void) {
#if !defined(_PR_PTHREADS) && !defined(_PR_BTHREADS)
  if (!_pr_initialized) {
    _PR_InitStuff();
  }
...
PR_IMPLEMENT(void) MPR_BlockClockInterrupts(void) {
#if !defined(_PR_PTHREADS) && !defined(_PR_BTHREADS)
  _PR_MD_BLOCK_CLOCK_INTERRUPTS();
#endif
}

PR_IMPLEMENT(void) MPR_UnblockClockInterrupts(void) {
#if !defined(_PR_PTHREADS) && !defined(_PR_BTHREADS)
...
PR_IMPLEMENT(void)
MPR_Init(PRThreadType type, PRThreadPriority priority, PRUintn maxPTDs) {
  _PR_ImplicitInitialization();
}

PR_IMPLEMENT(PRIntn)
MPR_Initialize(PRPrimordialFn prmain, PRIntn argc, char** argv,
              PRUintn maxPTDs) {
  PRIntn rv;
  _PR_ImplicitInitialization();
  rv = prmain(argc, argv);
 ...
 *   cleanup is done.  This function is used by MPR_Cleanup().
 *
 * See also: MPR_Cleanup().
 *
 *-----------------------------------------------------------------------
 */
#if defined(_PR_PTHREADS) || defined(_PR_BTHREADS)
...
static void _PR_CleanupBeforeExit(void) {
  ...
  do not make any calls that eventually may end up in MPR_Lock.  Because the
  ...
  */
   ...
   */
  {
    MPR_DestroyLock(_pr_terminationCVLock);
  }
 ...
 * MPR_Cleanup --
 *
 *   Perform a graceful shutdown of the NSPR runtime.  MPR_Cleanup() may
 ...
 *
 *   MPR_Cleanup() first blocks the primordial thread until all the
 *   other user (non-system) threads, if any, have terminated.
 *   Then it performs cleanup in preparation for exiting the process.
 *   MPR_Cleanup() does not exit the primordial thread (which would
 *   in turn exit the process).
 *
 *   MPR_Cleanup() only responds when it is called by the primordial
...
PR_IMPLEMENT(PRStatus) MPR_Cleanup() {
  ...
  if ((NULL != me) && (me->flags & _PR_PRIMORDIAL)) {
     ...
     * to terminate.
     */
    MPR_Lock(_pr_activeLock);
    while (_pr_userActive > _pr_primordialExitCount) {
      MPR_WaitCondVar(_pr_primordialExitCVar, PR_INTERVAL_NO_TIMEOUT);
    }
    ...
    PR_LOG(_pr_thread_lm, PR_LOG_MIN,
           ("MPR_Cleanup: clean up before destroying thread"));
    _PR_LogCleanup();
...
/* see ptthread.c */
#else
PR_IMPLEMENT(void) MPR_ProcessExit(PRIntn status) { _PR_MD_EXIT(status); }

#endif /* defined(_PR_PTHREADS) */

PR_IMPLEMENT(PRProcessAttr*)
MPR_NewProcessAttr(void) {
  PRProcessAttr* attr;
...
PR_IMPLEMENT(void)
MPR_ResetProcessAttr(PRProcessAttr* attr) {
...
PR_IMPLEMENT(void)
MPR_DestroyProcessAttr(PRProcessAttr* attr) {
  PR_FREEIF(attr->currentDirectory);
  PR_FREEIF(attr->fdInheritBuffer);
  PR_DELETE(attr);
}
...
PR_IMPLEMENT(void)
MPR_ProcessAttrSetStdioRedirect(PRProcessAttr* attr, PRSpecialFD stdioFd,
 ...
 */
PR_IMPLEMENT(void)
MPR_SetStdioRedirect(PRProcessAttr* attr, PRSpecialFD stdioFd,
                    PRFileDesc* redirectFd) {
#if defined(DEBUG)
  static PRBool warn = PR_TRUE;
  if (warn) {
    warn = _PR_Obsolete("MPR_SetStdioRedirect()",
                        "MPR_ProcessAttrSetStdioRedirect()");
  }
#endif
  MPR_ProcessAttrSetStdioRedirect(attr, stdioFd, redirectFd);
}

PR_IMPLEMENT(PRStatus)
MPR_ProcessAttrSetCurrentDirectory(PRProcessAttr* attr, const char* dir) {
  PR_FREEIF(attr->currentDirectory);
...
PR_IMPLEMENT(PRStatus)
MPR_ProcessAttrSetInheritableFD(PRProcessAttr* attr, PRFileDesc* fd,
                               const char* name) {
...
#define FD_TYPE_STRLEN 1
  ...
  if (fd->secret->inheritable != _PR_TRI_TRUE) {
    MPR_SetError(PR_NO_ACCESS_RIGHTS_ERROR, 0);
    return PR_FAILURE;
  }
  ...
  if (0 == attr->fdInheritBufferUsed) {
    nwritten =
        MPR_snprintf(cur, freeSize, "NSPR_INHERIT_FDS=%s:%d:0x%" PR_PRIxOSFD,
                    name, (PRIntn)fd->methods->file_type, fd->secret->md.osfd);
  } else {
    nwritten = MPR_snprintf(cur, freeSize, ":%s:%d:0x%" PR_PRIxOSFD, name,
...
PR_IMPLEMENT(PRFileDesc*) MPR_GetInheritedFD(const char* name) {
  PRFileDesc* fd;
  const char* envVar;
  const char* ptr;
  int len = strlen(name);
  ...
  envVar = MPR_GetEnv("NSPR_INHERIT_FDS");
  if (NULL == envVar || '\0' == envVar[0]) {
    MPR_SetError(PR_UNKNOWN_ERROR, 0);
    return NULL;
  }
  ...
  while (1) {
    if ((ptr[len] == ':') && (strncmp(ptr, name, len) == 0)) {
      ptr += len + 1;
      if (MPR_sscanf(ptr, "%d:0x%" PR_SCNxOSFD, &fileType, &osfd) != 2) {
        MPR_SetError(PR_UNKNOWN_ERROR, 0);
        return NULL;
      }
      switch ((PRDescType)fileType) {
        case PR_DESC_FILE:
          fd = MPR_ImportFile(osfd);
          break;
        case PR_DESC_PIPE:
          fd = MPR_ImportPipe(osfd);
          break;
        case PR_DESC_SOCKET_TCP:
          fd = MPR_ImportTCPSocket(osfd);
          break;
        case PR_DESC_SOCKET_UDP:
          fd = MPR_ImportUDPSocket(osfd);
          break;
        default:
          PR_ASSERT(0);
          MPR_SetError(PR_UNKNOWN_ERROR, 0);
      ...
      if (fd) {
         ...
         * The child process needs to call MPR_SetFDInheritable
...
MPR_CreateProcess(const char* path, char* const* argv, char* const* envp,
                 const PRProcessAttr* attr) {
  return _PR_MD_CREATE_PROCESS(path, argv, envp, attr);
} /* MPR_CreateProcess */

PR_IMPLEMENT(PRStatus)
MPR_CreateProcessDetached(const char* path, char* const* argv, char* const* envp,
                         const PRProcessAttr* attr) {
  PRProcess* process;
  PRStatus rv;

  process = MPR_CreateProcess(path, argv, envp, attr);
  if (NULL == process) {
    return PR_FAILURE;
  }
  rv = MPR_DetachProcess(process);
...
PR_IMPLEMENT(PRStatus) MPR_DetachProcess(PRProcess* process) {
  return _PR_MD_DETACH_PROCESS(process);
}

PR_IMPLEMENT(PRStatus) MPR_WaitProcess(PRProcess* process, PRInt32* exitCode) {
  return _PR_MD_WAIT_PROCESS(process, exitCode);
} /* MPR_WaitProcess */

PR_IMPLEMENT(PRStatus) MPR_KillProcess(PRProcess* process) {
...
static void _PR_InitCallOnce(void) {
  mod_init.ml = MPR_NewLock();
  PR_ASSERT(NULL != mod_init.ml);
  mod_init.cv = MPR_NewCondVar(mod_init.ml);
  PR_ASSERT(NULL != mod_init.cv);
}

void _PR_CleanupCallOnce() {
  MPR_DestroyLock(mod_init.ml);
  mod_init.ml = NULL;
  MPR_DestroyCondVar(mod_init.cv);
  mod_init.cv = NULL;
}

PR_IMPLEMENT(PRStatus) MPR_CallOnce(PRCallOnceType* once, PRCallOnceFN func) {
  if (!_pr_initialized) {
    _PR_ImplicitInitialization();
  }

  MPR_Lock(mod_init.ml);
  PRIntn initialized = once->initialized;
  PRStatus status = once->status;
  MPR_Unlock(mod_init.ml);
  if (!initialized) {
    if (PR_ATOMIC_SET(&once->inProgress, 1) == 0) {
      ...
      once->status = status;
      once->initialized = 1;
      MPR_NotifyAllCondVar(mod_init.cv);
      MPR_Unlock(mod_init.ml);
    } else {
      MPR_Lock(mod_init.ml);
      while (!once->initialized) {
        MPR_WaitCondVar(mod_init.cv, PR_INTERVAL_NO_TIMEOUT);
      }
      status = once->status;
      MPR_Unlock(mod_init.ml);
      if (PR_SUCCESS != status) {
        MPR_SetError(PR_CALL_ONCE_ERROR, 0);
      }
    }
    return status;
  }
...
PR_IMPLEMENT(PRStatus)
MPR_CallOnceWithArg(PRCallOnceType* once, PRCallOnceWithArgFN func, void* arg) {
  if (!_pr_initialized) {
    _PR_ImplicitInitialization();
  }
...
#if defined(DEBUG)
  MPR_fprintf(PR_STDERR, "'%s' is obsolete. Use '%s' instead.\n", obsolete,
