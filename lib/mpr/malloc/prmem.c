...
** use PRLocks because MPR_NewLock calls MPR_Calloc, resulting
...
PR_IMPLEMENT(void*) MPR_Malloc(PRUint32 size) {
  if (!_pr_initialized) {
...
PR_IMPLEMENT(void*) MPR_Calloc(PRUint32 nelem, PRUint32 elsize) {
  if (!_pr_initialized) {
    _PR_ImplicitInitialization();
  }
...
PR_IMPLEMENT(void*) MPR_Realloc(void* ptr, PRUint32 size) {
  if (!_pr_initialized) {
    _PR_ImplicitInitialization();
  }
...
PR_IMPLEMENT(void) MPR_Free(void* ptr) {
  if (use_zone_allocator) {
    pr_ZoneFree(ptr);
  } else {
    free(ptr);
...
/*
** The MPR_Malloc, MPR_Calloc, MPR_Realloc, and MPR_Free functions simply
...
*/
PR_IMPLEMENT(void*) MPR_Malloc(PRUint32 size) { return malloc(size); }
...
PR_IMPLEMENT(void) MPR_Free(void* ptr) { free(ptr); }
...
** are using the thread safe malloc code after MPR_Init but before
** MPR_AttachThread has been called (on a native thread that nspr has yet
