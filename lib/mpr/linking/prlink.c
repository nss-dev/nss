...
*/
{
  ...
  MPR_snprintf(errStrBuf, sizeof(errStrBuf), "error %d", oserr);
  error = errStrBuf;
#endif
  if (NULL != error) {
    MPR_SetErrorText(strlen(error), error);
  }
} /* DLLErrorInternal */
...
void _PR_InitLinker(void) {
  ...
  if (!pr_linker_lock) {
    pr_linker_lock = MPR_NewNamedMonitor("linker-lock");
  }
  MPR_EnterMonitor(pr_linker_lock);
...
#    if defined(USE_DLFCN) && !defined(NO_DLOPEN_NULL)
  ...
  if (!h) {
    ...
    error = (char*)PR_MALLOC(MPR_GetErrorTextLength());
    (void)MPR_GetErrorText(error);
...
#  endif   /* HAVE_DLL */
  ...
  MPR_ExitMonitor(pr_linker_lock);
}

/*
 * _PR_ShutdownLinker does not unload the dlls loaded by the application
 * via calls to MPR_LoadLibrary.  Any dlls that still remain on the
...
void _PR_ShutdownLinker(void) {
  ...
  MPR_DestroyMonitor(pr_linker_lock);
  pr_linker_lock = NULL;
...
PR_IMPLEMENT(PRStatus) MPR_SetLibraryPath(const char* path) {
...
*/
PR_IMPLEMENT(char*)
MPR_GetLibraryName(const char* path, const char* lib) {
  char* fullname;

#ifdef XP_PC
  if (strstr(lib, PR_DLL_SUFFIX) == NULL) {
    if (path) {
      fullname = MPR_smprintf("%s\\%s%s", path, lib, PR_DLL_SUFFIX);
    } else {
      fullname = MPR_smprintf("%s%s", lib, PR_DLL_SUFFIX);
    }
  } else {
    if (path) {
      fullname = MPR_smprintf("%s\\%s", path, lib);
    } else {
      fullname = MPR_smprintf("%s", lib);
    }
  }
#endif /* XP_PC */
#if defined(XP_UNIX)
  if (strstr(lib, PR_DLL_SUFFIX) == NULL) {
    if (path) {
      fullname = MPR_smprintf("%s/lib%s%s", path, lib, PR_DLL_SUFFIX);
    } else {
      fullname = MPR_smprintf("lib%s%s", lib, PR_DLL_SUFFIX);
    }
  } else {
    if (path) {
      fullname = MPR_smprintf("%s/%s", path, lib);
...
/*
** Free the memory allocated, for the caller, by MPR_GetLibraryName
*/
PR_IMPLEMENT(void)
MPR_FreeLibraryName(char* mem) { MPR_smprintf_free(mem); }
...
MPR_LoadLibraryWithFlags(PRLibSpec libSpec, PRIntn flags) {
  if (flags == 0) {
...
PR_IMPLEMENT(PRLibrary*)
MPR_LoadLibrary(const char* name) {
  PRLibSpec libSpec;

  libSpec.type = PR_LibSpec_Pathname;
  libSpec.value.pathname = name;
  return MPR_LoadLibraryWithFlags(libSpec, 0);
}
...
static PRLibrary* pr_LoadLibraryByPathname(const char* name, PRIntn flags) {
  ...
  if (flags & PR_LD_PATHW) {
    ...
    if (wlen > MAX_PATH) {
      wname = wname_malloc = MPR_Malloc(wlen * sizeof(PRUnichar));
    }
    if (wname == NULL ||
  ...
  if (len > MAX_PATH) {
    utf8name = utf8name_malloc = MPR_Malloc(len);
  }
  if (utf8name == NULL ||
...
#      if defined(DARWIN)
    ...
    if (strchr(name, PR_DIRECTORY_SEPARATOR) == NULL) {
      ...
      if (((name_len > systemPrefixLen1) &&
        ...
        okToLoad = PR_TRUE;
      } else if (MPR_Access(name, PR_ACCESS_EXISTS) == PR_SUCCESS) {
...
unlock:
  if (result == NULL) {
    MPR_SetError(PR_LOAD_LIBRARY_ERROR, oserr);
    DLLErrorInternal(oserr); /* sets error text */
  }
#ifdef WIN32
  if (utf8name_malloc) {
    MPR_Free(utf8name_malloc);
  }
  if (wname_malloc) {
    MPR_Free(wname_malloc);
  }
#endif
  MPR_ExitMonitor(pr_linker_lock);
  return result;
...
/*
** Unload a shared library which was loaded via MPR_LoadLibrary
*/
PR_IMPLEMENT(PRStatus)
MPR_UnloadLibrary(PRLibrary* lib) {
  int result = 0;
...
freeLib:
  ...
  PR_DELETE(lib);
  if (result != 0) {
    MPR_SetError(PR_UNLOAD_LIBRARY_ERROR, _MD_ERRNO());
    DLLErrorInternal(_MD_ERRNO());
    status = PR_FAILURE;
  }
...
static void* pr_FindSymbolInLib(PRLibrary* lm, const char* name) {
  ...
  if (lm->staticTable != NULL) {
    ...
    */
    MPR_SetError(PR_FIND_SYMBOL_ERROR, 0);
    return (void*)NULL;
  }
...
#endif   /* XP_UNIX */
  if (f == NULL) {
    MPR_SetError(PR_FIND_SYMBOL_ERROR, _MD_ERRNO());
    DLLErrorInternal(_MD_ERRNO());
  }
  return f;
}
...
*/
PR_IMPLEMENT(void*)
MPR_FindSymbol(PRLibrary* lib, const char* raw_name) {
  void* f = NULL;
...
#if defined(NEED_LEADING_UNDERSCORE)
  /* Need a leading _ */
  name = MPR_smprintf("_%s", raw_name);
#elif defined(AIX)
  /*
...
MPR_FindFunctionSymbol(PRLibrary* lib, const char* raw_name) {
  return ((PRFuncPtr)MPR_FindSymbol(lib, raw_name));
}

PR_IMPLEMENT(void*)
MPR_FindSymbolAndLibrary(const char* raw_name, PRLibrary** lib) {
  void* f = NULL;
#if defined(NEED_LEADING_UNDERSCORE)
  char* name;
#else
...
PR_IMPLEMENT(PRFuncPtr)
MPR_FindFunctionSymbolAndLibrary(const char* raw_name, PRLibrary** lib) {
  return ((PRFuncPtr)MPR_FindSymbolAndLibrary(raw_name, lib));
}
...
*/
PR_IMPLEMENT(PRLibrary*)
MPR_LoadStaticLibrary(const char* name, const PRStaticLinkTable* slt) {
  PRLibrary* lm = NULL;
  PRLibrary* result = NULL;
...
PR_IMPLEMENT(char*)
MPR_GetLibraryFilePathname(const char* name, PRFuncPtr addr) {
#if defined(USE_DLFCN) && defined(HAVE_DLADDR)
  Dl_info dli;
  char* result;

  if (dladdr((void*)addr, &dli) == 0) {
    MPR_SetError(PR_LIBRARY_NOT_LOADED_ERROR, _MD_ERRNO());
    DLLErrorInternal(_MD_ERRNO());
    return NULL;
  }
  result = MPR_Malloc(strlen(dli.dli_fname) + 1);
  if (result != NULL) {
...
#  define LD_INFO_INCREMENT 64
  ...
  for (infop = info;;
    ...
    if (start <= (unsigned long)addr && end > (unsigned long)addr) {
      result = MPR_Malloc(strlen(infop->ldinfo_filename) + 1);
      if (result != NULL) {
    ...
    if (!infop->ldinfo_next) {
      MPR_SetError(PR_LIBRARY_NOT_LOADED_ERROR, 0);
      result = NULL;
      break;
    }
  }
  ...
  result = MPR_Malloc(len * sizeof(PRUnichar));
  if (result != NULL) {
