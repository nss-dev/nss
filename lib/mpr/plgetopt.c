...
MPL_CreateOptState(PRIntn argc, char** argv, const char* options) {
  return MPL_CreateLongOptState(argc, argv, options, NULL);
} /* MPL_CreateOptState */

PR_IMPLEMENT(PLOptState*)
MPL_CreateLongOptState(PRIntn argc, char** argv, const char* options,
  ...
  internal->optionsLen = MPL_strlen(options);

  return opt;
} /* MPL_CreateLongOptState */

/*
** Destroy object created by CreateOptState()
*/
PR_IMPLEMENT(void) MPL_DestroyOptState(PLOptState* opt) {
  PR_DELETE(opt->internal);
  PR_DELETE(opt);
} /* MPL_DestroyOptState */

PR_IMPLEMENT(PLOptStatus) MPL_GetNextOpt(PLOptState* opt) {
