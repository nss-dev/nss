...
MPR_OpenSharedMemory(const char* name, PRSize size, PRIntn flags, PRIntn mode) {
  if (!_pr_initialized) {
    _PR_ImplicitInitialization();
  }
  return (_PR_MD_OPEN_SHARED_MEMORY(name, size, flags, mode));
} /* end MPR_OpenSharedMemory() */
...
*/
PR_IMPLEMENT(void*)
MPR_AttachSharedMemory(PRSharedMemory* shm, PRIntn flags) {
  return (_PR_MD_ATTACH_SHARED_MEMORY(shm, flags));
} /* end MPR_AttachSharedMemory() */
...
MPR_DetachSharedMemory(PRSharedMemory* shm, void* addr) {
  return (_PR_MD_DETACH_SHARED_MEMORY(shm, addr));
} /* end MPR_DetachSharedMemory() */
...
*/
PR_IMPLEMENT(PRStatus)
MPR_CloseSharedMemory(PRSharedMemory* shm) {
  return (_PR_MD_CLOSE_SHARED_MEMORY(shm));
} /* end MPR_CloseSharedMemory() */
...
*/
PR_EXTERN(PRStatus)
MPR_DeleteSharedMemory(const char* name) {
  if (!_pr_initialized) {
    _PR_ImplicitInitialization();
  }
