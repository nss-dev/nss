...
PR_IMPLEMENT(PRStatus) MPR_Yield() {
...
** a unique 'timeout'.
*/
PR_IMPLEMENT(PRStatus) MPR_Sleep(PRIntervalTime timeout) {
  ...
  if (PR_INTERVAL_NO_WAIT == timeout) {
    ...
    */
    ...
    if (_PR_IS_NATIVE_THREAD(me)) {
      ...
      if (_PR_RUNQREADYMASK(cpu) >> pri) {
        ...
        PR_LOG(_pr_sched_lm, PR_LOG_MIN, ("MPR_Yield: yielding"));
        _PR_MD_SWITCH_CONTEXT(me);
        PR_LOG(_pr_sched_lm, PR_LOG_MIN, ("MPR_Yield: done"));
    ...
    ** A thread in this state is interruptible (MPR_Interrupt()),
    ...
    */
    ...
    do {
      PRIntervalTime delta = MPR_IntervalNow() - timein;
      if (delta > timeout) {
        break;
      }
      rv = MPR_WaitCondVar(cv, timeout - delta);
    } while (rv == PR_SUCCESS);
...
PR_IMPLEMENT(PRUint32) MPR_GetThreadID(PRThread* thread) { return thread->id; }

PR_IMPLEMENT(PRThreadPriority) MPR_GetThreadPriority(const PRThread* thread) {
  return (PRThreadPriority)thread->priority;
}

PR_IMPLEMENT(PRThread*) MPR_GetCurrentThread() {
  if (!_pr_initialized) {
    _PR_ImplicitInitialization();
  }
...
** block in i/o functions when this happens. Also, any MPR_Wait's in
** progress will be undone. The interrupt remains in force until
** MPR_ClearInterrupt is called.
*/
PR_IMPLEMENT(PRStatus) MPR_Interrupt(PRThread* thread) {
  ...
  if ((NULL != victim) && (!(thread->flags & _PR_INTERRUPT_BLOCKED))) {
    ...
    if (!haveLock) {
      MPR_Lock(victim->lock);
    }
    MPR_NotifyAllCondVar(victim);
    if (!haveLock) {
      MPR_Unlock(victim->lock);
    }
  }
  return PR_SUCCESS;
...
*/
PR_IMPLEMENT(void) MPR_ClearInterrupt() {
  PRIntn is;
  PRThread* me = _PR_MD_CURRENT_THREAD();
...
PR_IMPLEMENT(void) MPR_BlockInterrupt() {
  PRIntn is;
  PRThread* me = _PR_MD_CURRENT_THREAD();
...
PR_IMPLEMENT(void) MPR_UnblockInterrupt() {
  PRIntn is;
  PRThread* me = _PR_MD_CURRENT_THREAD();
...
*/
PR_IMPLEMENT(void*) MPR_GetSP(PRThread* thread) {
  return (void*)_PR_MD_GET_SP(thread);
}

PR_IMPLEMENT(void*) MPR_GetExecutionEnvironment(PRThread* thread) {
  return thread->environment;
}

PR_IMPLEMENT(void) MPR_SetExecutionEnvironment(PRThread* thread, void* env) {
...
PR_IMPLEMENT(PRInt32) PR_SetCPUAffinityMask(PRUint32 mask) {
  ...
  while (qp != &_PR_CPUQ()) {
    ...
    cpu = _PR_CPU_PTR(qp);
    MPR_SetThreadAffinityMask(cpu->thread, mask);
...
PR_IMPLEMENT(void) MPR_SetThreadRecycleMode(PRUint32 count) {
...
PR_IMPLEMENT(void) MPR_SetThreadGCAble() {
  if (!_pr_initialized) {
    _PR_ImplicitInitialization();
  }
...
PR_IMPLEMENT(void) MPR_ClearThreadGCAble() {
  if (!_pr_initialized) {
    _PR_ImplicitInitialization();
  }
  MPR_Lock(_pr_activeLock);
...
PR_IMPLEMENT(PRThreadScope) MPR_GetThreadScope(const PRThread* thread) {
  if (!_pr_initialized) {
    _PR_ImplicitInitialization();
  }
...
PR_IMPLEMENT(PRThreadType) MPR_GetThreadType(const PRThread* thread) {
  return (thread->flags & _PR_SYSTEM) ? PR_SYSTEM_THREAD : PR_USER_THREAD;
}

PR_IMPLEMENT(PRThreadState) MPR_GetThreadState(const PRThread* thread) {
