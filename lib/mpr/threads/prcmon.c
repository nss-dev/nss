...
#    define _PR_NEW_LOCK_MCACHE() (_pr_mcacheLock = MPR_NewLock())
#    define _PR_DESTROY_LOCK_MCACHE()   \
      PR_BEGIN_MACRO                    \
      if (_pr_mcacheLock) {             \
        MPR_DestroyLock(_pr_mcacheLock); \
        _pr_mcacheLock = NULL;          \
      }                                 \
      PR_END_MACRO
#    define _PR_LOCK_MCACHE() MPR_Lock(_pr_mcacheLock)
#    define _PR_UNLOCK_MCACHE() MPR_Unlock(_pr_mcacheLock)
#  endif
#endif
...
void _PR_CleanupCMon(void) {
  ...
  while (free_entries) {
    MPR_DestroyMonitor(free_entries->mon);
...
*/
PR_IMPLEMENT(PRMonitor*) MPR_CEnterMonitor(void* address) {
  PRMonitor* mon;
  ...
  MPR_EnterMonitor(mon);
  return mon;
}

PR_IMPLEMENT(PRStatus) MPR_CExitMonitor(void* address) {
  MonitorCacheEntry **pp, *p;
...
PR_IMPLEMENT(PRStatus) MPR_CWait(void* address, PRIntervalTime ticks) {
  ...
  return MPR_Wait(mon, ticks);
}

PR_IMPLEMENT(PRStatus) MPR_CNotify(void* address) {
  MonitorCacheEntry** pp;
  PRMonitor* mon;
  ...
  return MPR_Notify(mon);
}

PR_IMPLEMENT(PRStatus) MPR_CNotifyAll(void* address) {
  MonitorCacheEntry** pp;
  PRMonitor* mon;
  ...
  return MPR_NotifyAll(mon);
}

PR_IMPLEMENT(void)
MPR_CSetOnMonitorRecycle(void (*callback)(void* address)) {
