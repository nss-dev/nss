...
MPR_NewRWLock(PRUint32 lock_rank, const char* lock_name) {
  PRRWLock* rwlock;
#if defined(HAVE_UNIX98_RWLOCK) || defined(HAVE_UI_RWLOCK)
  int err;
#endif
  ...
  if (lock_name != NULL) {
    rwlock->rw_name = (char*)MPR_Malloc(strlen(lock_name) + 1);
...
*/
PR_IMPLEMENT(void)
MPR_DestroyRWLock(PRRWLock* rwlock) {
#if defined(HAVE_UNIX98_RWLOCK) || defined(HAVE_UI_RWLOCK)
  int err;
...
*/
PR_IMPLEMENT(void)
MPR_RWLock_Rlock(PRRWLock* rwlock) {
#if defined(HAVE_UNIX98_RWLOCK) || defined(HAVE_UI_RWLOCK)
  int err;
#endif
...
*/
PR_IMPLEMENT(void)
MPR_RWLock_Wlock(PRRWLock* rwlock) {
#if defined(DEBUG)
...
*/
PR_IMPLEMENT(void)
MPR_RWLock_Unlock(PRRWLock* rwlock) {
#if defined(DEBUG)
  PRThread* me = MPR_GetCurrentThread();
#endif
#if defined(HAVE_UNIX98_RWLOCK) || defined(HAVE_UI_RWLOCK)
...
void _PR_InitRWLocks(void) {
   ...
   */
  if (MPR_NewThreadPrivateIndex(&pr_thread_rwlock_key, _PR_RELEASE_LOCK_STACK) ==
      PR_FAILURE) {
...
static void _PR_SET_THREAD_RWLOCK_RANK(PRRWLock* rwlock) {
   ...
   */
  if ((lock_stack = MPR_GetThreadPrivate(pr_thread_rwlock_key)) == NULL) {
    lock_stack =
        (thread_rwlock_stack*)PR_CALLOC(1 * sizeof(thread_rwlock_stack));
    if (lock_stack) {
      rv = MPR_SetThreadPrivate(pr_thread_rwlock_key, lock_stack);
      if (rv == PR_FAILURE) {
...
static PRUint32 _PR_GET_THREAD_RWLOCK_RANK(void) {
  ...
  lock_stack = MPR_GetThreadPrivate(pr_thread_rwlock_key);
