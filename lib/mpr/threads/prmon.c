...
static void _PR_PostNotifiesFromMonitor(PRCondVar* cv, PRIntn times) {
  ...
  PR_ASSERT(times != 0);
  if (times == -1) {
    rv = MPR_NotifyAllCondVar(cv);
    PR_ASSERT(rv == PR_SUCCESS);
  } else {
    while (times-- > 0) {
      rv = MPR_NotifyCondVar(cv);
      PR_ASSERT(rv == PR_SUCCESS);
    }
  }
}
...
*/
PR_IMPLEMENT(PRMonitor*) MPR_NewMonitor() {
  PRMonitor* mon;
  PRStatus rv;
...
PR_IMPLEMENT(void) MPR_EnterMonitor(PRMonitor* mon) {
  ...
  if (mon->entryCount != 0) {
    ...
    while (mon->entryCount != 0) {
      rv = MPR_WaitCondVar(&mon->entryCV, PR_INTERVAL_NO_TIMEOUT);
      PR_ASSERT(rv == PR_SUCCESS);
    }
  }
...
PR_IMPLEMENT(PRStatus) MPR_ExitMonitor(PRMonitor* mon) {
  ...
  if (mon->entryCount == 0) {
    ...
    rv = MPR_NotifyCondVar(&mon->entryCV);
    PR_ASSERT(rv == PR_SUCCESS);
  }
  rv = MPR_Unlock(&mon->lock);
  PR_ASSERT(rv == PR_SUCCESS);
...
** MPR_ExitMonitor as many times as it had called MPR_EnterMonitor).  When
...
*/
PR_IMPLEMENT(PRStatus) MPR_Wait(PRMonitor* mon, PRIntervalTime ticks) {
  PRStatus rv;
  ...
  rv = MPR_WaitCondVar(&mon->waitCV, ticks);
  PR_ASSERT(rv == PR_SUCCESS);
...
** MPR_Wait) then it is awakened and begins waiting on the monitor's lock.
*/
...
PRUint32 _PR_MonitorToString(PRMonitor* mon, char* buf, PRUint32 buflen) {
  ...
  if (mon->owner) {
    nb = MPR_snprintf(buf, buflen, "[%p] owner=%d[%p] count=%ld", mon,
                     mon->owner->id, mon->owner, mon->entryCount);
  } else {
    nb = MPR_snprintf(buf, buflen, "[%p]", mon);
  }
  return nb;
}
