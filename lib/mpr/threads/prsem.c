...
** no longer in use.
*/
PR_IMPLEMENT(void) MPR_DestroySem(PRSemaphore* sem) {
#ifdef HAVE_CVAR_BUILT_ON_SEM
  _PR_MD_DESTROY_SEM(&sem->md);
#else
  PR_ASSERT(sem->waiters == 0);

  MPR_DestroyLock(sem->cvar->lock);
  MPR_DestroyCondVar(sem->cvar);
#endif
  PR_DELETE(sem);
}
...
*/
PR_IMPLEMENT(PRStatus) MPR_WaitSem(PRSemaphore* sem) {
  PRStatus status = PR_SUCCESS;

#ifdef HAVE_CVAR_BUILT_ON_SEM
  return _PR_MD_WAIT_SEM(&sem->md);
#else
  MPR_Lock(sem->cvar->lock);
  while (sem->count == 0) {
    sem->waiters++;
    status = MPR_WaitCondVar(sem->cvar, PR_INTERVAL_NO_TIMEOUT);
    sem->waiters--;
...
*/
PR_IMPLEMENT(void) MPR_PostSem(PRSemaphore* sem) {
#ifdef HAVE_CVAR_BUILT_ON_SEM
  _PR_MD_POST_SEM(&sem->md);
#else
  MPR_Lock(sem->cvar->lock);
  if (sem->waiters) {
    MPR_NotifyCondVar(sem->cvar);
  }
  sem->count++;
  MPR_Unlock(sem->cvar->lock);
#endif
