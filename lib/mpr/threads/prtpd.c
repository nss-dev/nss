...
**
** The vector to hold the TPD are allocated when MPR_SetThreadPrivate() is
...
MPR_NewThreadPrivateIndex(PRUintn* newIndex, PRThreadPrivateDTOR dtor) {
  PRStatus rv;
  PRInt32 index;
  ...
  if (_PR_TPD_LIMIT <= index) {
    MPR_SetError(PR_TPD_RANGE_ERROR, 0);
...
PR_IMPLEMENT(PRStatus) MPR_SetThreadPrivate(PRUintn index, void* priv) {
...
PR_IMPLEMENT(void*) MPR_GetThreadPrivate(PRUintn index) {
  PRThread* self = MPR_GetCurrentThread();
