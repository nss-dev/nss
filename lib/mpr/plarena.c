...
PR_IMPLEMENT(void)
MPL_InitArenaPool(PLArenaPool* pool, const char* name, PRUint32 size,
                 PRUint32 align) {
  /*
   * Look-up table of PR_BITMASK(MPR_CeilingLog2(align)) values for
   * align = 1 to 32.
   */
  ...
  if (align < sizeof(pmasks) / sizeof(pmasks[0])) {
    ...
    pool->mask = PR_BITMASK(MPR_CeilingLog2(align));
  }
   ...
   * that MPL_ArenaAllocate adds to the net size.
   */
...
/*
** MPL_ArenaAllocate() -- allocate space from an arena pool
**
** Description: MPL_ArenaAllocate() allocates space from an arena
** pool.
**
...
PR_IMPLEMENT(void*) MPL_ArenaAllocate(PLArenaPool* pool, PRUint32 nb) {
  PLArena* a;
  ...
  return (NULL);
} /* --- end MPL_ArenaAllocate() --- */

PR_IMPLEMENT(void*)
MPL_ArenaGrow(PLArenaPool* pool, void* p, PRUint32 size, PRUint32 incr) {
  void* newp;
...
PR_IMPLEMENT(void) MPL_ClearArenaPool(PLArenaPool* pool, PRInt32 pattern) {
  PLArena* a;
...
PR_IMPLEMENT(void) MPL_ArenaRelease(PLArenaPool* pool, char* mark) {
  PLArena* a;
...
PR_IMPLEMENT(void) MPL_FreeArenaPool(PLArenaPool* pool) {
  FreeArenaList(pool, &pool->first);
  COUNT(pool, ndeallocs);
}

PR_IMPLEMENT(void) MPL_FinishArenaPool(PLArenaPool* pool) {
  FreeArenaList(pool, &pool->first);
#ifdef PL_ARENAMETER
  {
...
PR_IMPLEMENT(void) MPL_CompactArenaPool(PLArenaPool* ap) {}

PR_IMPLEMENT(void) MPL_ArenaFinish(void) {}

PR_IMPLEMENT(size_t)
MPL_SizeOfArenaPoolExcludingPool(const PLArenaPool* pool,
