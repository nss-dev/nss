...
extern PRSharedMemory* _MD_OpenSharedMemory(const char* name, PRSize size,
  ...
  if (NULL == shm) {
    ...
    PR_LOG(_pr_shm_lm, PR_LOG_DEBUG,
           ("MPR_OpenSharedMemory: New PRSharedMemory out of memory"));
    return (NULL);
  }
  ...
  if (NULL == shm->ipcname) {
    MPR_SetError(PR_OUT_OF_MEMORY_ERROR, 0);
    PR_LOG(_pr_shm_lm, PR_LOG_DEBUG,
           ("MPR_OpenSharedMemory: New shm->ipcname out of memory"));
    PR_DELETE(shm);
    return (NULL);
  }
  ...
  if (flags & PR_SHM_CREATE) {
    ...
    if ((errno == EEXIST) && (flags & PR_SHM_EXCL)) {
      MPR_SetError(PR_FILE_EXISTS_ERROR, errno);
      PR_LOG(_pr_shm_lm, PR_LOG_DEBUG,
...
extern PRFileMap* _md_OpenAnonFileMap(const char* dirName, PRSize size,
  ...
  PRThread* tid = MPR_GetCurrentThread(); /* for generating filename */
...
#define NSPR_AFM_FILENAME "%s/.NSPR-AFM-%d-%p.%d"
    genName = MPR_smprintf(NSPR_AFM_FILENAME, dirName, (int)pid, tid, incr);
    if (NULL == genName) {
      PR_LOG(_pr_shma_lm, PR_LOG_DEBUG,
             ("_md_OpenAnonFileMap(): MPR_snprintf(): failed, generating "
              "filename"));
      goto Finished;
    }
  ...
  if (NULL == fd) {
    PR_LOG(_pr_shma_lm, PR_LOG_DEBUG,
           ("_md_OpenAnonFileMap(): MPR_ImportFile(): failed"));
    goto Finished;
  }
  ...
  fm = MPR_CreateFileMap(fd, size64, prot);
  if (NULL == fm) {
    PR_LOG(_pr_shma_lm, PR_LOG_DEBUG, ("MPR_OpenAnonFileMap(): failed"));
    MPR_Close(fd);
    goto Finished;
  }
  ...
  PR_LOG(_pr_shma_lm, PR_LOG_DEBUG,
         ("_md_OpenAnonFileMap(): MPR_CreateFileMap(): fm: %p", fm));
...
extern PRStatus _md_ExportFileMapAsString(PRFileMap* fm, PRSize bufSize,
  ...
  written = MPR_snprintf(buf, bufSize, "%ld:%d", fm->fd->secret->md.osfd, prot);
...
extern PRFileMap* _md_ImportFileMapFromString(const char* fmstring) {
  ...
  MPR_sscanf(fmstring, "%ld:%d", &osfd, &prot);

  /* import the os file descriptor */
  fd = MPR_ImportFile(osfd);
  if (NULL == fd) {
    PR_LOG(_pr_shma_lm, PR_LOG_DEBUG,
           ("_md_ImportFileMapFromString(): MPR_ImportFile() failed"));
    goto Finished;
  }

  rc = MPR_GetOpenFileInfo64(fd, &info);
  if (PR_FAILURE == rc) {
    PR_LOG(_pr_shma_lm, PR_LOG_DEBUG,
           ("_md_ImportFileMapFromString(): MPR_GetOpenFileInfo64() failed"));
    goto Finished;
  }

  fm = MPR_CreateFileMap(fd, info.size, (PRFileMapProtect)prot);
  if (NULL == fm) {
    PR_LOG(_pr_shma_lm, PR_LOG_DEBUG,
           ("_md_ImportFileMapFromString(): MPR_CreateFileMap() failed"));
  }
