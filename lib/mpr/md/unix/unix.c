...
PRInt32 _MD_rename(const char* from, const char* to) {
  ...
  */
  ...
  if (NULL != _pr_unix_rename_lock) {
    MPR_Unlock(_pr_unix_rename_lock);
  }
  return rv;
}
...
PRInt32 _MD_mkdir(const char* name, PRIntn mode) {
  ...
  ** in MPR_Rename. Look there for more fun details.
  */
  if (NULL != _pr_unix_rename_lock) {
    MPR_Lock(_pr_unix_rename_lock);
  }
  ...
  switch (timeout) {
    ...
    default:
      ...
      do {
        ...
        tv.tv_sec = MPR_IntervalToSeconds(remaining);
        if (tv.tv_sec > _PR_INTERRUPT_CHECK_INTERVAL_SECS) {
          wait_for_remaining = PR_FALSE;
          tv.tv_sec = _PR_INTERRUPT_CHECK_INTERVAL_SECS;
          tv.tv_usec = 0;
        } else {
          tv.tv_usec = MPR_IntervalToMicroseconds(
        ...
        if (rv == 0 || (rv == -1 && syserror == EINTR)) {
           ...
           * MPR_IntervalNow() call.
           */
          if (rv == 0) {
            if (wait_for_remaining) {
              now += remaining;
            } else {
              now += MPR_SecondsToInterval(tv.tv_sec) +
                     MPR_MicrosecondsToInterval(tv.tv_usec);
            }
          } else {
            now = MPR_IntervalNow();
          }
...
PRStatus _MD_set_fd_inheritable(PRFileDesc* fd, PRBool inheritable) {
  ...
  if (-1 == rv) {
    MPR_SetError(PR_UNKNOWN_ERROR, _MD_ERRNO());
    return PR_FAILURE;
  }
  return PR_SUCCESS;
}
...
#  ifdef _PR_USE_POLL
  if (min_timeout == PR_INTERVAL_NO_TIMEOUT) {
    ...
    timeout = MPR_IntervalToMilliseconds(min_timeout);
  }
#  else
  if (min_timeout == PR_INTERVAL_NO_TIMEOUT) {
    tvp = NULL;
  } else {
    timeout.tv_sec = MPR_IntervalToSeconds(min_timeout);
    timeout.tv_usec = MPR_IntervalToMicroseconds(min_timeout) % PR_USEC_PER_SEC;
    tvp = &timeout;
  }
#  endif /* _PR_USE_POLL */
...
#  endif
  ...
  ** the semantics of MPR_Rename. (see the latter for more details)
  */
...
static int _MD_convert_stat_to_fileinfo(const struct stat* sb,
  ...
  if (0x7fffffffL < sb->st_size) {
    MPR_SetError(PR_FILE_TOO_BIG_ERROR, 0);
    return -1;
  }
...
static void _PR_InitIOV(void) {
  ...
  open64_func = MPR_FindSymbolAndLibrary("open64", &lib);
  if (NULL != open64_func) {
    PR_ASSERT(NULL != lib);
    _md_iovector._open64 = (_MD_Open64)open64_func;
    _md_iovector._mmap64 = (_MD_Mmap64)MPR_FindSymbol(lib, "mmap64");
    _md_iovector._fstat64 = (_MD_Fstat64)MPR_FindSymbol(lib, "fstat64");
    _md_iovector._stat64 = (_MD_Stat64)MPR_FindSymbol(lib, "stat64");
    _md_iovector._lseek64 = (_MD_Lseek64)MPR_FindSymbol(lib, "lseek64");
...
void _PR_UnixInit(void) {
  ...
  intr_timeout_ticks = MPR_SecondsToInterval(_PR_INTERRUPT_CHECK_INTERVAL_SECS);
...
#  endif
  ...
  _pr_unix_rename_lock = MPR_NewLock();
  PR_ASSERT(NULL != _pr_unix_rename_lock);
  _pr_Xfe_mon = MPR_NewMonitor();
...
void _PR_UnixCleanup(void) {
  if (_pr_unix_rename_lock) {
    MPR_DestroyLock(_pr_unix_rename_lock);
    _pr_unix_rename_lock = NULL;
  }
  if (_pr_Xfe_mon) {
    MPR_DestroyMonitor(_pr_Xfe_mon);
    _pr_Xfe_mon = NULL;
  }
}
...
void _PR_XLock(void) { MPR_EnterMonitor(_pr_Xfe_mon); }

void _PR_XUnlock(void) { MPR_ExitMonitor(_pr_Xfe_mon); }
...
PRStatus _MD_getsysinfo(PRSysInfo cmd, char* name, PRUint32 namelen) {
  ...
  if (PR_SI_SYSNAME == cmd) {
    (void)MPR_snprintf(name, namelen, info.sysname);
  } else if (PR_SI_RELEASE == cmd) {
    (void)MPR_snprintf(name, namelen, info.release);
  } else if (PR_SI_RELEASE_BUILD == cmd) {
    (void)MPR_snprintf(name, namelen, info.version);
  } else {
    return PR_FAILURE;
  }
  return PR_SUCCESS;
...
PRStatus _MD_CreateFileMap(PRFileMap* fmap, PRInt64 size) {
  ...
  LL_L2UI(sz, size);
  if (sz) {
    if (MPR_GetOpenFileInfo(fmap->fd, &info) == PR_FAILURE) {
      return PR_FAILURE;
    }
    if (sz > info.size) {
      /*
      ...
      if (MPR_Seek(fmap->fd, sz - 1, PR_SEEK_SET) == -1) {
        return PR_FAILURE;
      }
      if (MPR_Write(fmap->fd, "", 1) != 1) {
        return PR_FAILURE;
      }
    }
  }
