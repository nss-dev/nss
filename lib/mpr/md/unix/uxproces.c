 ...
 * need to catch SIGCHLD, and the pipe to unblock MPR_Poll() is
 * also not necessary.
 */
...
void _MD_unix_terminate_waitpid_daemon(void) {
  if (pr_wp.thread) {
    ...
    (void)write(pr_wp.pipefd[1], "", 1);
    MPR_JoinThread(pr_wp.thread);
  }
}
#endif
...
#  endif
  ...
  if ((pid_t)-1 == process->md.pid) {
    MPR_SetError(PR_INSUFFICIENT_RESOURCES_ERROR, errno);
    PR_DELETE(process);
    if (newEnvp) {
      PR_DELETE(newEnvp);
    }
...
PRProcess* _MD_CreateUnixProcess(const char* path, char* const* argv,
  ...
  if (MPR_CallOnce(&pr_wp.once, _MD_InitProcesses) == PR_FAILURE) {
    return NULL;
  }
  ...
  op->attr = attr;
  op->done = PR_FALSE;
  op->doneCV = MPR_NewCondVar(pr_wp.ml);
  if (NULL == op->doneCV) {
    PR_DELETE(op);
    return NULL;
  }
  ...
  while (op->done == PR_FALSE) {
    MPR_WaitCondVar(op->doneCV, PR_INTERVAL_NO_TIMEOUT);
  }
  MPR_Unlock(pr_wp.ml);
  MPR_DestroyCondVar(op->doneCV);
  proc = op->process;
  if (!proc) {
    MPR_SetError(op->prerror, op->oserror);
  }
  PR_DELETE(op);
  return proc;
}
...
static void ProcessReapedChildInternal(pid_t pid, int status) {
  ...
  if (NULL == pRec) {
    ...
    if (_PR_PID_DETACHED == pRec->state) {
      ...
      pRec->exitStatus = ExtractExitStatus(status);
      pRec->state = _PR_PID_REAPED;
      MPR_NotifyCondVar(pRec->reapedCV);
    }
  }
}
...
static void WaitPidDaemonThread(void* unused) {
  ...
  while (1) {
    ...
    while (0 == pr_wp.numProcs) {
      MPR_WaitCondVar(pr_wp.cv, PR_INTERVAL_NO_TIMEOUT);
    }
    MPR_Unlock(pr_wp.ml);
...
#  endif
  ...
  fd = MPR_ImportFile(pr_wp.pipefd[0]);
  PR_ASSERT(NULL != fd);
  pd.fd = fd;
  pd.in_flags = PR_POLL_READ;

  while (1) {
    rv = MPR_Poll(&pd, 1, PR_INTERVAL_NO_TIMEOUT);
    PR_ASSERT(1 == rv);
...
#  ifdef _PR_SHARE_CLONES
    while ((op = pr_wp.opHead) != NULL) {
      ...
      if (NULL == op->process) {
        op->prerror = MPR_GetError();
        op->oserror = MPR_GetOSError();
      }
      MPR_Lock(pr_wp.ml);
...
static PRStatus _MD_InitProcesses(void) {
  ...
  pr_wp.ml = MPR_NewLock();
  PR_ASSERT(NULL != pr_wp.ml);

#if defined(_PR_NATIVE_THREADS)
  pr_wp.numProcs = 0;
  pr_wp.cv = MPR_NewCondVar(pr_wp.ml);
...
#  endif
  ...
  pr_wp.thread = MPR_CreateThread(PR_SYSTEM_THREAD, WaitPidDaemonThread, NULL,
                                 PR_PRIORITY_NORMAL,
#ifdef _PR_SHARE_CLONES
                                 PR_GLOBAL_THREAD,
#else
...
PRStatus _MD_WaitUnixProcess(PRProcess* process, PRInt32* exitCode) {
  ...
  if (NULL == pRec) {
    ...
    pRec->reapedCV = MPR_NewCondVar(pr_wp.ml);
    ...
    while (!interrupted && _PR_PID_REAPED != pRec->state) {
      if (MPR_WaitCondVar(pRec->reapedCV, PR_INTERVAL_NO_TIMEOUT) ==
              PR_FAILURE &&
          MPR_GetError() == PR_PENDING_INTERRUPT_ERROR) {
        interrupted = PR_TRUE;
      }
    }
    if (_PR_PID_REAPED == pRec->state) {
    ...
    DeletePidTable(pRec);
    MPR_DestroyCondVar(pRec->reapedCV);
    PR_DELETE(pRec);
  } else {
    PR_ASSERT(_PR_PID_REAPED == pRec->state);
    PR_ASSERT(NULL == pRec->reapedCV);
