...
static PRInt32 NativeThreadPoll(PRPollDesc* pds, PRIntn npds,
  ...
  /*
   * This function is mostly duplicated from ptio.s's MPR_Poll().
   */
  PRInt32 ready = 0;
  /*
  ...
  for (index = 0; index < npds; ++index) {
    ...
    if ((NULL != pds[index].fd) && (0 != pds[index].in_flags)) {
      ...
      if ((0 != (in_flags_read & out_flags_read)) ||
        ...
        bottom = MPR_GetIdentitiesLayer(pds[index].fd, PR_NSPR_IO_LAYER);
  ...
  for (pd = pds, epd = pd + npds; pd < epd; pd++) {
    ...
    if ((NULL != pd->fd) && (0 != pd->in_flags)) {
      ...
      if ((0 != (in_flags_read & out_flags_read)) ||
        ...
        bottom = MPR_GetIdentitiesLayer(pd->fd, PR_NSPR_IO_LAYER);
        PR_ASSERT(NULL != bottom); /* what to do about that? */
        if ((NULL != bottom) && (_PR_FILEDESC_OPEN == bottom->secret->state)) {
          if (0 == ready) {
...
retry:
  if (timeout != PR_INTERVAL_NO_TIMEOUT) {
    PRInt32 ticksPerSecond = MPR_TicksPerSecond();
    tv.tv_sec = remaining / ticksPerSecond;
    tv.tv_usec = MPR_IntervalToMicroseconds(remaining % ticksPerSecond);
    tvp = &tv;
  }
