...
#  endif
  ...
  if (tv) {
    ...
    timeout = MPR_MicrosecondsToInterval(1000000 * tv->tv_sec + tv->tv_usec);
  } else {
  ...
  /*
   * Set up for MPR_Poll().  The PRPollDesc array is allocated
...
#    endif
  ...
  if (timeout == -1) {
    ...
    ticks = MPR_MillisecondsToInterval(timeout);
  }
...
#    endif /* _PR_USE_POLL */
  ...
  if (-1 == ready) {
    if (MPR_GetError() == PR_PENDING_INTERRUPT_ERROR) {
      errno = EINTR; /* XXX we aren't interrupted by a signal, but... */
    } else {
      errno = MPR_GetOSError();
    }
  }
  if (ready <= 0) {
    goto done;
