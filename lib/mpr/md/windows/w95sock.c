...
PROsfd _PR_MD_SOCKET(int af, int type, int flags) {
  ...
  */
  if (ioctlsocket(sock, FIONBIO, &one) != 0) {
    MPR_SetError(PR_UNKNOWN_ERROR, WSAGetLastError());
    closesocket(sock);
    return -1;
  }
...
PRInt32 _PR_MD_TCPSENDTO(PRFileDesc* fd, const void* buf, PRInt32 amount,
  ...
  if (MPR_CallOnce(&_pr_has_connectex_once, _pr_set_connectex) != PR_SUCCESS) {
    MPR_SetError(PR_NOT_IMPLEMENTED_ERROR, 0);
    return PR_FAILURE;
  }
      ...
      do {
        ...
        if (rv == 0) {
          if (wait_for_remaining) {
            ...
            elapsed = MPR_SecondsToInterval(tv.tv_sec) +
