...
#define CLOSE_TIMEOUT MPR_SecondsToInterval(5)
...
PRInt32 _PR_MD_PAUSE_CPU(PRIntervalTime ticks) {
  ...
  if (ticks == PR_INTERVAL_NO_TIMEOUT)
    ...
    timeout = MPR_IntervalToMilliseconds(ticks);
  }
...
static PRStatus _native_thread_md_wait(PRThread* thread, PRIntervalTime ticks) {
  ...
  PRUint32 msecs = (ticks == PR_INTERVAL_NO_TIMEOUT)
                       ? INFINITE
                       : MPR_IntervalToMilliseconds(ticks);
...
void _PR_Unblock_IO_Wait(PRThread* thr) {
   ...
   * in an IO_WAIT state except as a result of calling MPR_Close or
...
void _PR_MD_INIT_IO() {
  ...
  pt_tq.ml = MPR_NewLock();
  PR_ASSERT(NULL != pt_tq.ml);
  pt_tq.new_op = MPR_NewCondVar(pt_tq.ml);
  PR_ASSERT(NULL != pt_tq.new_op);
#if defined(DEBUG)
  memset(&pt_debug, 0, sizeof(struct pt_debug_s));
#endif

  pt_tq.thread = MPR_CreateThread(PR_SYSTEM_THREAD, ContinuationThread, NULL,
...
retry:
  if (me->io_suspended) {
    ...
    err = getsockopt(*newSock, SOL_SOCKET, SO_CONNECT_TIME, (char*)&seconds,
                     (PINT)&bytes);
    if (err == NO_ERROR) {
      PRIntervalTime elapsed = MPR_SecondsToInterval(seconds);
...
PRInt32 _PR_MD_SOCKETAVAILABLE(PRFileDesc* fd) {
  ...
  if (ioctlsocket(fd->secret->md.osfd, FIONREAD, &result) < 0) {
    MPR_SetError(PR_BAD_DESCRIPTOR_ERROR, WSAGetLastError());
    return -1;
  }
  return result;
}
...
PRStatus _PR_MD_OPEN_DIR(_MDDir* d, const char* name) {
  ...
  if (len + 5 > MAX_PATH) {
    MPR_SetError(PR_NAME_TOO_LONG_ERROR, 0);
    return PR_FAILURE;
  }
...
void _PR_FileTimeToPRTime(const FILETIME* filetime, PRTime* prtm) {
  ...
  *prtm = (*prtm - _pr_filetime_offset) / 10i64;
  ...
  /* Doublecheck our calculation. */
  {
    ...
    /*
     * MPR_ImplodeTime ignores wday and yday.
     */
...
PR_IMPLEMENT(PRStatus) PR_NT_CancelIo(PRFileDesc* fd) {
  ...
  bottom = MPR_GetIdentitiesLayer(fd, PR_NSPR_IO_LAYER);
  ...
  if (me->io_pending) {
    if (!CancelIo((HANDLE)bottom->secret->md.osfd)) {
      MPR_SetError(PR_INVALID_STATE_ERROR, GetLastError());
      return PR_FAILURE;
    }
  }
  _PR_THREAD_LOCK(me);
...
static PROsfd _nt_nonblock_accept(PRFileDesc* fd, struct sockaddr* addr,
  ...
  retry:
    if ((sock = accept(osfd, addr, addrlen)) == -1) {
      if (((err = WSAGetLastError()) == WSAEWOULDBLOCK) &&
          (!fd->secret->nonblocking)) {
        tv.tv_sec = MPR_IntervalToSeconds(timeout);
        tv.tv_usec = MPR_IntervalToMicroseconds(timeout -
                                               MPR_SecondsToInterval(tv.tv_sec));
        tvp = &tv;
...
static PRInt32 _nt_nonblock_writev(PRFileDesc* fd, const PRIOVec* iov, int size,
  ...
  for (index = 0; index < size; index++) {
    ...
    if (rv != iov[index].iov_len) {
      if (rv < 0) {
        if (fd->secret->nonblocking &&
            (MPR_GetError() == PR_WOULD_BLOCK_ERROR) && (sent > 0)) {
          return sent;
        } else {
          return -1;
        }
...
static void pt_InsertTimedInternal(pt_Continuation* op) {
  ...
  PRIntervalTime now = MPR_IntervalNow(), op_tmo, qd_tmo;
...
static void ContinuationThread(void* arg) {
  ...
  PRIntervalTime mx_select_ticks =
      MPR_MillisecondsToInterval(PT_DEFAULT_SELECT_MSEC);
  ...
  while (1) {
    ...
    while (NULL == pt_tq.head) {
      status = MPR_WaitCondVar(pt_tq.new_op, PR_INTERVAL_NO_TIMEOUT);
      if ((PR_FAILURE == status) &&
          (PR_PENDING_INTERRUPT_ERROR == MPR_GetError())) {
        break;
      }
    }
    ...
    tv.tv_sec = MPR_IntervalToSeconds(timeout);
    tv.tv_usec = MPR_IntervalToMicroseconds(timeout) % PR_USEC_PER_SEC;
...
static int pt_Continue(pt_Continuation* op) {
  ...
  op->complete = MPR_NewCondVar(pt_tq.ml);

  MPR_Lock(pt_tq.ml); /* we provide the locking */

  pt_InsertTimedInternal(op); /* insert in the structure */

  MPR_NotifyCondVar(pt_tq.new_op); /* notify the continuation thread */

  while (pt_continuation_done != op->status) /* wait for completion */
  {
    rv = MPR_WaitCondVar(op->complete, PR_INTERVAL_NO_TIMEOUT);
    /*
     ...
     */
    if ((PR_FAILURE == rv) && (PR_PENDING_INTERRUPT_ERROR == MPR_GetError())) {
      op->status = pt_continuation_abort; /* our status */
    }
  }

  MPR_Unlock(pt_tq.ml); /* we provide the locking */

  MPR_DestroyCondVar(op->complete);
