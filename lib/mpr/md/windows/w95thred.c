...
void _PR_MD_EARLY_INIT() {
  ...
  _fd_waiting_for_overlapped_done_lock = MPR_NewLock();
#endif
...
void _PR_MD_CLEANUP_BEFORE_EXIT(void) {
  ...
  if (_fd_waiting_for_overlapped_done_lock) {
    PRIntervalTime delay = MPR_MillisecondsToInterval(1000);
    PRFileDescList* cur;
    do {
...
#  endif
      if (cur) {
        MPR_Sleep(delay);  // wait another 1s.
      }
    } while (cur);

    MPR_DestroyLock(_fd_waiting_for_overlapped_done_lock);
  }
#endif
}
...
void _PR_MD_SET_PRIORITY(_MDThread* thread, PRThreadPriority newPri) {
  ...
  if (!rv) {
    PR_LOG(_pr_thread_lm, PR_LOG_MIN,
           ("MPR_SetThreadPriority: can't set thread priority\n"));
  }
  return;
}
