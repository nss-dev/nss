...
void _PR_NT_InitSids(void) {
  ...
  _pr_nt_sids.owner = (PSID)MPR_Malloc(dwLength);
  ...
  _pr_nt_sids.group = (PSID)MPR_Malloc(dwLength);
...
void _PR_NT_FreeSids(void) {
  ...
  if (_pr_nt_sids.owner) {
    MPR_Free(_pr_nt_sids.owner);
  }
  if (_pr_nt_sids.group) {
    MPR_Free(_pr_nt_sids.group);
  }
...
PRStatus _PR_NT_MakeSecurityDescriptorACL(PRIntn mode, DWORD accessTable[],
  ...
  pSD = (PSECURITY_DESCRIPTOR)MPR_Malloc(SECURITY_DESCRIPTOR_MIN_LENGTH);
  if (pSD == NULL) {
