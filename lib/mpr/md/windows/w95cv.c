 ...
 *
 *  We use a deferred condition notify algorithm.  When MPR_NotifyCondVar
 *  or MPR_NotifyAllCondVar is called, the condition notifies are simply
...
void _PR_MD_WAIT_CV(_MDCVar* cv, _MDLock* lock, PRIntervalTime timeout) {
  ...
  DWORD msecs = (timeout == PR_INTERVAL_NO_TIMEOUT)
                    ? INFINITE
                    : MPR_IntervalToMilliseconds(timeout);
