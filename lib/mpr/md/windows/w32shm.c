...
extern PRSharedMemory* _MD_OpenSharedMemory(const char* name, PRSize size,
  ...
  if (PR_FAILURE == rc) {
    ...
    PR_LOG(_pr_shm_lm, PR_LOG_DEBUG, ("MPR_OpenSharedMemory: name is invalid"));
    return (NULL);
  }
...
#  endif
    ...
    if (NULL == shm->handle) {
      PR_LOG(_pr_shm_lm, PR_LOG_DEBUG,
             ("MPR_OpenSharedMemory: CreateFileMapping() failed: %s",
              shm->ipcname));
      ...
      if ((flags & PR_SHM_EXCL) && (GetLastError() == ERROR_ALREADY_EXISTS)) {
        PR_LOG(_pr_shm_lm, PR_LOG_DEBUG,
               ("MPR_OpenSharedMemory: Request exclusive & already exists",
                shm->ipcname));
        MPR_SetError(PR_FILE_EXISTS_ERROR, ERROR_ALREADY_EXISTS);
        ...
        PR_LOG(
            ...
            ("MPR_OpenSharedMemory: CreateFileMapping() success: %s, handle: %d",
...
#  endif
    if (NULL == shm->handle) {
      _PR_MD_MAP_DEFAULT_ERROR(GetLastError());
      PR_LOG(_pr_shm_lm, PR_LOG_DEBUG,
             ("MPR_OpenSharedMemory: OpenFileMapping() failed: %s, error: %d",
      ...
      PR_LOG(_pr_shm_lm, PR_LOG_DEBUG,
             ("MPR_OpenSharedMemory: OpenFileMapping() success: %s, handle: %d",
              shm->ipcname, shm->handle));
      return (shm);
    }
  }
...
extern PRFileMap* _md_OpenAnonFileMap(const char* dirName, PRSize size,
  ...
  fm = MPR_CreateFileMap((PRFileDesc*)-1, size, prot);
  if (NULL == fm) {
    PR_LOG(_pr_shma_lm, PR_LOG_DEBUG,
           ("_md_OpenAnonFileMap(): MPR_CreateFileMap(): failed"));
    goto Finished;
  }
...
extern PRStatus _md_ExportFileMapAsString(PRFileMap* fm, PRSize bufSize,
  ...
  written = MPR_snprintf(buf, (PRUint32)bufSize, "%d:%" PR_PRIdOSFD ":%ld",
...
extern PRFileMap* _md_ImportFileMapFromString(const char* fmstring) {
  ...
  MPR_sscanf(fmstring, "%d:%" PR_SCNdOSFD ":%ld", &prot, &hFileMap, &dwAccess);
