  ...
  switch (osvi.dwPlatformId) {
    case VER_PLATFORM_WIN32_NT:
      if (PR_SI_SYSNAME == cmd) {
        (void)MPR_snprintf(name, namelen, "Windows_NT");
      } else if (PR_SI_RELEASE == cmd) {
        (void)MPR_snprintf(name, namelen, "%d.%d", osvi.dwMajorVersion,
                          osvi.dwMinorVersion);
      } else if (PR_SI_RELEASE_BUILD == cmd) {
        (void)MPR_snprintf(name, namelen, "%d", osvi.dwBuildNumber);
      }
      break;
    case VER_PLATFORM_WIN32_WINDOWS:
      if (PR_SI_SYSNAME == cmd) {
        if ((osvi.dwMajorVersion > 4) ||
          ...
          (void)MPR_snprintf(name, namelen, "Windows_98");
        } else {
          (void)MPR_snprintf(name, namelen, "Windows_95");
        }
      } else if (PR_SI_RELEASE == cmd) {
        (void)MPR_snprintf(name, namelen, "%d.%d", osvi.dwMajorVersion,
                          osvi.dwMinorVersion);
    ...
    case VER_PLATFORM_WIN32_CE:
      if (PR_SI_SYSNAME == cmd) {
        (void)MPR_snprintf(name, namelen, "Windows_CE");
      } else if (PR_SI_RELEASE == cmd) {
        (void)MPR_snprintf(name, namelen, "%d.%d", osvi.dwMajorVersion,
                          osvi.dwMinorVersion);
      } else if (PR_SI_RELEASE_BUILD == cmd) {
    ...
    default:
      if (PR_SI_SYSNAME == cmd) {
        (void)MPR_snprintf(name, namelen, "Windows_Unknown");
      } else if (PR_SI_RELEASE == cmd) {
        (void)MPR_snprintf(name, namelen, "%d.%d", 0, 0);
      } else if (PR_SI_RELEASE_BUILD == cmd) {
        if (namelen) {
          *name = 0;
        }
...
#    pragma warning(disable : 4035)
void MPR_StackPush(PRStack* stack, PRStackElem* stack_elem) {
#    if defined(__GNUC__)
  void** tos = (void**)stack;
  void* tmp;
...
#    pragma warning(disable : 4035)
PRStackElem* MPR_StackPop(PRStack* stack) {
#    if defined(__GNUC__)
  void** tos = (void**)stack;
  void* tmp;
