...
** should be created joinable.  Then, use MPR_JoinThread to synchrnoize the
...
*/
NSPR_API(PRThread*) MPR_CreateThread(PRThreadType type,
...
** terminated.
*/
NSPR_API(PRStatus) MPR_JoinThread(PRThread *thread);

/*
** Return the current thread object for the currently running code.
** Never returns NULL.
*/
NSPR_API(PRThread*) MPR_GetCurrentThread(void);
#ifndef NO_NSPR_10_SUPPORT
#define PR_CurrentThread() MPR_GetCurrentThread() /* for nspr1.0 compat. */
#endif /* NO_NSPR_10_SUPPORT */

/*
** Get the priority of "thread".
*/
NSPR_API(PRThreadPriority) MPR_GetThreadPriority(const PRThread *thread);

/*
** Change the priority of the "thread" to "priority".
**
** MPR_SetThreadPriority works in a best-effort manner. On some platforms a
...
** effect except causing a future MPR_GetThreadPriority call to return
** |priority|.
*/
NSPR_API(void) MPR_SetThreadPriority(PRThread *thread, PRThreadPriority priority);

/*
** Set the name of the current thread, which will be visible in a debugger
** and accessible via a call to MPR_GetThreadName().
*/
NSPR_API(PRStatus) MPR_SetCurrentThreadName(const char *name);

/*
** Return the name of "thread", if set.  Otherwise return NULL.
*/
NSPR_API(const char *) MPR_GetThreadName(const PRThread *thread);

/*
** This routine returns a new index for per-thread-private data table.
** The index is visible to all threads within a process. This index can
** be used with the MPR_SetThreadPrivate() and MPR_GetThreadPrivate() routines
...
NSPR_API(PRStatus) MPR_NewThreadPrivateIndex(
...
*/
NSPR_API(PRStatus) MPR_SetThreadPrivate(PRUintn tpdIndex, void *priv);
...
** A thread can only get access to its own thread-specific-data.
*/
NSPR_API(void*) MPR_GetThreadPrivate(PRUintn tpdIndex);
...
**
** MPR_Interrupt may itself fail if the target thread is invalid.
*/
NSPR_API(PRStatus) MPR_Interrupt(PRThread *thread);

/*
** Clear the interrupt request for the calling thread. If no such request
** is pending, this operation is a noop.
*/
NSPR_API(void) MPR_ClearInterrupt(void);

/*
** Block the interrupt for the calling thread.
*/
NSPR_API(void) MPR_BlockInterrupt(void);

/*
** Unblock the interrupt for the calling thread.
*/
NSPR_API(void) MPR_UnblockInterrupt(void);
...
** equivalent to calling MPR_Yield. Calling MPR_Sleep with an argument
** equivalent to PR_INTERVAL_NO_TIMEOUT is an error and will result
** in a PR_FAILURE error return.
*/
NSPR_API(PRStatus) MPR_Sleep(PRIntervalTime ticks);

/*
** Get the scoping of this thread.
*/
NSPR_API(PRThreadScope) MPR_GetThreadScope(const PRThread *thread);

/*
** Get the type of this thread.
*/
NSPR_API(PRThreadType) MPR_GetThreadType(const PRThread *thread);

/*
** Get the join state of this thread.
*/
NSPR_API(PRThreadState) MPR_GetThreadState(const PRThread *thread);
