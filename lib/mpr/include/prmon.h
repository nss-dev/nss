...
** is low.
*/
NSPR_API(PRMonitor*) MPR_NewMonitor(void);
...
**
*/
NSPR_API(void) MPR_DestroyMonitor(PRMonitor *mon);
...
*/
NSPR_API(void) MPR_EnterMonitor(PRMonitor *mon);
...
*/
NSPR_API(PRStatus) MPR_ExitMonitor(PRMonitor *mon);
...
*/
NSPR_API(PRStatus) MPR_Wait(PRMonitor *mon, PRIntervalTime ticks);

/*
** Notify a thread waiting on the monitor's condition variable. If a thread
** is waiting on the condition variable (using MPR_Wait) then it is awakened
** and attempts to reenter the monitor.
*/
NSPR_API(PRStatus) MPR_Notify(PRMonitor *mon);
...
** monitor.
*/
NSPR_API(PRStatus) MPR_NotifyAll(PRMonitor *mon);
...
#define PR_ASSERT_CURRENT_THREAD_IN_MONITOR(/* PRMonitor* */ mon) \
    MPR_AssertCurrentThreadInMonitor(mon)
#else
#define PR_ASSERT_CURRENT_THREAD_IN_MONITOR(/* PRMonitor* */ mon)
#endif

/* Don't call this function directly. */
NSPR_API(void) MPR_AssertCurrentThreadInMonitor(PRMonitor *mon);
