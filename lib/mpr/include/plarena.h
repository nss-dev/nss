...
#define PL_INIT_ARENA_POOL(pool, name, size) \
        MPL_InitArenaPool(pool, name, size, PL_ARENA_CONST_ALIGN_MASK + 1)
#else
...
#define PL_ARENA_ALLOCATE(p, pool, nb) \
    PR_BEGIN_MACRO \
        ...
        if (_nb < (PRUint32)nb) { \
            ...
            _p = (PRUword)MPL_ArenaAllocate(pool, _nb); \
        } else { \
            _a->avail += _nb; \
        } \
        p = (void *)_p; \
...
#define PL_ARENA_GROW(p, pool, size, incr) \
    PR_BEGIN_MACRO \
        ...
        if (_incr < (PRUint32)incr) { \
            ...
            p = MPL_ArenaGrow(pool, p, size, (PRUint32)incr); \
        } \
        if (p) {\
...
**/
PR_EXTERN(void) MPL_InitArenaPool(
    PLArenaPool *pool, const char *name, PRUint32 size, PRUint32 align);

/*
** Finish using arenas, freeing all memory associated with them.
** NOTE: this function is now a no-op. If you want to free a single
** PLArenaPoolUse use MPL_FreeArenaPool() or MPL_FinishArenaPool().
**/
PR_EXTERN(void) MPL_ArenaFinish(void);

/*
** Free the arenas in pool.  The user may continue to allocate from pool
** after calling this function.  There is no need to call MPL_InitArenaPool()
** again unless MPL_FinishArenaPool(pool) has been called.
**/
PR_EXTERN(void) MPL_FreeArenaPool(PLArenaPool *pool);

/*
** Free the arenas in pool and finish using it altogether.
**/
PR_EXTERN(void) MPL_FinishArenaPool(PLArenaPool *pool);

/*
** Compact all of the arenas in a pool so that no space is wasted.
** NOT IMPLEMENTED.  Do not use.
**/
PR_EXTERN(void) MPL_CompactArenaPool(PLArenaPool *pool);
...
** PL_ARENA_*() macros.
**/
PR_EXTERN(void *) MPL_ArenaAllocate(PLArenaPool *pool, PRUint32 nb);

PR_EXTERN(void *) MPL_ArenaGrow(
    PLArenaPool *pool, void *p, PRUint32 size, PRUint32 incr);

PR_EXTERN(void) MPL_ArenaRelease(PLArenaPool *pool, char *mark);

/*
** memset contents of all arenas in pool to pattern
*/
PR_EXTERN(void) MPL_ClearArenaPool(PLArenaPool *pool, PRInt32 pattern);
...
** structure.
*/
PR_EXTERN(size_t) MPL_SizeOfArenaPoolExcludingPool(
