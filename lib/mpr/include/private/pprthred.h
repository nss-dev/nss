...
** thread. MPR_Init does this automatically for the primordial thread.
*/
NSPR_API(PRThread*) MPR_AttachThread(PRThreadType type,
...
** created by MPR_Init for the primordial thread.
**
** This call returns after the nspr thread object is destroyed.
*/
NSPR_API(void) MPR_DetachThread(void);

/*
** Get the id of the named thread. Each thread is assigned a unique id
** when it is created or attached.
*/
NSPR_API(PRUint32) MPR_GetThreadID(PRThread *thread);
...
NSPR_API(void) MPR_SetThreadDumpProc(
...
*/
NSPR_API(PRInt32) MPR_GetThreadAffinityMask(PRThread *thread, PRUint32 *mask);
...
** Returns 0 on success, -1 on failure.
*/
NSPR_API(PRInt32) MPR_SetThreadAffinityMask(PRThread *thread, PRUint32 mask );
...
*/
NSPR_API(void) MPR_SetThreadRecycleMode(PRUint32 flag);
...
/*
** Same as MPR_CreateThread except that the thread is marked as garbage
** collectible.
*/
NSPR_API(PRThread*) MPR_CreateThreadGCAble(PRThreadType type,
        void (*start)(void *arg),
        void *arg,
        PRThreadPriority priority,
        PRThreadScope scope,
...
/*
** Same as MPR_AttachThread except that the thread being attached is marked as
** garbage collectible.
*/
...
*/
NSPR_API(PRWord *) MPR_GetGCRegisters(PRThread *t, int isCurrent, int *np);
...
*/
NSPR_API(void*) MPR_GetExecutionEnvironment(PRThread *thread);
NSPR_API(void) MPR_SetExecutionEnvironment(PRThread* thread, void *environment);
...
** stops, returning the failure value from MPR_EnumerateThreads.
** Needed by the garbage collector.
*/
typedef PRStatus (PR_CALLBACK *PREnumerator)(PRThread *t, int i, void *arg);
NSPR_API(PRStatus) MPR_EnumerateThreads(PREnumerator func, void *arg);
...
*/
NSPR_API(PRStatus)
MPR_ThreadScanStackPointers(PRThread* t,
                           PRScanStackFun scanFun, void* scanClosure);

/*
** Calls MPR_ThreadScanStackPointers for every thread.
*/
NSPR_API(PRStatus)
...
** is low.
*/
NSPR_API(PRMonitor*) MPR_NewNamedMonitor(const char* name);
...
*/
NSPR_API(PRIntn) MPR_GetMonitorEntryCount(PRMonitor *mon);

/*
** Just like MPR_CEnterMonitor except that if the monitor is owned by
...
/* I think MPR_GetMonitorEntryCount is useless. All you really want is this... */
#define PR_InMonitor(m)     (MPR_GetMonitorEntryCount(m) > 0)
