...
#define        _MD_CLEAN_THREAD(_thread) \
    PR_BEGIN_MACRO \
        MPR_DestroyCondVar(_thread->md.asyncIOCVar); \
        MPR_DestroyLock(_thread->md.asyncIOLock); \
    PR_END_MACRO
...
NSPR_API(void) MPT_FPrintStats(PRFileDesc *fd, const char *msg);
...
#define _PR_ATTACHED        0x04        /* created via MPR_AttachThread */
#define _PR_PRIMORDIAL      0x08        /* the thread that called MPR_Init */
...
#define _PR_DEADQ_LOCK      MPR_Lock(_pr_deadQLock);
#define _PR_DEADQ_UNLOCK    MPR_Unlock(_pr_deadQLock);
...
struct PRMonitor {
    ...
    PRInt32 refCount;           /* reference count, an atomic variable.
                                 * MPR_NewMonitor adds a reference to the
                                 * newly created PRMonitor, and
                                 * MPR_DestroyMonitor releases that reference.
                                 * MPR_ExitMonitor adds a reference before
...
struct PRThread {
    ...
    */
    ...
    PRIntn  errorStringLength;      /* textLength from last call to MPR_SetErrorText() */
    ...
    struct pollfd *syspoll_list;    /* Unix polling list used by MPR_Poll */
    ...
    PRCList links;
    PRCList waitQLinks;             /* when thread is MPR_Wait'ing */
...
**   on the PRFileDesc is MPR_Close().
...
/* --- MPR_GetRandomNoise() related things --- */
...
/* end MPR_GetRandomNoise() related */
