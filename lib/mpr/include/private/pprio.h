...
NSPR_API(const PRIOMethods*)    MPR_GetFileMethods(void);
NSPR_API(const PRIOMethods*)    MPR_GetTCPMethods(void);
NSPR_API(const PRIOMethods*)    MPR_GetUDPMethods(void);
NSPR_API(const PRIOMethods*)    MPR_GetPipeMethods(void);
...
*/
NSPR_API(PROsfd)       MPR_FileDesc2NativeHandle(PRFileDesc *);
NSPR_API(void)         MPR_ChangeFileDescNativeHandle(PRFileDesc *, PROsfd);
NSPR_API(PRFileDesc*)  MPR_AllocFileDesc(PROsfd osfd,
                                        const PRIOMethods *methods);
NSPR_API(void)         PR_FreeFileDesc(PRFileDesc *fd);
/*
** Import an existing OS file to NSPR.
*/
NSPR_API(PRFileDesc*)  MPR_ImportFile(PROsfd osfd);
NSPR_API(PRFileDesc*)  MPR_ImportPipe(PROsfd osfd);
NSPR_API(PRFileDesc*)  MPR_ImportTCPSocket(PROsfd osfd);
NSPR_API(PRFileDesc*)  MPR_ImportUDPSocket(PROsfd osfd);


/*
 *************************************************************************
 * FUNCTION: MPR_CreateSocketPollFd
 * DESCRIPTION:
 ...
 *     None
 * RETURN: PRFileDesc*
 *     Upon successful completion, MPR_CreateSocketPollFd returns a pointer
...
NSPR_API(PRFileDesc*)   MPR_CreateSocketPollFd(PROsfd osfd);

/*
 *************************************************************************
 * FUNCTION: MPR_DestroySocketPollFd
 * DESCRIPTION:
 *     Destroy the PRFileDesc wrapper created by MPR_CreateSocketPollFd
 * INPUTS:
 *     None
 * OUTPUTS:
 *     None
 * RETURN: PRFileDesc*
 *     Upon successful completion, MPR_DestroySocketPollFd returns
 *     PR_SUCCESS, else PR_FAILURE
 *
 **************************************************************************
 */

NSPR_API(PRStatus) MPR_DestroySocketPollFd(PRFileDesc *fd);


/*
** Macros for MPR_Socket
**
...
*/
NSPR_API(PRFileDesc*)   MPR_Socket(PRInt32 domain, PRInt32 type, PRInt32 proto);

/* FUNCTION: MPR_LockFile
** DESCRIPTION:
...
*/
NSPR_API(PRStatus) MPR_LockFile(PRFileDesc *fd);

/* FUNCTION: MPR_TLockFile
** DESCRIPTION:
...
**    PR_FAILURE otherwise
*/
NSPR_API(PRStatus) MPR_TLockFile(PRFileDesc *fd);

/* FUNCTION: MPR_UnlockFile
** DESCRIPTION:
...
**    PR_FAILURE otherwise
*/
NSPR_API(PRStatus) MPR_UnlockFile(PRFileDesc *fd);

/*
** Emulate acceptread by accept and recv.
*/
NSPR_API(PRInt32) MPR_EmulateAcceptRead(PRFileDesc *sd, PRFileDesc **nd,
...
*/
NSPR_API(PRInt32) MPR_EmulateSendFile(
...
**    the acceptcontext.  The generic version of MPR_AcceptRead always
...
**    order to make certain calls.  By default, a socket connected via
**    AcceptEx can only do a limited number of things without updating
**    the acceptcontext.  The generic version of MPR_Accept always
**    updates the accept context.  This version does not.
**/
...
**    Are MPR_Read, MPR_Write, PR_Transmitfile, and MPR_Close.
*/
