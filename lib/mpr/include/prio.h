...
** PRTransmitFileFlags
**
** Flags for MPR_TransmitFile.  Pass PR_TRANSMITFILE_CLOSE_SOCKET to
** MPR_TransmitFile if the connection should be closed after the file
** is transmitted.
***************************************************************************
*/
...
NSPR_API(PRDescType) MPR_GetDescType(PRFileDesc *file);
 ...
 * OUTPUTS: none
 * RETURNS: PRFileDesc *
 *     If the argument is valid, MPR_GetSpecialFD returns a file descriptor
 *     that represents the corresponding standard I/O stream.  Otherwise,
 *     MPR_GetSpecialFD returns NULL and sets error PR_INVALID_ARGUMENT_ERROR.
 **************************************************************************
 */
...
NSPR_API(PRFileDesc*) MPR_GetSpecialFD(PRSpecialFD id);

#define PR_STDIN    MPR_GetSpecialFD(PR_StandardInput)
#define PR_STDOUT   MPR_GetSpecialFD(PR_StandardOutput)
#define PR_STDERR   MPR_GetSpecialFD(PR_StandardError)
 ...
 * equivalent.
 *
 *      rv = MPR_PushIOLayer(stack, PR_TOP_IO_LAYER, my_layer);
 *      rv = MPR_PushIOLayer(stack, MPR_GetLayersIdentity(stack), my_layer)
 *
...
NSPR_API(PRDescIdentity) MPR_GetUniqueIdentity(const char *layer_name);
NSPR_API(const char*) MPR_GetNameForIdentity(PRDescIdentity ident);
NSPR_API(PRDescIdentity) MPR_GetLayersIdentity(PRFileDesc* fd);
NSPR_API(PRFileDesc*) MPR_GetIdentitiesLayer(PRFileDesc* fd_stack, PRDescIdentity id);

/*
 **************************************************************************
 * MPR_GetDefaultIOMethods: Accessing the default methods table.
 ...
 **************************************************************************
 */
NSPR_API(const PRIOMethods *) MPR_GetDefaultIOMethods(void);

/*
 **************************************************************************
 * Creating a layer
 *
 * A new layer may be allocated by calling MPR_CreateIOLayerStub(). The
 ...
 **************************************************************************
 */
NSPR_API(PRFileDesc*) MPR_CreateIOLayerStub(
 ...
 * Creating a layer
 *
 * A new stack may be created by calling MPR_CreateIOLayer(). The
 ...
 * header. The old style stack, created by a call to MPR_PushIOLayer,
 * results in modifying contents of the top layer of the stack, when
 * pushing and popping layers of the stack.
 **************************************************************************
 */
NSPR_API(PRFileDesc*) MPR_CreateIOLayer(PRFileDesc* fd);

/*
 **************************************************************************
 * Pushing a layer
 *
 * A file descriptor (perhaps allocated using MPR_CreateIOLayerStub()) may
 ...
 **************************************************************************
 */
NSPR_API(PRStatus) MPR_PushIOLayer(
 ...
 **************************************************************************
 */
NSPR_API(PRFileDesc*) MPR_PopIOLayer(PRFileDesc *fd_stack, PRDescIdentity id);
...
** The 'mode' argument may be ignored by MPR_Open on other platforms.
**
**   00400   Read by owner.
...
NSPR_API(PRFileDesc*) MPR_Open(const char *name, PRIntn flags, PRIntn mode);

/*
 **************************************************************************
 * FUNCTION: MPR_OpenFile
 * DESCRIPTION:
 *     Open a file for reading, writing, or both.
 *     MPR_OpenFile has the same prototype as MPR_Open but implements
...
NSPR_API(PRFileDesc*) MPR_OpenFile(
...
NSPR_API(PRStatus)    MPR_Close(PRFileDesc *fd);
...
NSPR_API(PRInt32) MPR_Read(PRFileDesc *fd, void *buf, PRInt32 amount);
 ...
 *     for the failure is obtained by calling MPR_GetError().
 ***************************************************************************
 */

NSPR_API(PRInt32) MPR_Write(PRFileDesc *fd,const void *buf,PRInt32 amount);
...
NSPR_API(PRStatus) MPR_Delete(const char *name);
...
/****************************************************************************
 * FUNCTION: MPR_GetFileInfo, MPR_GetFileInfo64
 * DESCRIPTION:
 ...
 * RETURN: PRStatus
 *     MPR_GetFileInfo returns PR_SUCCESS if file information is successfully
 *     obtained, otherwise it returns PR_FAILURE.
 ***************************************************************************
 */

NSPR_API(PRStatus) MPR_GetFileInfo(const char *fn, PRFileInfo *info);
NSPR_API(PRStatus) MPR_GetFileInfo64(const char *fn, PRFileInfo64 *info);
...
/*
 **************************************************************************
 * FUNCTION: MPR_GetOpenFileInfo, MPR_GetOpenFileInfo64
 * DESCRIPTION:
 ...
 * OUTPUTS:
 *     Same as MPR_GetFileInfo, MPR_GetFileInfo64
 * RETURN: PRStatus
 *     MPR_GetFileInfo returns PR_SUCCESS if file information is successfully
 *     obtained, otherwise it returns PR_FAILURE.
 ***************************************************************************
...
NSPR_API(PRStatus) MPR_GetOpenFileInfo(PRFileDesc *fd, PRFileInfo *info);
NSPR_API(PRStatus) MPR_GetOpenFileInfo64(PRFileDesc *fd, PRFileInfo64 *info);
...
NSPR_API(PRStatus)    MPR_Rename(const char *from, const char *to);
...
NSPR_API(PRStatus) MPR_Access(const char *name, PRAccessHow how);

/*
 *************************************************************************
 * FUNCTION: MPR_Seek, MPR_Seek64
 * DESCRIPTION:
 ...
 *     If the MPR_Seek() function fails, the file offset remains
 *     unchanged, and the returned value is -1. The error code can
 *     then be retrieved via MPR_GetError().
 *************************************************************************
 */

NSPR_API(PROffset32) MPR_Seek(PRFileDesc *fd, PROffset32 offset, PRSeekWhence whence);
NSPR_API(PROffset64) MPR_Seek64(PRFileDesc *fd, PROffset64 offset, PRSeekWhence whence);

/*
 ************************************************************************
 * FUNCTION: MPR_Available
 * DESCRIPTION:
 ...
 *     Upon successful completion, MPR_Available returns the number of
 *     bytes beyond the current read pointer that is available for
 *     reading.  Otherwise, it returns a -1 and the reason for the
 *     failure can be retrieved via MPR_GetError().
 ************************************************************************
 */

NSPR_API(PRInt32) MPR_Available(PRFileDesc *fd);
NSPR_API(PRInt64) MPR_Available64(PRFileDesc *fd);
...
NSPR_API(PRStatus)  MPR_Sync(PRFileDesc *fd);
...
NSPR_API(PRDir*) MPR_OpenDir(const char *name);
 ...
 *     error occurs. The actual reason can be retrieved via MPR_GetError().
 *************************************************************************
 */
...
NSPR_API(PRDirEntry*) MPR_ReadDir(PRDir *dir, PRDirFlags flags);
 ...
 *        trieved using MPR_GetError().
 *************************************************************************
 */

NSPR_API(PRStatus) MPR_CloseDir(PRDir *dir);
 ...
 *     PRIntn mode
 *        See 'mode' definiton in MPR_Open().
 * OUTPUTS:
 *     None
 * RETURN: PRStatus
 *        If successful, will return a status of PR_SUCCESS. Otherwise
...
NSPR_API(PRStatus) MPR_MkDir(const char *name, PRIntn mode);

/*
 *************************************************************************
 * FUNCTION: MPR_MakeDir
 * DESCRIPTION:
 *     Create a new directory with the given name and access mode.
 *     MPR_MakeDir has the same prototype as MPR_MkDir but implements
 *     the specified access mode where possible.
 *************************************************************************
 */

NSPR_API(PRStatus) MPR_MakeDir(const char *name, PRIntn mode);
...
NSPR_API(PRStatus) MPR_RmDir(const char *name);

/*
 *************************************************************************
 * FUNCTION: MPR_NewUDPSocket
 * DESCRIPTION:
 ...
 *     None
 * RETURN: PRFileDesc*
 *     Upon successful completion, MPR_NewUDPSocket returns a pointer
...
NSPR_API(PRFileDesc*)    MPR_NewUDPSocket(void);

/*
 *************************************************************************
 * FUNCTION: MPR_NewTCPSocket
 * DESCRIPTION:
 ...
 *     None
 * RETURN: PRFileDesc*
 *     Upon successful completion, MPR_NewTCPSocket returns a pointer
...
NSPR_API(PRFileDesc*)    MPR_NewTCPSocket(void);

/*
 *************************************************************************
 * FUNCTION: MPR_OpenUDPSocket
 * DESCRIPTION:
 ...
 *     None
 * RETURN: PRFileDesc*
 *     Upon successful completion, MPR_OpenUDPSocket returns a pointer
 *     to the PRFileDesc created for the newly opened UDP socket.
 *     Returns a NULL pointer if the creation of a new UDP socket failed.
 *
 **************************************************************************
...
NSPR_API(PRFileDesc*)    MPR_OpenUDPSocket(PRIntn af);

/*
 *************************************************************************
 * FUNCTION: MPR_OpenTCPSocket
 * DESCRIPTION:
...
NSPR_API(PRFileDesc*)    MPR_OpenTCPSocket(PRIntn af);

/*
 *************************************************************************
 * FUNCTION: MPR_Connect
 * DESCRIPTION:
 ...
 *     None
 * RETURN: PRStatus
 *     Upon successful completion of connection initiation, MPR_Connect
 *     returns PR_SUCCESS.  Otherwise, it returns PR_FAILURE.  Further
 *     failure information can be obtained by calling MPR_GetError().
 **************************************************************************
 */

NSPR_API(PRStatus) MPR_Connect(
    PRFileDesc *fd, const PRNetAddr *addr, PRIntervalTime timeout);

/*
 *************************************************************************
 * FUNCTION: MPR_ConnectContinue
 * DESCRIPTION:
 *     Continue a nonblocking connect.  After a nonblocking connect
 *     is initiated with MPR_Connect() (which fails with
 *     PR_IN_PROGRESS_ERROR), one should call MPR_Poll() on the socket,
 *     with the in_flags PR_POLL_WRITE | PR_POLL_EXCEPT.  When
 *     MPR_Poll() returns, one calls MPR_ConnectContinue() on the
 *     socket to determine whether the nonblocking connect has
 *     completed or is still in progress.  Repeat the MPR_Poll(),
 *     MPR_ConnectContinue() sequence until the nonblocking connect
 *     has completed.
 * INPUTS:
 *     PRFileDesc *fd
 ...
 *     MPR_ConnectContinue returns PR_SUCCESS.  If MPR_ConnectContinue()
 *     returns PR_FAILURE, call MPR_GetError():
 ...
 *       PR_POLL_WRITE|PR_POLL_EXCEPT and retry MPR_ConnectContinue
 *       later when MPR_Poll() returns.
 *     - Other errors: the nonblocking connect has failed with this
 *       error code.
 */

NSPR_API(PRStatus)    MPR_ConnectContinue(PRFileDesc *fd, PRInt16 out_flags);

/*
 *************************************************************************
 * THIS FUNCTION IS DEPRECATED.  USE MPR_ConnectContinue INSTEAD.
 *
 * FUNCTION: MPR_GetConnectStatus
 * DESCRIPTION:
 *     Get the completion status of a nonblocking connect.  After
 *     a nonblocking connect is initiated with MPR_Connect() (which
 *     fails with PR_IN_PROGRESS_ERROR), one should call MPR_Poll()
 *     on the socket, with the in_flags PR_POLL_WRITE | PR_POLL_EXCEPT.
 *     When MPR_Poll() returns, one calls MPR_GetConnectStatus on the
 ...
 *         MPR_Poll() should have been called and set the out_flags.
 * RETURN: PRStatus
 *     If the nonblocking connect has successfully completed,
 *     MPR_GetConnectStatus returns PR_SUCCESS.  If MPR_GetConnectStatus()
 *     returns PR_FAILURE, call MPR_GetError():
 *     - PR_IN_PROGRESS_ERROR: the nonblocking connect is still in
...
NSPR_API(PRStatus)    MPR_GetConnectStatus(const PRPollDesc *pd);
 ...
 * RETURN: PRFileDesc*
 *     Upon successful acceptance of a connection, MPR_Accept
 *     returns a valid file descriptor. Otherwise, it returns NULL.
 *     Further failure information can be obtained by calling MPR_GetError().
 **************************************************************************
 */

NSPR_API(PRFileDesc*) MPR_Accept(
    PRFileDesc *fd, PRNetAddr *addr, PRIntervalTime timeout);
 ...
 *     None
 * RETURN: PRStatus
 *     Upon successful binding of an address to a socket, MPR_Bind
 *     returns PR_SUCCESS.  Otherwise, it returns PR_FAILURE.  Further
 *     failure information can be obtained by calling MPR_GetError().
 **************************************************************************
 */

NSPR_API(PRStatus) MPR_Bind(PRFileDesc *fd, const PRNetAddr *addr);
 ...
 *     None
 * RETURN: PRStatus
 *     Upon successful completion of listen request, MPR_Listen
 *     returns PR_SUCCESS.  Otherwise, it returns PR_FAILURE.  Further
 *     failure information can be obtained by calling MPR_GetError().
 **************************************************************************
 */

NSPR_API(PRStatus) MPR_Listen(PRFileDesc *fd, PRIntn backlog);

/*
 *************************************************************************
 * FUNCTION: MPR_Shutdown
 * DESCRIPTION:
 ...
 *     None
 * RETURN: PRStatus
 *     Upon successful completion of shutdown request, MPR_Shutdown
 *     returns PR_SUCCESS.  Otherwise, it returns PR_FAILURE.  Further
 *     failure information can be obtained by calling MPR_GetError().
 **************************************************************************
 */
...
NSPR_API(PRStatus)    MPR_Shutdown(PRFileDesc *fd, PRShutdownHow how);
...
NSPR_API(PRInt32)    MPR_Recv(PRFileDesc *fd, void *buf, PRInt32 amount,
                             PRIntn flags, PRIntervalTime timeout);
...
NSPR_API(PRInt32)    MPR_Send(PRFileDesc *fd, const void *buf, PRInt32 amount,
                             PRIntn flags, PRIntervalTime timeout);

/*
 *************************************************************************
 * FUNCTION: MPR_RecvFrom
 * DESCRIPTION:
...
NSPR_API(PRInt32) MPR_SendTo(
    PRFileDesc *fd, const void *buf, PRInt32 amount, PRIntn flags,
    const PRNetAddr *addr, PRIntervalTime timeout);

/*
*************************************************************************
** FUNCTION: MPR_TransmitFile
** DESCRIPTION:
...
NSPR_API(PRInt32) MPR_TransmitFile(
...
/*
*************************************************************************
** FUNCTION: MPR_SendFile
** DESCRIPTION:
**    MPR_SendFile sends data from a file (sendData->fd) across a socket
...
NSPR_API(PRInt32) MPR_SendFile(
    PRFileDesc *networkSocket, PRSendFileData *sendData,
    PRTransmitFileFlags flags, PRIntervalTime timeout);

/*
*************************************************************************
** FUNCTION: MPR_AcceptRead
** DESCRIPTION:
...
**        A socket descriptor that has been called with the MPR_Listen()
...
**        operation.  MPR_AcceptRead will block indefinitely until the
...
** RETURN: PRStatus
**     Upon successful completion of TCP socket pair, MPR_NewTCPSocketPair
**     returns PR_SUCCESS.  Otherwise, it returns PR_FAILURE.  Further
**     failure information can be obtained by calling MPR_GetError().
** XXX can we implement this on windoze and mac?
**************************************************************************
**/
NSPR_API(PRStatus) MPR_NewTCPSocketPair(PRFileDesc *fds[2]);

/*
*************************************************************************
** FUNCTION: MPR_GetSockName
** DESCRIPTION:
...
** RETURN: PRStatus
**     Upon successful completion, MPR_GetSockName returns PR_SUCCESS.
**     Otherwise, it returns PR_FAILURE.  Further failure information can
**     be obtained by calling MPR_GetError().
**************************************************************************
**/
NSPR_API(PRStatus)  MPR_GetSockName(PRFileDesc *fd, PRNetAddr *addr);

/*
*************************************************************************
** FUNCTION: MPR_GetPeerName
** DESCRIPTION:
...
**       space.
** RETURN: PRStatus
**     Upon successful completion, MPR_GetPeerName returns PR_SUCCESS.
**     Otherwise, it returns PR_FAILURE.  Further failure information can
**     be obtained by calling MPR_GetError().
**************************************************************************
**/
NSPR_API(PRStatus)  MPR_GetPeerName(PRFileDesc *fd, PRNetAddr *addr);

NSPR_API(PRStatus)  MPR_GetSocketOption(
    PRFileDesc *fd, PRSocketOptionData *data);

NSPR_API(PRStatus)  MPR_SetSocketOption(
    PRFileDesc *fd, const PRSocketOptionData *data);
 ...
 * RETURN: PRStatus
 *     Upon successful completion, MPR_SetFDInheritable returns PR_SUCCESS.
 *     Otherwise, it returns PR_FAILURE.  Further failure information can
 *     be obtained by calling MPR_GetError().
 *************************************************************************
 */
NSPR_API(PRStatus) MPR_SetFDInheritable(
    PRFileDesc *fd,
    PRBool inheritable);

/*
 ************************************************************************
 * FUNCTION: MPR_GetInheritedFD
 * DESCRIPTION:
...
NSPR_API(PRFileMap *) MPR_CreateFileMap(
    PRFileDesc *fd,
    PRInt64 size,
    PRFileMapProtect prot);

/*
 * return the alignment (in bytes) of the offset argument to MPR_MemMap
 */
NSPR_API(PRInt32) MPR_GetMemMapAlignment(void);

NSPR_API(void *) MPR_MemMap(
    PRFileMap *fmap,
    PROffset64 offset,  /* must be aligned and sized according to the
                         * return value of MPR_GetMemMapAlignment() */
    PRUint32 len);

NSPR_API(PRStatus) MPR_MemUnmap(void *addr, PRUint32 len);

NSPR_API(PRStatus) MPR_CloseFileMap(PRFileMap *fmap);
 ...
 *
 * On some platforms, the function will call MPR_Sync(fd) internally if it is
 * necessary for flushing modified data to disk synchronously.
 */
NSPR_API(PRStatus) MPR_SyncMemMap(
    PRFileDesc *fd,
    void *addr,
    PRUint32 len);
...
NSPR_API(PRStatus) MPR_CreatePipe(
    PRFileDesc **readPipe,
...
** MPR_Poll that this PRFileDesc object should be ignored.
...
** i/o can be performed on the respective descriptor. MPR_Poll() uses the
** out_flags fields as scratch variables during the call. If MPR_Poll()
...
**                              If this argument is zero MPR_Poll is
**                              equivalent to a MPR_Sleep(timeout).
**
...
** with I/O waiting in a single MPR_Poll call.  Pollable events
...
**
** A new pollable event is created by a MPR_NewPollableEvent
** call and is initially in the unset state.
**
** MPR_WaitForPollableEvent blocks the calling thread until
** the pollable event is set, and then it atomically unsets
** the pollable event before it returns.
**
** To set a pollable event, call MPR_SetPollableEvent.
**
** One can call MPR_Poll with the PR_POLL_READ flag on a pollable
** event.  When the pollable event is set, MPR_Poll returns with
** the PR_POLL_READ flag set in the out_flags.
**
** To close a pollable event, call MPR_DestroyPollableEvent
** (not MPR_Close).
**
**************************************************************************
*/

NSPR_API(PRFileDesc *) MPR_NewPollableEvent(void);

NSPR_API(PRStatus) MPR_DestroyPollableEvent(PRFileDesc *event);

NSPR_API(PRStatus) MPR_SetPollableEvent(PRFileDesc *event);

NSPR_API(PRStatus) MPR_WaitForPollableEvent(PRFileDesc *event);
