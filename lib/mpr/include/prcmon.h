...
/**
** Like MPR_EnterMonitor except use the "address" to find a monitor in the
** monitor cache. If successful, returns the PRMonitor now associated
** with "address". Note that you must MPR_CExitMonitor the address to
...
*/
NSPR_API(PRMonitor*) MPR_CEnterMonitor(void *address);

/*
** Like MPR_ExitMonitor except use the "address" to find a monitor in the
** monitor cache.
*/
NSPR_API(PRStatus) MPR_CExitMonitor(void *address);

/*
** Like MPR_Wait except use the "address" to find a monitor in the
** monitor cache.
*/
NSPR_API(PRStatus) MPR_CWait(void *address, PRIntervalTime timeout);

/*
** Like MPR_Notify except use the "address" to find a monitor in the
** monitor cache.
*/
NSPR_API(PRStatus) MPR_CNotify(void *address);

/*
** Like MPR_NotifyAll except use the "address" to find a monitor in the
** monitor cache.
*/
NSPR_API(PRStatus) MPR_CNotifyAll(void *address);
...
*/
NSPR_API(void) MPR_CSetOnMonitorRecycle(void (PR_CALLBACK *callback)(void *address));
