...
NSPR_API(PRBool) MPR_VersionCheck(const char *importedVersion);

/*
 * Returns a const string of the NSPR library version.
 */
NSPR_API(const char*) MPR_GetVersion(void);
...
** The specificaiton of 'maxPTDs' is ignored.
**
** The value returned by MPR_Initialize is the value returned from the root
** function, 'prmain'.
*/

typedef PRIntn (PR_CALLBACK *PRPrimordialFn)(PRIntn argc, char **argv);

NSPR_API(PRIntn) MPR_Initialize(
    PRPrimordialFn prmain, PRIntn argc, char **argv, PRUintn maxPTDs);

/*
** Return PR_TRUE if MPR_Init has already been called.
*/
NSPR_API(PRBool) MPR_Initialized(void);

/*
 * Perform a graceful shutdown of NSPR.  MPR_Cleanup() may be called by
 * the primordial thread near the end of the main() function.
 *
 * MPR_Cleanup() attempts to synchronize the natural termination of
...
*/
NSPR_API(PRStatus) MPR_SetFDCacheSize(PRIntn low, PRIntn high);
 ...
 * process.
 */
NSPR_API(void) MPR_ProcessExit(PRIntn status);
...
NSPR_API(PRStatus) MPR_CallOnce(
    PRCallOnceType *once,
    PRCallOnceFN    func
);

NSPR_API(PRStatus) MPR_CallOnceWithArg(
    PRCallOnceType      *once,
