 ...
 * - High-performance servers like the fact that MPR_EnterMonitor
 *   only calls MPR_Lock and MPR_ExitMonitor only calls MPR_Unlock.
 *   (A portable implementation would use a PRLock and a PRCondVar
 *   to implement the recursive lock in a monitor and call both
 *   MPR_Lock and MPR_Unlock in MPR_EnterMonitor and MPR_ExitMonitor.)
 ...
 *   instruction).  However, MPR_GetCurrentThread calls calloc if
 ...
 *   MPR_EnterMonitor calls any of these functions, infinite
 *   recursion ensues.
 */
