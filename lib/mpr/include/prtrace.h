...
** PR_TRACE( h1,0,1,2, ...); not MPR_Trace(h1,0,1,2, ...);
**
...
typedef struct PRTraceEntry
{
    ...
    PRTime          time;           /* Value of MPR_Now() at time of trace entry */
...
/* -----------------------------------------------------------------------
** FUNCTION: MPR_CreateTrace() -- Create a trace handle
**
** DESCRIPTION:
**  MPR_CreateTrace() creates a new trace handle. Tracing is
**  enabled for this handle when it is created. The trace handle
**  is intended for use in other Trace Facility calls.
**
**  MPR_CreateTrace() registers the QName, RName and description
...
#define PR_CREATE_TRACE(handle,qName,rName,description)\
    (handle) = MPR_CreateTrace((qName),(rName),(description))
#else
#define PR_CREATE_TRACE(handle,qName,rName,description)
#endif
...
/* -----------------------------------------------------------------------
** FUNCTION: MPR_DestroyTrace() -- Destroy a trace handle
**
** DESCRIPTION:
**  MPR_DestroyTrace() removes the referenced trace handle and
...
/* -----------------------------------------------------------------------
** FUNCTION: MPR_Trace() -- Make a trace entry in the in-memory trace
**
** DESCRIPTION:
** MPR_Trace() makes an entry in the in-memory trace buffer for
...
**
** MPR_Trace() for a specific trace handle may be disabled by
** calling MPR_SetTraceOption() specifying PRTraceDisable for the
...
#define PR_TRACE(handle,ud0,ud1,ud2,ud3,ud4,ud5,ud6,ud7)\
    MPR_Trace((handle),(ud0),(ud1),(ud2),(ud3),(ud4),(ud5),(ud6),(ud7))
#else
#define PR_TRACE(handle,ud0,ud1,ud2,ud3,ud4,ud5,ud6,ud7)
#endif
...
/* -----------------------------------------------------------------------
** FUNCTION: MPR_SetTraceOption() -- Control the Trace Facility
**
** DESCRIPTION:
** MPR_SetTraceOption() controls the Trace Facility. Depending on
...
** (see: ** MPR_RecordTraceEntries()) is being done,
** PRTraceStopRecording causes MPR_RecordTraceEntries() to return
...
** calls to MPR_CreateTrace() will block until the lock is
** released.
**
...
#define PR_SET_TRACE_OPTION(command,value)\
    MPR_SetTraceOption((command),(value))
#else
#define PR_SET_TRACE_OPTION(command,value)
#endif
...
/* -----------------------------------------------------------------------
** FUNCTION: MPR_GetTraceOption() -- Retrieve settings from the Trace Facility
**
** DESCRIPTION:
** MPR_GetTraceOption() retrieves the current setting of the
...
#define PR_GET_TRACE_OPTION(command,value)\
    MPR_GetTraceOption((command),(value))
#else
#define PR_GET_TRACE_OPTION(command,value)
#endif
...
/* -----------------------------------------------------------------------
** FUNCTION: MPR_GetTraceHandleFromName() -- Retrieve an existing
** handle by name.
**
** DESCRIPTION:
** MPR_GetTraceHandleFromName() retreives an existing tracehandle
...
#define PR_GET_TRACE_HANDLE_FROM_NAME(handle,qName,rName)\
    (handle) = MPR_GetTraceHandleFromName((qName),(rName))
#else
#define PR_GET_TRACE_HANDLE_FROM_NAME(handle,qName,rName)
#endif

NSPR_API(PRTraceHandle)
MPR_GetTraceHandleFromName(
    const char *qName,      /* QName search argument */
    const char *rName       /* RName search argument */
);

/* -----------------------------------------------------------------------
** FUNCTION: MPR_GetTraceNameFromHandle() -- Retreive trace name
** by bandle.
**
** DESCRIPTION:
** MPR_GetTraceNameFromHandle() retreives the existing qName,
...
#define PR_GET_TRACE_NAME_FROM_HANDLE(handle,qName,rName,description)\
    MPR_GetTraceNameFromHandle((handle),(qName),(rName),(description))
#else
#define PR_GET_TRACE_NAME_FROM_HANDLE(handle,qName,rName,description)
#endif

NSPR_API(void)
MPR_GetTraceNameFromHandle(
...
/* -----------------------------------------------------------------------
** FUNCTION: MPR_FindNextTraceQname() -- Retrieive a QName handle
** iterator.
**
** DESCRIPTION:
** MPR_FindNextTraceQname() retreives the first or next trace
...
** See: MPR_SetTraceOption( PRLockTraceHandles ).
**
*/
#if defined (DEBUG) || defined (FORCE_NSPR_TRACE)
#define PR_FIND_NEXT_TRACE_QNAME(next,handle)\
    (next) = MPR_FindNextTraceQname((handle))
#else
#define PR_FIND_NEXT_TRACE_QNAME(next,handle)
#endif

NSPR_API(PRTraceHandle)
MPR_FindNextTraceQname(
    PRTraceHandle handle
);


/* -----------------------------------------------------------------------
** FUNCTION: MPR_FindNextTraceRname() -- Retrieive an RName handle
** iterator.
**
** DESCRIPTION:
** MPR_FindNextTraceRname() retreives the first or next trace
...
** MPR_SetTraceOption( PRLockTraceHandles ).
**
*/
#if defined (DEBUG) || defined (FORCE_NSPR_TRACE)
#define PR_FIND_NEXT_TRACE_RNAME(next,rhandle,qhandle)\
    (next) = MPR_FindNextTraceRname((rhandle),(qhandle))
#else
#define PR_FIND_NEXT_TRACE_RNAME(next,rhandle,qhandle)
#endif

NSPR_API(PRTraceHandle)
MPR_FindNextTraceRname(
    PRTraceHandle rhandle,
    PRTraceHandle qhandle
);

/* -----------------------------------------------------------------------
** FUNCTION: MPR_RecordTraceEntries() -- Write trace entries to external media
**
** DESCRIPTION:
** MPR_RecordTraceEntries() causes entries in the in-memory trace
** buffer to be written to external media.
**
** When MPR_RecordTraceEntries() is called from an application
** thread, the function appears to block until another thread
** calls MPR_SetTraceOption() with the PRTraceStopRecording
** option. This suggests that MPR_RecordTraceEntries() should be
...
**
** MPR_RecordTraceEntries() attempts to record the in-memory
...
**
** RESTRICTIONS:
** Only one thread can call MPR_RecordTraceEntries() within a
** process.
**
** On error, MPR_RecordTraceEntries() may return prematurely.
**
*/
#if defined (DEBUG) || defined (FORCE_NSPR_TRACE)
#define PR_RECORD_TRACE_ENTRIES()\
    MPR_RecordTraceEntries()
#else
#define PR_RECORD_TRACE_ENTRIES()
#endif
...
/* -----------------------------------------------------------------------
** FUNCTION: MPR_GetTraceEntries() -- Retreive trace entries from
** the Trace Facility
**
** DESCRIPTION:
** MPR_GetTraceEntries() retreives trace entries from the Trace
** Facility. Up to count trace entries are copied from the Trace
** Facility into buffer. Only those trace entries that have not
** been copied via a previous call to MPR_GetTraceEntries() are
...
** Facility since the last call to MPR_GetTraceEntries()
...
#define PR_GET_TRACE_ENTRIES(buffer,count,found)\
        MPR_GetTraceEntries((buffer),(count),(found))
#else
#define PR_GET_TRACE_ENTRIES(buffer,count,found)
#endif
