...
/*
** MPR_GetRandomNoise() -- Get random noise from the host platform
**
** Description:
** MPR_GetRandomNoise() provides, depending on platform, a random value.
...
**
** The intent of MPR_GetRandomNoise() is to provide a "seed" value for a
...
** generated by MPR_GetRandomNoise() is at best, extremely difficult to
...
**   requested. A return value of zero means that MPR_GetRandomNoise() is
**   not implemented on this platform, or there is no available noise
**   available to be returned at the time of the call.
**
** Restrictions:
**   Calls to MPR_GetRandomNoise() may use a lot of CPU on some platforms.
...
**   care is advised when using MPR_GetRandomNoise() frequently in your
**   application.
**
** History:
**   Parts of the model dependent implementation for MPR_GetRandomNoise()
**   were taken in whole or part from code previously in Netscape's NSS
**   component.
**
*/
NSPR_API(PRSize) MPR_GetRandomNoise(
    void    *buf,
    PRSize  size
);
