...
/* -----------------------------------------------------------------------
** FUNCTION: MPR_CreateOrderedLock() -- Create an Ordered Lock
**
** DESCRIPTION: MPR_CreateOrderedLock() creates an ordered lock.
**
** INPUTS:
...
#define PR_CREATE_ORDERED_LOCK(order,name)\
    MPR_CreateOrderedLock((order),(name))
#else
#define PR_CREATE_ORDERED_LOCK(order) MPR_NewLock()
#endif

NSPR_API(PROrderedLock *)
MPR_CreateOrderedLock(
    PRInt32 order,
    const char *name
);

/* -----------------------------------------------------------------------
** FUNCTION: MPR_DestroyOrderedLock() -- Destroy an Ordered Lock
**
** DESCRIPTION: MPR_DestroyOrderedLock() destroys the ordered lock
** referenced by lock.
**
...
*/
#if defined(DEBUG) || defined(FORCE_NSPR_ORDERED_LOCKS)
#define PR_DESTROY_ORDERED_LOCK(lock) MPR_DestroyOrderedLock((lock))
#else
#define PR_DESTROY_ORDERED_LOCK(lock) MPR_DestroyLock((lock))
#endif

NSPR_API(void)
MPR_DestroyOrderedLock(
    PROrderedLock *lock
);

/* -----------------------------------------------------------------------
** FUNCTION: MPR_LockOrderedLock() -- Lock an ordered lock
**
** DESCRIPTION: MPR_LockOrderedLock() locks the ordered lock
...
*/
#if defined(DEBUG) || defined(FORCE_NSPR_ORDERED_LOCKS)
#define PR_LOCK_ORDERED_LOCK(lock) MPR_LockOrderedLock((lock))
#else
#define PR_LOCK_ORDERED_LOCK(lock) MPR_Lock((lock))
#endif
...
/* -----------------------------------------------------------------------
** FUNCTION: MPR_UnlockOrderedLock() -- unlock and Ordered Lock
**
** DESCRIPTION: MPR_UnlockOrderedLock() unlocks the lock referenced
** by lock.
**
** INPUTS: lock: a pointer to a PROrderedLock
**
...
*/
#if defined(DEBUG) || defined(FORCE_NSPR_ORDERED_LOCKS)
#define PR_UNLOCK_ORDERED_LOCK(lock) MPR_UnlockOrderedLock((lock))
#else
#define PR_UNLOCK_ORDERED_LOCK(lock) MPR_Unlock((lock))
#endif

NSPR_API(PRStatus)
MPR_UnlockOrderedLock(
    PROrderedLock *lock
);
