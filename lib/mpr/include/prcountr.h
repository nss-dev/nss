...
/* -----------------------------------------------------------------------
** FUNCTION: MPR_CreateCounter() -- Create a counter
**
** DESCRIPTION: MPR_CreateCounter() creates a counter object and
...
#define PR_CREATE_COUNTER(handle,qName,rName,description)\
   (handle) = MPR_CreateCounter((qName),(rName),(description))
#else
#define PR_CREATE_COUNTER(handle,qName,rName,description)
#endif
...
/* -----------------------------------------------------------------------
** FUNCTION: MPR_DestroyCounter() -- Destroy a counter object.
**
** DESCRIPTION: MPR_DestroyCounter() removes a counter and
...
*/
#if defined(DEBUG) || defined(FORCE_NSPR_COUNTERS)
#define PR_DESTROY_COUNTER(handle) MPR_DestroyCounter((handle))
#else
...
/* -----------------------------------------------------------------------
** FUNCTION: MPR_GetCounterHandleFromName() -- Retreive a
** counter's handle give its name.
**
** DESCRIPTION: MPR_GetCounterHandleFromName() retreives a
...
#define PR_GET_COUNTER_HANDLE_FROM_NAME(handle,qName,rName)\
    (handle) = MPR_GetCounterHandleFromName((qName),(rName))
#else
#define PR_GET_COUNTER_HANDLE_FROM_NAME(handle,qName,rName)
#endif

NSPR_API(PRCounterHandle)
MPR_GetCounterHandleFromName(
    const char *qName,
    const char *rName
);

/* -----------------------------------------------------------------------
** FUNCTION: MPR_GetCounterNameFromHandle() -- Retreive a
** counter's name, given its handle.
**
** DESCRIPTION: MPR_GetCounterNameFromHandle() retreives a
** counter's name given its handle.
**
** INPUTS:
...
#define PR_GET_COUNTER_NAME_FROM_HANDLE(handle,qName,rName,description)\
    MPR_GetCounterNameFromHandle((handle),(qName),(rName),(description))
#else
#define PR_GET_COUNTER_NAME_FROM_HANDLE(handle,qName,rName,description )
#endif

NSPR_API(void)
MPR_GetCounterNameFromHandle(
    PRCounterHandle handle,
    const char **qName,
    const char **rName,
    const char **description
...
/* -----------------------------------------------------------------------
** FUNCTION: MPR_IncrementCounter() -- Add one to the referenced
** counter.
**
...
*/
#if defined(DEBUG) || defined(FORCE_NSPR_COUNTERS)
#define PR_INCREMENT_COUNTER(handle) MPR_IncrementCounter(handle)
#else
#define PR_INCREMENT_COUNTER(handle)
#endif

NSPR_API(void)
MPR_IncrementCounter(
    PRCounterHandle handle
);


/* -----------------------------------------------------------------------
** FUNCTION: MPR_DecrementCounter() -- Subtract one from the
** referenced counter
**
...
*/
#if defined(DEBUG) || defined(FORCE_NSPR_COUNTERS)
#define PR_DECREMENT_COUNTER(handle) MPR_DecrementCounter(handle)
#else
#define PR_DECREMENT_COUNTER(handle)
#endif

NSPR_API(void)
MPR_DecrementCounter(
    PRCounterHandle handle
);

/* -----------------------------------------------------------------------
** FUNCTION: MPR_AddToCounter() -- Add a value to a counter.
**
...
/* -----------------------------------------------------------------------
** FUNCTION: MPR_SubtractFromCounter() -- A value is subtracted
** from a counter.
**
** DESCRIPTION:
...
#define PR_SUBTRACT_FROM_COUNTER(handle,value)\
    MPR_SubtractFromCounter((handle),(value))
#else
#define PR_SUBTRACT_FROM_COUNTER(handle,value)
#endif

NSPR_API(void)
MPR_SubtractFromCounter(
    PRCounterHandle handle,
    PRUint32 value
);
...
/* -----------------------------------------------------------------------
** FUNCTION: MPR_GetCounter() -- Retreive the value of a counter
**
** DESCRIPTION:
...
/* -----------------------------------------------------------------------
** FUNCTION: MPR_SetCounter() -- Replace the content of counter
** with value.
**
...
*/
#if defined(DEBUG) || defined(FORCE_NSPR_COUNTERS)
#define PR_SET_COUNTER(handle,value) MPR_SetCounter((handle),(value))
#else
...
/* -----------------------------------------------------------------------
** FUNCTION: MPR_FindNextCounterQname() -- Retreive the next QName counter
** handle iterator
**
** DESCRIPTION:
** MPR_FindNextCounterQname() retreives the first or next Qname
...
**
** RESTRICTIONS:
**  A concurrent MPR_CreateCounter() or MPR_DestroyCounter() may
** cause unpredictable results.
**
** A PRCounterHandle returned from this function may only be used
** in another MPR_FindNextCounterQname() function call; other
** operations may cause unpredictable results.
**
*/
#if defined(DEBUG) || defined(FORCE_NSPR_COUNTERS)
#define PR_FIND_NEXT_COUNTER_QNAME(next,handle)\
    (next) = MPR_FindNextCounterQname((handle))
#else
#define PR_FIND_NEXT_COUNTER_QNAME(next,handle) NULL
#endif

NSPR_API(PRCounterHandle)
MPR_FindNextCounterQname(
    PRCounterHandle handle
);

/* -----------------------------------------------------------------------
** FUNCTION: MPR_FindNextCounterRname() -- Retreive the next RName counter
** handle iterator
**
** DESCRIPTION:
** MPR_FindNextCounterRname() retreives the first or next RNname
...
**
** A PRCounterHandle returned from this function may only be used
** in another MPR_FindNextCounterRname() function call; other
** operations may cause unpredictable results.
**
*/
#if defined(DEBUG) || defined(FORCE_NSPR_COUNTERS)
#define PR_FIND_NEXT_COUNTER_RNAME(next,rhandle,qhandle)\
    (next) = MPR_FindNextCounterRname((rhandle),(qhandle))
#else
#define PR_FIND_NEXT_COUNTER_RNAME(next,rhandle,qhandle)
#endif

NSPR_API(PRCounterHandle)
MPR_FindNextCounterRname(
