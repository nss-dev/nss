...
** MPR_Yield() is MPR_Sleep() with an argument of PR_INTERVAL_NO_WAIT.
*/
NSPR_API(PRStatus) MPR_Yield(void);
...
** zero is returned.  On return, MPR_Select replaces the given descriptor sets with
...
** MPR_FD_ZERO(&fdset) initializes a descriptor set fdset to the null set.
** MPR_FD_SET(fd, &fdset) includes a particular file descriptor fd in fdset.
** MPR_FD_CLR(fd, &fdset) removes a file descriptor fd from fdset.
** MPR_FD_ISSET(fd, &fdset) is nonzero if file descriptor fd is a member of
** fdset, zero otherwise.
**
** MPR_FD_NSET(osfd, &fdset) includes a particular native file descriptor osfd
** in fdset.
** MPR_FD_NCLR(osfd, &fdset) removes a native file descriptor osfd from fdset.
** MPR_FD_NISSET(osfd, &fdset) is nonzero if native file descriptor osfd is a member of
** fdset, zero otherwise.
*/

NSPR_API(void)        MPR_FD_ZERO(PR_fd_set *set);
NSPR_API(void)        MPR_FD_SET(PRFileDesc *fd, PR_fd_set *set);
NSPR_API(void)        MPR_FD_CLR(PRFileDesc *fd, PR_fd_set *set);
NSPR_API(PRInt32)     MPR_FD_ISSET(PRFileDesc *fd, PR_fd_set *set);
NSPR_API(void)        MPR_FD_NSET(PROsfd osfd, PR_fd_set *set);
NSPR_API(void)        MPR_FD_NCLR(PROsfd osfd, PR_fd_set *set);
NSPR_API(PRInt32)     MPR_FD_NISSET(PROsfd osfd, PR_fd_set *set);
...
NSPR_API(PRInt32) MPR_GetSysfdTableMax(void);

NSPR_API(PRInt32) MPR_SetSysfdTableSize(PRIntn table_size);

#ifndef NO_NSPR_10_SUPPORT
#include <sys/stat.h>

NSPR_API(PRInt32) MPR_Stat(const char *path, struct stat *buf);
