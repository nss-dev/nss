...
/*
 * The MPR_Now routine returns the current time relative to the
...
/* Reverse operation of MPR_ExplodeTime */
NSPR_API(PRTime)
...
NSPR_API(void) MPR_NormalizeTime(PRExplodedTime* exploded, PRTimeParamFn params);
...
NSPR_API(PRTimeParameters) MPR_LocalTimeParameters(const PRExplodedTime* gmt);

/* Time parameters that represent Greenwich Mean Time */
NSPR_API(PRTimeParameters) MPR_GMTParameters(const PRExplodedTime* gmt);
...
/*
 * This uses MPR_ParseTimeStringToExplodedTime to parse
 * a time/date string and MPR_ImplodeTime to transform it into
