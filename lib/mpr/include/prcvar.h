...
*/
NSPR_API(PRCondVar*) MPR_NewCondVar(PRLock *lock);
...
**
*/
NSPR_API(void) MPR_DestroyCondVar(PRCondVar *cvar);
...
** with the condition variable or the thread was interrupted (MPR_Interrupt()).
** The particular reason can be extracted with MPR_GetError().
*/
NSPR_API(PRStatus) MPR_WaitCondVar(PRCondVar *cvar, PRIntervalTime timeout);
...
*/
NSPR_API(PRStatus) MPR_NotifyCondVar(PRCondVar *cvar);
...
** with the condition variable.
*/
NSPR_API(PRStatus) MPR_NotifyAllCondVar(PRCondVar *cvar);
