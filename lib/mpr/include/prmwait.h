...
**                      can be retrieved by calling MPR_GetError().
...
**                      calling MPR_CancelWaitFileDesc() or destroying the
**                      entire wait group (MPR_DestroyWaitGroup()).
*/
...
**      function MPR_WaitRecvReady().
**
**  INPUTS
**      group       A reference to a PRWaitGroup or NULL. Wait groups are
**                  created by calling MPR_CreateWaitGroup() and are used
...
**                  of the failure are avaiable via MPR_GetError().
**
**  ERRORS
...
*/
NSPR_API(PRStatus) MPR_AddWaitFileDesc(PRWaitGroup *group, PRRecvWait *desc);

/*
** FUNCTION:    MPR_WaitRecvReady
** DESCRIPTION:
**      MPR_WaitRecvReady will block the calling thread until one of the
**      file descriptors that have been added via MPR_AddWaitFileDesc is
...
**                  outcome field. Details can be retrieved from MPR_GetError().
**
**  ERRORS
**      PR_INVALID_ARGUMENT_ERROR
...
**                  The group is being destroyed.
*/
NSPR_API(PRRecvWait*) MPR_WaitRecvReady(PRWaitGroup *group);

/*
** FUNCTION:    MPR_CancelWaitFileDesc
** DESCRIPTION:
**      MPR_CancelWaitFileDesc is provided as a means for cancelling operations
**      on objects previously submitted by use of MPR_AddWaitFileDesc(). If
**      the runtime knows of the object, it will be marked as having failed
**      because it was interrupted (similar to MPR_Interrupt()). The first
...
**                  The group is being destroyed.
*/
NSPR_API(PRStatus) MPR_CancelWaitFileDesc(PRWaitGroup *group, PRRecvWait *desc);

/*
** FUNCTION:    MPR_CancelWaitGroup
** DESCRIPTION:
**      MPR_CancelWaitGroup is provided as a means for cancelling operations
**      on objects previously submitted by use of MPR_AddWaitFileDesc(). Each
**      successive call will return a pointer to a PRRecvWait object that
**      was previously registered via MPR_AddWaitFileDesc(). If no wait
...
**      to reclaim all the wait objects prior to calling MPR_DestroyWaitGroup().
**
**  INPUTS
**      group       The wait group under which the wait receive object was
**                  added.
...
**                  for the NULL may be retrieved by calling MPR_GetError().
**
**  ERRORS
**      PR_INVALID_ARGUMENT_ERROR
**      PR_GROUP_EMPTY_ERROR
*/
NSPR_API(PRRecvWait*) MPR_CancelWaitGroup(PRWaitGroup *group);

/*
** FUNCTION:    MPR_CreateWaitGroup
** DESCRIPTION:
...
**                  by calling MPR_DestroyWaitGroup().
**
**  ERRORS
**      PR_OUT_OF_MEMORY_ERROR
*/
NSPR_API(PRWaitGroup*) MPR_CreateWaitGroup(PRInt32 size);

/*
** FUNCTION:    MPR_DestroyWaitGroup
** DESCRIPTION:
**      Undo the effects of MPR_CreateWaitGroup(). Any receive wait operations
...
*/
NSPR_API(PRStatus) MPR_DestroyWaitGroup(PRWaitGroup *group);

/*
** FUNCTION:    MPR_CreateMWaitEnumerator
** DESCRIPTION:
**      The MPR_CreateMWaitEnumerator() function returns a reference to an
...
**      PR_OUT_OF_MEMORY_ERROR
*/
NSPR_API(PRMWaitEnumerator*) MPR_CreateMWaitEnumerator(PRWaitGroup *group);

/*
** FUNCTION:    MPR_DestroyMWaitEnumerator
** DESCRIPTION:
**      Destroys the object created by MPR_CreateMWaitEnumerator(). The reference
...
*/
NSPR_API(PRStatus) MPR_DestroyMWaitEnumerator(PRMWaitEnumerator* enumerator);

/*
** FUNCTION:    MPR_EnumerateWaitGroup
** DESCRIPTION:
**      MPR_EnumerateWaitGroup is a thread safe enumerator over a wait group.
...
*/
NSPR_API(PRRecvWait*) MPR_EnumerateWaitGroup(
