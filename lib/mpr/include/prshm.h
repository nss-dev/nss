...
**   given in the MPR_OpenSharedMemory() call, a protocol for using the
**   shared memory API is required to ensure desired behavior. Failing
**   to follow the protocol may yield unpredictable results.
**
**   MPR_OpenSharedMemory() will create the shared memory segment, if it
**   does not already exist, or open a connection that the existing
**   shared memory segment if it already exists.
**
**   MPR_AttachSharedMemory() should be called following
**   MPR_OpenSharedMemory() to map the memory segment to an address in
**   the application's address space.
**
**   MPR_AttachSharedMemory() may be called to re-map a shared memory
**   segment after detaching the same PRSharedMemory object. Be
**   sure to detach it when done.
**
**   MPR_DetachSharedMemory() should be called to un-map the shared
**   memory segment from the application's address space.
**
**   MPR_CloseSharedMemory() should be called when no further use of the
**   PRSharedMemory object is required within a process. Following a
**   call to  MPR_CloseSharedMemory() the PRSharedMemory object is
**   invalid and cannot be reused.
**
**   MPR_DeleteSharedMemory() should be called before process
**   termination. After calling MPR_DeleteSharedMemory() any further use
**   of the shared memory associated with the name may cause
**   unpredictable results.
**
** Files:
**   The name passed to MPR_OpenSharedMemory() should be a valid filename
**   for a unix platform. MPR_OpenSharedMemory() creates file using the
...
**   termination.
**
**   All names provided to MPR_OpenSharedMemory() should be valid
...
/*
** FUNCTION: MPR_OpenSharedMemory()
**
** DESCRIPTION:
**   MPR_OpenSharedMemory() creates a new shared-memory segment or
...
**   mode -- Same as is passed to MPR_Open()
**
** OUTPUTS:
...
**   retrieved via MPR_GetError() and MPR_GetOSError();
**
*/
NSPR_API( PRSharedMemory * )
MPR_OpenSharedMemory(
    const char *name,
    PRSize      size,
    PRIntn      flags,
    PRIntn      mode
...
/*
** FUNCTION: MPR_AttachSharedMemory()
**
** DESCRIPTION:
** MPR_AttachSharedMemory() maps the shared-memory described by
** shm to the current process.
**
** INPUTS:
**   shm -- The handle returned from MPR_OpenSharedMemory().
...
*/
NSPR_API( void * )
MPR_AttachSharedMemory(
    PRSharedMemory *shm,
    PRIntn  flags
);
/* Define values for MPR_AttachSharedMemory(...,flags) */
#define PR_SHM_READONLY 0x01

/*
** FUNCTION: MPR_DetachSharedMemory()
**
** DESCRIPTION:
**   MPR_DetachSharedMemory() detaches the shared-memory described
**   by shm.
**
** INPUTS:
**   shm -- The handle returned from MPR_OpenSharedMemory().
...
**   MPR_AttachSharedMemory() is unmapped.
**
** RETURNS: PRStatus
**
*/
NSPR_API( PRStatus )
MPR_DetachSharedMemory(
    PRSharedMemory *shm,
    void  *addr
);

/*
** FUNCTION: MPR_CloseSharedMemory()
**
** DESCRIPTION:
**   MPR_CloseSharedMemory() closes the shared-memory described by
**   shm.
**
** INPUTS:
**   shm -- The handle returned from MPR_OpenSharedMemory().
...
*/
NSPR_API( PRStatus )
MPR_CloseSharedMemory(
    PRSharedMemory *shm
);

/*
** FUNCTION: MPR_DeleteSharedMemory()
**
** DESCRIPTION:
