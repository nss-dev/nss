 ...
 *********************************************************************
 */
NSPR_API(PRStatus) MPR_StringToNetAddr(
    const char *string, PRNetAddr *addr);

NSPR_API(PRStatus) MPR_NetAddrToString(
...
**                      for the failure can be retrieved by MPR_GetError().
***********************************************************************/
NSPR_API(PRStatus) MPR_GetHostByName(
    const char *hostname, char *buf, PRIntn bufsize, PRHostEnt *hostentry);

/***********************************************************************
** FUNCTION:
** DESCRIPTION: MPR_GetIPNodeByName()
...
NSPR_API(PRStatus) MPR_GetIPNodeByName(
    const char *hostname,
    PRUint16 af,
    PRIntn flags,
    char *buf,
...
**                      for the failure can be retrieved by MPR_GetError().
***********************************************************************/
NSPR_API(PRStatus) MPR_GetHostByAddr(
    const PRNetAddr *hostaddr, char *buf, PRIntn bufsize, PRHostEnt *hostentry);

/***********************************************************************
** FUNCTION:    MPR_EnumerateHostEnt()
** DESCRIPTION:
**  A stateless enumerator over a PRHostEnt structure acquired from
**  MPR_GetHostByName() MPR_GetHostByAddr() to evaluate the possible
**  network addresses.
**
** INPUTS:
...
**                      previously returned by MPR_GetHostByName() or
**                      MPR_GetHostByAddr().
**
...
**                      retrieved by calling MPR_GetError().
***********************************************************************/
NSPR_API(PRIntn) MPR_EnumerateHostEnt(
    PRIntn enumIndex, const PRHostEnt *hostEnt, PRUint16 port, PRNetAddr *address);

/***********************************************************************
** FUNCTION: MPR_InitializeNetAddr(),
** DESCRIPTION:
...
NSPR_API(PRStatus) MPR_InitializeNetAddr(
    PRNetAddrValue val, PRUint16 port, PRNetAddr *addr);

/***********************************************************************
** FUNCTION: MPR_SetNetAddr(),
** DESCRIPTION:
**  Set the fields of a PRNetAddr, assigning well known values as
**  appropriate. This function is similar to MPR_InitializeNetAddr
...
**                      MPR_GetError();
***********************************************************************/
NSPR_API(PRStatus) MPR_SetNetAddr(
    PRNetAddrValue val, PRUint16 af, PRUint16 port, PRNetAddr *addr);

/***********************************************************************
** FUNCTION:
** DESCRIPTION: MPR_IsNetAddrType()
...
***********************************************************************/
NSPR_API(PRBool) MPR_IsNetAddrType(const PRNetAddr *addr, PRNetAddrValue val);

/***********************************************************************
** FUNCTION:
** DESCRIPTION: MPR_ConvertIPv4AddrToIPv6()
...
**
***********************************************************************/
NSPR_API(void) MPR_ConvertIPv4AddrToIPv6(PRUint32 v4addr, PRIPv6Addr *v6addr);
...
/***********************************************************************
** FUNCTION:
** DESCRIPTION: MPR_GetProtoByName()
...
**                      for the failure can be retrieved by MPR_GetError().
***********************************************************************/
NSPR_API(PRStatus) MPR_GetProtoByNumber(
    PRInt32 protocolnumber, char* buffer, PRInt32 bufsize, PRProtoEnt* result);

/***********************************************************************
** FUNCTION:
** DESCRIPTION: MPR_GetAddrInfoByName()
...
**                      of the host lookup. Use MPR_EnumerateAddrInfo to
**                      inspect the PRNetAddr values stored in this object.
**                      When no longer needed, this handle must be destroyed
**                      with a call to MPR_FreeAddrInfo.  If a lookup error
**                      occurs, then NULL will be returned.
***********************************************************************/
typedef struct PRAddrInfo PRAddrInfo;

NSPR_API(PRAddrInfo*) MPR_GetAddrInfoByName(
    const char *hostname, PRUint16 af, PRIntn flags);

/***********************************************************************
** FUNCTION:
** DESCRIPTION: MPR_FreeAddrInfo()
**  Destroy the PRAddrInfo handle allocated by MPR_GetAddrInfoByName().
**
** INPUTS:
**  PRAddrInfo *addrInfo
**                      The handle resulting from a successful call to
**                      MPR_GetAddrInfoByName().
** RETURN:
**  void
***********************************************************************/
NSPR_API(void) MPR_FreeAddrInfo(PRAddrInfo *addrInfo);

/***********************************************************************
** FUNCTION:
** DESCRIPTION: MPR_EnumerateAddrInfo()
**  A stateless enumerator over a PRAddrInfo handle acquired from
**  MPR_GetAddrInfoByName() to inspect the possible network addresses.
**
** INPUTS:
...
***********************************************************************/
NSPR_API(void *) MPR_EnumerateAddrInfo(
    void *enumPtr, const PRAddrInfo *addrInfo, PRUint16 port, PRNetAddr *result);

NSPR_API(PRStatus) MPR_GetPrefLoopbackAddrInfo(PRNetAddr *result,
                                              PRUint16 port);

/***********************************************************************
** FUNCTION:
** DESCRIPTION: MPR_GetCanonNameFromAddrInfo()
...
**                      by a call to MPR_FreeAddrInfo().
***********************************************************************/
NSPR_API(const char *) MPR_GetCanonNameFromAddrInfo(
    const PRAddrInfo *addrInfo);

/***********************************************************************
** FUNCTIONS: MPR_ntohs, MPR_ntohl, MPR_ntohll, MPR_htons, MPR_htonl, MPR_htonll
**
** DESCRIPTION: API entries for the common byte ordering routines.
**
**      MPR_ntohs        16 bit conversion from network to host
**      MPR_ntohl        32 bit conversion from network to host
**      MPR_ntohll       64 bit conversion from network to host
**      MPR_htons        16 bit conversion from host to network
**      MPR_htonl        32 bit conversion from host to network
**      PR_ntonll       64 bit conversion from host to network
**
***********************************************************************/
NSPR_API(PRUint16) MPR_ntohs(PRUint16);
NSPR_API(PRUint32) MPR_ntohl(PRUint32);
NSPR_API(PRUint64) MPR_ntohll(PRUint64);
NSPR_API(PRUint16) MPR_htons(PRUint16);
NSPR_API(PRUint32) MPR_htonl(PRUint32);
NSPR_API(PRUint64) MPR_htonll(PRUint64);
