...
** PRLogModuleInfo structure by calling MPR_NewLogModule(). After
...
**
** Where:
**  <moduleName> is the name passed to MPR_NewLogModule().
...
** main() {
**    PRIntn one = 1;
**    PRLogModuleInfo * myLm = MPR_NewLogModule("gizmo");
...
*/
NSPR_API(PRLogModuleInfo*) MPR_NewLogModule(const char *name);

/*
** Set the file to use for logging. Returns PR_FALSE if the file cannot
** be created
*/
NSPR_API(PRBool) MPR_SetLogFile(const char *name);

/*
** Set the size of the logging buffer. If "buffer_size" is zero then the
** logging becomes "synchronous" (or unbuffered).
*/
NSPR_API(void) MPR_SetLogBuffering(PRIntn buffer_size);

/*
** Print a string to the log. "fmt" is a MPR_snprintf format type. All
...
** is not provided.
*/
NSPR_API(void) MPR_LogPrint(const char *fmt, ...);

/*
** Flush the log to its file.
*/
NSPR_API(void) MPR_LogFlush(void);

NSPR_API(void) MPR_Assert(const char *s, const char *file, PRIntn ln)
PR_PRETEND_NORETURN;
...
#define PR_LOG_DEFINE   MPR_NewLogModule
#else
...
#define PR_ASSERT(_expr) \
    ((_expr)?((void)0):MPR_Assert(# _expr,__FILE__,__LINE__))

#define PR_ASSERT_ARG(_expr) PR_ASSERT(_expr)

#define PR_NOT_REACHED(_reasonStr) \
    MPR_Assert(_reasonStr,__FILE__,__LINE__)
