...
** memory is done via helper-functions with MPR_CreateProcess(). In the
...
** Server:
**
**   fm = MPR_OpenAnonFileMap(dirName, size, FilemapProt);
**   addr = MPR_MemMap(fm);
**   attr = MPR_NewProcessAttr();
**   MPR_ProcessAttrSetInheritableFileMap( attr, fm, shmname );
**   MPR_CreateProcess(Client);
...
**
** Client:
**   ... started by server via MPR_CreateProcess()
**   fm = MPR_GetInheritedFileMap( shmname );
**   addr = MPR_MemMap(fm);
**   ... yadda ...
**   MPR_MemUnmap(addr);
**   MPR_CloseFileMap(fm);
...
**
**   fm = MPR_OpenAnonFileMap(dirName, size, FilemapProt);
**   fmstring = MPR_ExportFileMapAsString( fm );
**   addr = MPR_MemMap(fm);
...
**   fm = MPR_ImportFileMapFromString( fmstring )
**   addr = MPR_MemMap(fm);
**   ... yadda ...
**   MPR_MemUnmap(addr);
**   MPR_CloseFileMap(fm);
...
**
** Description:
** MPR_OpenAnonFileMap() creates an anonymous shared memory. If the
** shared memory already exists, a handle is returned to that shared
** memory object.
**
** On Unix platforms, MPR_OpenAnonFileMap() uses 'dirName' as a
...
**
** Description:
** MPR_ProcessAttrSetInheritableFileMap() connects the PRFileMap to
** PRProcessAttr with shmname. A subsequent call to MPR_CreateProcess()
** makes the PRFileMap importable by the child process.
**
** Inputs:
**   attr -- PRProcessAttr, used to pass data to MPR_CreateProcess()
...
*/
NSPR_API(PRStatus)
MPR_ProcessAttrSetInheritableFileMap(
    PRProcessAttr   *attr,
    PRFileMap       *fm,
    const char      *shmname
);
...
**
** Description:
** MPR_GetInheritedFileMap() retrieves a PRFileMap object exported from
** its parent process via MPR_CreateProcess().
**
** Inputs:
**    shmname -- The name provided to MPR_ProcessAttrSetInheritableFileMap()
**
** Outputs:
**   PRFileMap *
**
...
** previously created with MPR_OpenAnonFileMap().
**
** Inputs:
...
/*
** MPR_ImportFileMapFromString() -- Creates a PRFileMap from the identifying string
**
** Description:
** MPR_ImportFileMapFromString() creates a PRFileMap object from a
** string previously created by MPR_ExportFileMapAsString().
**
** Inputs:
**   fmstring -- string created by MPR_ExportFileMapAsString()
**
** Returns:
**   PRFileMap pointer or NULL.
**
