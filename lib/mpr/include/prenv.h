...
/*
** MPR_GetEnv() -- Retrieve value of environment variable
**
** Description:
** MPR_GetEnv() is modeled on Unix getenv().
**
**
** Inputs:
...
**
**   MPR_GetEnv(), MPR_SetEnv() provide a consistent behavior
...
**   mean the same as a return of NULL from MPR_GetEnv().
**
**   A call to MPR_SetEnv() where the parameter is of the form
**   "name" will return PR_FAILURE; the environment remains
**   unchanged. A call to MPR_SetEnv() where the parameter is
...
**   null-string from MPR_GetEnv(), use the following code
**   fragment:
**
**      char *val = MPR_GetEnv("foo");
**      if ((NULL == val) || ('\0' == *val)) {
**          ... interpret this as un-set ...
**      }
**
**   The caller must ensure that the string passed
**   to MPR_SetEnv() is persistent. That is: The string should
**   not be on the stack, where it can be overwritten
**   on return from the function calling MPR_SetEnv().
**   Similarly, the string passed to MPR_SetEnv() must not be
...
**
*/
NSPR_API(char*) MPR_GetEnv(const char *var);

/*
** MPR_GetEnvSecure() -- get a security-sensitive environment variable
**
** Description:
**
** MPR_GetEnvSecure() is similar to MPR_GetEnv(), but it returns NULL if
...
*/
NSPR_API(char*) MPR_GetEnvSecure(const char *var);

/*
** MPR_SetEnv() -- set, unset or change an environment variable
**
** Description:
** MPR_SetEnv() is modeled on the Unix putenv() function.
**
** Inputs:
...
**   MPR_GetEnv() in this header file.
**
**
*/
NSPR_API(PRStatus) MPR_SetEnv(const char *string);

/*
** MPR_DuplicateEnvironment() -- Obtain a copy of the environment.
**
** Description:
** MPR_DuplicateEnvironment() copies the environment so that it can be
...
** multithreaded process calls fork(), and MPR_SetEnv does both.
**
** Inputs:
**   none
**
...
**   the strings are allocated with MPR_Malloc(), and it is the
...
**
** Restrictions:
**   Similarly to MPR_GetEnv(), this function may not interoperate as
**   expected with the operating system's native environment accessors.
*/
NSPR_API(char **) MPR_DuplicateEnvironment(void);
