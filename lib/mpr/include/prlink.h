...
*/
NSPR_API(PRStatus) MPR_SetLibraryPath(const char *path);

/*
** Return a character string which contains the path used to search for
** dynamically loadable libraries.
**
** The returned value is basically a copy of a MPR_SetLibraryPath().
** The storage is allocated by the runtime and becomes the responsibilty
** of the caller.
*/
NSPR_API(char*) MPR_GetLibraryPath(void);
...
** and prepared for MPR_LoadLibrary. The result must be free'd when the
** caller is done with it.
**
** The storage for the result is allocated by the runtime and becomes the
** responsibility of the caller.
*/
NSPR_API(char*) MPR_GetLibraryName(const char *dir, const char *lib);

/*
**
** Free the memory allocated, for the caller, by MPR_GetLibraryName
*/
NSPR_API(void) MPR_FreeLibraryName(char *mem);
...
** returned by MPR_GetLibraryName.  If the library is already loaded,
...
*/
NSPR_API(PRLibrary*) MPR_LoadLibrary(const char *name);
...
** as the 'flags' argument to MPR_LoadLibraryWithFlags.
...
*/
NSPR_API(PRStatus) MPR_UnloadLibrary(PRLibrary *lib);
...
** use MPR_LoadStaticLibrary to register symbols in the main program.
**
** This function does not modify the reference count of the library.
*/
NSPR_API(void*) MPR_FindSymbol(PRLibrary *lib, const char *name);

/*
** Similar to MPR_FindSymbol, except that the return value is a pointer to
...
NSPR_API(PRFuncPtr) MPR_FindFunctionSymbol(PRLibrary *lib, const char *name);
...
** This increments the reference count of the library.
*/
NSPR_API(void*) MPR_FindSymbolAndLibrary(const char *name,
                                        PRLibrary* *lib);

/*
** Similar to MPR_FindSymbolAndLibrary, except that the return value is
...
** This increments the reference count of the library.
*/
NSPR_API(PRFuncPtr) MPR_FindFunctionSymbolAndLibrary(const char *name,
        PRLibrary* *lib);
...
** available to MPR_FindSymbol. If "name" is null then the symbols will be
...
** This increments the reference count of the library.
*/
NSPR_API(PRLibrary*) MPR_LoadStaticLibrary(
...
**
** The caller is responsible for freeing the result with MPR_Free.
*/
NSPR_API(char *) MPR_GetLibraryFilePathname(const char *name, PRFuncPtr addr);
