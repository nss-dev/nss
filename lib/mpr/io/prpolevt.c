...
static PRStatus PR_CALLBACK _pr_PolEvtInit(void) {
  _pr_polevt_id = MPR_GetUniqueIdentity("NSPR pollable events");
...
PR_IMPLEMENT(PRFileDesc*) MPR_NewPollableEvent(void) {
  PRFileDesc* event;
  ...
  if (MPR_CallOnce(&_pr_polevt_once_control, _pr_PolEvtInit) == PR_FAILURE) {
    return NULL;
  }

  event = MPR_CreateIOLayerStub(_pr_polevt_id, &_pr_polevt_methods);
  if (NULL == event) {
    goto errorExit;
  }
  ...
  if (MPR_CreatePipe(&fd[0], &fd[1]) == PR_FAILURE) {
    fd[0] = fd[1] = NULL;
    goto errorExit;
  }
#else
  if (MPR_NewTCPSocketPair(fd) == PR_FAILURE) {
    fd[0] = fd[1] = NULL;
    goto errorExit;
  }
  /*
  ...
  rv = MPR_SetSocketOption(fd[1], &socket_opt);
  PR_ASSERT(PR_SUCCESS == rv);
#endif

  event->secret->writeEnd = fd[1];
  if (MPR_PushIOLayer(fd[0], PR_TOP_IO_LAYER, event) == PR_FAILURE) {
    goto errorExit;
  }
...
static PRStatus PR_CALLBACK _pr_PolEvtClose(PRFileDesc* fd) {
  ...
  event = MPR_PopIOLayer(fd, PR_TOP_IO_LAYER);
  PR_ASSERT(NULL == event->higher && NULL == event->lower);
  MPR_Close(fd);
  MPR_Close(event->secret->writeEnd);
  PR_DELETE(event->secret);
  event->dtor(event);
  return PR_SUCCESS;
}

PR_IMPLEMENT(PRStatus) MPR_DestroyPollableEvent(PRFileDesc* event) {
  return MPR_Close(event);
}

static const char magicChar = '\x38';

PR_IMPLEMENT(PRStatus) MPR_SetPollableEvent(PRFileDesc* event) {
  if (MPR_Write(event->secret->writeEnd, &magicChar, 1) != 1) {
    return PR_FAILURE;
  }
  return PR_SUCCESS;
}

PR_IMPLEMENT(PRStatus) MPR_WaitForPollableEvent(PRFileDesc* event) {
  char buf[1024];
  PRInt32 nBytes;
#ifdef DEBUG
  PRIntn i;
  ...
  nBytes = MPR_Read(event->lower, buf, sizeof(buf));
  if (nBytes == -1) {
    return PR_FAILURE;
  }
