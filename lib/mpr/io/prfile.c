...
static PRInt32 PR_CALLBACK FileWrite(PRFileDesc* fd, const void* buf,
  ...
  if (fd->secret->appendMode) {
    if (MPR_Seek64(fd, 0, PR_SEEK_END) == -1) {
      return -1;
    }
  } /* if (fd->secret->appendMode...) */
...
PR_IMPLEMENT(PRFileDesc*) MPR_Open(const char* name, PRIntn flags, PRIntn mode) {
  ...
  if (osfd != -1) {
    fd = MPR_AllocFileDesc(osfd, &_pr_fileMethods);
    if (!fd) {
...
PR_IMPLEMENT(PRInt32) MPR_SetSysfdTableSize(int table_size) {
#if defined(XP_UNIX) && !defined(AIX) && !defined(QNX)
  struct rlimit rlim;
...
*/
PR_IMPLEMENT(PRFileDesc*) MPR_ImportFile(PROsfd osfd) {
  PRFileDesc* fd = NULL;
...
*/
PR_IMPLEMENT(PRFileDesc*) MPR_ImportPipe(PROsfd osfd) {
  PRFileDesc* fd = NULL;
  ...
  fd = MPR_AllocFileDesc(osfd, &_pr_pipeMethods);
  if (!fd) {
    (void)_PR_MD_CLOSE_FILE(osfd);
  } else {
    _PR_MD_INIT_FD_INHERITABLE(fd, PR_TRUE);
 ...
 *
 * PR_IMPLEMENT(PRStatus) MPR_Stat(const char *name, struct stat *buf)
 * {
 *     PRInt32 rv;
 *
...
MPR_CreatePipe(PRFileDesc** readPipe, PRFileDesc** writePipe) {
  ...
  if (CreatePipe(&readEnd, &writeEnd, &pipeAttributes, 0) == 0) {
    MPR_SetError(PR_UNKNOWN_ERROR, GetLastError());
    return PR_FAILURE;
  }
  *readPipe = MPR_AllocFileDesc((PROsfd)readEnd, &_pr_pipeMethods);
  if (NULL == *readPipe) {
    CloseHandle(readEnd);
    CloseHandle(writeEnd);
    return PR_FAILURE;
  }
  *writePipe = MPR_AllocFileDesc((PROsfd)writeEnd, &_pr_pipeMethods);
  if (NULL == *writePipe) {
    MPR_Close(*readPipe);
    CloseHandle(writeEnd);
    return PR_FAILURE;
...
#  endif
  ...
  *readPipe = MPR_AllocFileDesc(pipefd[0], &_pr_pipeMethods);
  if (NULL == *readPipe) {
    close(pipefd[0]);
    close(pipefd[1]);
    return PR_FAILURE;
  }
  *writePipe = MPR_AllocFileDesc(pipefd[1], &_pr_pipeMethods);
  if (NULL == *writePipe) {
    MPR_Close(*readPipe);
    close(pipefd[1]);
    return PR_FAILURE;
