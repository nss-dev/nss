...
PR_IMPLEMENT(PRDescType) MPR_GetDescType(PRFileDesc* file) {
  return file->methods->file_type;
}

PR_IMPLEMENT(PRStatus) MPR_Close(PRFileDesc* fd) {
  return (fd->methods->close)(fd);
}

PR_IMPLEMENT(PRInt32) MPR_Read(PRFileDesc* fd, void* buf, PRInt32 amount) {
  return ((fd->methods->read)(fd, buf, amount));
}

PR_IMPLEMENT(PRInt32)
MPR_Write(PRFileDesc* fd, const void* buf, PRInt32 amount) {
  return ((fd->methods->write)(fd, buf, amount));
}

PR_IMPLEMENT(PRInt32)
MPR_Seek(PRFileDesc* fd, PRInt32 offset, PRSeekWhence whence) {
  return ((fd->methods->seek)(fd, offset, whence));
}

PR_IMPLEMENT(PRInt64)
MPR_Seek64(PRFileDesc* fd, PRInt64 offset, PRSeekWhence whence) {
  return ((fd->methods->seek64)(fd, offset, whence));
}

PR_IMPLEMENT(PRInt32) MPR_Available(PRFileDesc* fd) {
  return ((fd->methods->available)(fd));
}

PR_IMPLEMENT(PRInt64) MPR_Available64(PRFileDesc* fd) {
  return ((fd->methods->available64)(fd));
}

PR_IMPLEMENT(PRStatus) MPR_GetOpenFileInfo(PRFileDesc* fd, PRFileInfo* info) {
  return ((fd->methods->fileInfo)(fd, info));
}

PR_IMPLEMENT(PRStatus)
MPR_GetOpenFileInfo64(PRFileDesc* fd, PRFileInfo64* info) {
  return ((fd->methods->fileInfo64)(fd, info));
}

PR_IMPLEMENT(PRStatus) MPR_Sync(PRFileDesc* fd) {
  return ((fd->methods->fsync)(fd));
}

PR_IMPLEMENT(PRStatus)
MPR_Connect(PRFileDesc* fd, const PRNetAddr* addr, PRIntervalTime timeout) {
  return ((fd->methods->connect)(fd, addr, timeout));
}

PR_IMPLEMENT(PRStatus) MPR_ConnectContinue(PRFileDesc* fd, PRInt16 out_flags) {
  return ((fd->methods->connectcontinue)(fd, out_flags));
}

PR_IMPLEMENT(PRFileDesc*)
MPR_Accept(PRFileDesc* fd, PRNetAddr* addr, PRIntervalTime timeout) {
  return ((fd->methods->accept)(fd, addr, timeout));
}

PR_IMPLEMENT(PRStatus) MPR_Bind(PRFileDesc* fd, const PRNetAddr* addr) {
  return ((fd->methods->bind)(fd, addr));
}

PR_IMPLEMENT(PRStatus) MPR_Shutdown(PRFileDesc* fd, PRShutdownHow how) {
  return ((fd->methods->shutdown)(fd, how));
}

PR_IMPLEMENT(PRStatus) MPR_Listen(PRFileDesc* fd, PRIntn backlog) {
  return ((fd->methods->listen)(fd, backlog));
}

PR_IMPLEMENT(PRInt32)
MPR_Recv(PRFileDesc* fd, void* buf, PRInt32 amount, PRIntn flags,
        PRIntervalTime timeout) {
  return ((fd->methods->recv)(fd, buf, amount, flags, timeout));
}

PR_IMPLEMENT(PRInt32)
MPR_Send(PRFileDesc* fd, const void* buf, PRInt32 amount, PRIntn flags,
        PRIntervalTime timeout) {
  return ((fd->methods->send)(fd, buf, amount, flags, timeout));
}

PR_IMPLEMENT(PRInt32)
MPR_Writev(PRFileDesc* fd, const PRIOVec* iov, PRInt32 iov_size,
          PRIntervalTime timeout) {
...
PR_IMPLEMENT(PRInt32)
MPR_RecvFrom(PRFileDesc* fd, void* buf, PRInt32 amount, PRIntn flags,
            PRNetAddr* addr, PRIntervalTime timeout) {
  return ((fd->methods->recvfrom)(fd, buf, amount, flags, addr, timeout));
}

PR_IMPLEMENT(PRInt32)
MPR_SendTo(PRFileDesc* fd, const void* buf, PRInt32 amount, PRIntn flags,
          const PRNetAddr* addr, PRIntervalTime timeout) {
  return ((fd->methods->sendto)(fd, buf, amount, flags, addr, timeout));
}

PR_IMPLEMENT(PRInt32)
MPR_TransmitFile(PRFileDesc* sd, PRFileDesc* fd, const void* hdr, PRInt32 hlen,
                PRTransmitFileFlags flags, PRIntervalTime timeout) {
  return ((sd->methods->transmitfile)(sd, fd, hdr, hlen, flags, timeout));
}

PR_IMPLEMENT(PRInt32)
MPR_AcceptRead(PRFileDesc* sd, PRFileDesc** nd, PRNetAddr** raddr, void* buf,
              PRInt32 amount, PRIntervalTime timeout) {
  return ((sd->methods->acceptread)(sd, nd, raddr, buf, amount, timeout));
}

PR_IMPLEMENT(PRStatus) MPR_GetSockName(PRFileDesc* fd, PRNetAddr* addr) {
  return ((fd->methods->getsockname)(fd, addr));
}

PR_IMPLEMENT(PRStatus) MPR_GetPeerName(PRFileDesc* fd, PRNetAddr* addr) {
  return ((fd->methods->getpeername)(fd, addr));
}

PR_IMPLEMENT(PRStatus)
MPR_GetSocketOption(PRFileDesc* fd, PRSocketOptionData* data) {
  return ((fd->methods->getsocketoption)(fd, data));
}

PR_IMPLEMENT(PRStatus)
MPR_SetSocketOption(PRFileDesc* fd, const PRSocketOptionData* data) {
  return ((fd->methods->setsocketoption)(fd, data));
}

PR_IMPLEMENT(PRInt32)
MPR_SendFile(PRFileDesc* sd, PRSendFileData* sfd, PRTransmitFileFlags flags,
            PRIntervalTime timeout) {
  return ((sd->methods->sendfile)(sd, sfd, flags, timeout));
}

PR_IMPLEMENT(PRInt32)
MPR_EmulateAcceptRead(PRFileDesc* sd, PRFileDesc** nd, PRNetAddr** raddr,
  ...
  */
  accepted = MPR_Accept(sd, &remote, PR_INTERVAL_NO_TIMEOUT);
  if (NULL == accepted) {
    return rv;
  }

  rv = MPR_Recv(accepted, buf, amount, 0, timeout);
  if (rv >= 0) {
...
PR_IMPLEMENT(PRInt32)
MPR_EmulateSendFile(PRFileDesc* sd, PRSendFileData* sfd,
                   PRTransmitFileFlags flags, PRIntervalTime timeout) {
  PRInt32 rv, count = 0;
  ...
  /* Get file size */
  if (PR_SUCCESS != MPR_GetOpenFileInfo(sfd->fd, &info)) {
    count = -1;
    goto done;
  }
  ...
  alignment = MPR_GetMemMapAlignment();
  ...
  if (len) {
    LL_I2L(zero64, 0);
    mapHandle = MPR_CreateFileMap(sfd->fd, zero64, PR_PROT_READONLY);
    if (!mapHandle) {
      count = -1;
      goto done;
    }
    LL_I2L(file_mmap_offset64, file_mmap_offset);
    addr = MPR_MemMap(mapHandle, file_mmap_offset64, mmap_len);
    if (!addr) {
      count = -1;
      goto done;
    }
  ...
  rv = MPR_Writev(sd, iov, index, timeout);
  if (len) {
    MPR_MemUnmap(addr, mmap_len);
  }
  if (rv < 0) {
    count = -1;
    goto done;
  ...
  while (len > 0) {
    ...
    LL_I2L(file_mmap_offset64, file_mmap_offset);
    addr = MPR_MemMap(mapHandle, file_mmap_offset64, len);
    if (!addr) {
      count = -1;
      goto done;
    }
    rv = MPR_Send(sd, addr, len, 0, timeout);
    MPR_MemUnmap(addr, len);
    if (rv < 0) {
      count = -1;
      goto done;
    }
...
#  define _SENDFILE_BUFSIZE (16 * 1024)
  ...
  buffer = sfd->header;
  while (buflen) {
    rv = MPR_Send(sd, buffer, buflen, 0, timeout);
    if (rv < 0) {
      /* MPR_Send() has invoked MPR_SetError(). */
      rv = -1;
      goto done;
    } else {
      count += rv;
   ...
   * send file next
   */
  if (MPR_Seek(sfd->fd, sfd->file_offset, PR_SEEK_SET) < 0) {
    rv = -1;
    goto done;
  }
  ...
  if (sendbytes == 0) {
    /* send entire file */
    while ((rlen = MPR_Read(sfd->fd, buf, _SENDFILE_BUFSIZE)) > 0) {
      while (rlen) {
        char* bufptr = buf;

        rv = MPR_Send(sd, bufptr, rlen, 0, timeout);
        if (rv < 0) {
          /* MPR_Send() has invoked MPR_SetError(). */
          rv = -1;
          goto done;
    ...
    if (rlen < 0) {
      /* MPR_Read() has invoked MPR_SetError(). */
      rv = -1;
      goto done;
    }
  } else {
    readbytes = PR_MIN(sendbytes, _SENDFILE_BUFSIZE);
    while (readbytes && ((rlen = MPR_Read(sfd->fd, buf, readbytes)) > 0)) {
      while (rlen) {
        char* bufptr = buf;
