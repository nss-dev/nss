...
static void _PR_ConvertToIpv4NetAddr(const PRNetAddr* src_v6addr,
  ...
  if (MPR_IsNetAddrType(src_v6addr, PR_IpAddrV4Mapped)) {
    srcp = src_v6addr->ipv6.ip.pr_s6_addr;
    memcpy((char*)&dst_v4addr->inet.ip, srcp + 12, 4);
  } else if (MPR_IsNetAddrType(src_v6addr, PR_IpAddrAny)) {
    dst_v4addr->inet.ip = htonl(INADDR_ANY);
  } else if (MPR_IsNetAddrType(src_v6addr, PR_IpAddrLoopback)) {
...
static PRStatus PR_CALLBACK Ipv6ToIpv4SocketBind(PRFileDesc* fd,
  ...
  if (MPR_IsNetAddrType(addr, PR_IpAddrV4Mapped) ||
      MPR_IsNetAddrType(addr, PR_IpAddrAny)) {
    _PR_ConvertToIpv4NetAddr(addr, &tmp_ipv4addr);
    tmp_addrp = &tmp_ipv4addr;
  } else {
    MPR_SetError(PR_NETWORK_UNREACHABLE_ERROR, 0);
    return PR_FAILURE;
  }
...
static PRStatus PR_CALLBACK Ipv6ToIpv4SocketConnect(PRFileDesc* fd,
  ...
  if (MPR_IsNetAddrType(addr, PR_IpAddrV4Mapped) ||
      MPR_IsNetAddrType(addr, PR_IpAddrLoopback)) {
    _PR_ConvertToIpv4NetAddr(addr, &tmp_ipv4addr);
    tmp_addrp = &tmp_ipv4addr;
  } else {
    MPR_SetError(PR_NETWORK_UNREACHABLE_ERROR, 0);
...
#  endif
  ...
  _pr_ipv6_to_ipv4_id = MPR_GetUniqueIdentity("Ipv6_to_Ipv4 layer");
  PR_ASSERT(PR_INVALID_IO_LAYER != _pr_ipv6_to_ipv4_id);

  stubMethods = MPR_GetDefaultIOMethods();
...
PRBool _pr_ipv6_is_present(void) {
  if (MPR_CallOnce(&_pr_init_ipv6_once, _pr_init_ipv6) != PR_SUCCESS) {
    return PR_FALSE;
  }
...
PR_IMPLEMENT(PRStatus) _pr_push_ipv6toipv4_layer(PRFileDesc* fd) {
  ...
  if (fd->methods->file_type == PR_DESC_SOCKET_TCP)
    ipv6_fd = MPR_CreateIOLayerStub(_pr_ipv6_to_ipv4_id, &ipv6_to_v4_tcpMethods);
  else
    ipv6_fd = MPR_CreateIOLayerStub(_pr_ipv6_to_ipv4_id, &ipv6_to_v4_udpMethods);
  if (NULL == ipv6_fd) {
    goto errorExit;
  }
  ipv6_fd->secret = NULL;

  if (MPR_PushIOLayer(fd, PR_TOP_IO_LAYER, ipv6_fd) == PR_FAILURE) {
    goto errorExit;
  }
