...
#define LOCAL_MAXIOV 8
  ...
  if (_PR_IO_PENDING(me)) {
    MPR_SetError(PR_IO_PENDING_ERROR, 0);
    return -1;
  }
...
PR_IMPLEMENT(PRFileDesc*) MPR_ImportTCPSocket(PROsfd osfd) {
  PRFileDesc* fd;

  if (!_pr_initialized) {
    _PR_ImplicitInitialization();
  }
  fd = MPR_AllocFileDesc(osfd, MPR_GetTCPMethods());
  if (fd != NULL) {
...
PR_IMPLEMENT(PRFileDesc*) MPR_ImportUDPSocket(PROsfd osfd) {
  PRFileDesc* fd;

  if (!_pr_initialized) {
    _PR_ImplicitInitialization();
  }
  fd = MPR_AllocFileDesc(osfd, MPR_GetUDPMethods());
  if (fd != NULL) {
    _PR_MD_MAKE_NONBLOCK(fd);
    _PR_MD_INIT_FD_INHERITABLE(fd, PR_TRUE);
  } else {
...
PR_IMPLEMENT(PRFileDesc*) MPR_CreateSocketPollFd(PROsfd osfd) {
  PRFileDesc* fd;
...
static PRFileDesc* PR_CALLBACK SocketAccept(PRFileDesc* fd, PRNetAddr* addr,
  ...
  fd2 = MPR_AllocFileDesc(osfd, MPR_GetTCPMethods());
  if (!fd2) {
    _PR_MD_CLOSE_SOCKET(osfd);
    return NULL;
  }
...
static PRInt32 PR_CALLBACK SocketRecv(PRFileDesc* fd, void* buf, PRInt32 amount,
  ...
  if ((PR_MSG_PEEK == flags) && _PR_FD_NEED_EMULATE_MSG_PEEK(fd)) {
    ...
    if (fd->secret->peekBufSize < amount) {
      if (fd->secret->peekBuffer) {
        MPR_Free(fd->secret->peekBuffer);
      }
      fd->secret->peekBufSize = amount;
      fd->secret->peekBuffer = MPR_Malloc(amount);
  ...
  PR_LOG(_pr_io_lm, PR_LOG_MAX,
         ("recv -> %d, error = %d, os error = %d", rv, MPR_GetError(),
          MPR_GetOSError()));
...
static PRStatus PR_CALLBACK SocketClose(PRFileDesc* fd) {
  ...
  if (fd->secret->state == _PR_FILEDESC_OPEN) {
    ...
    if (fd->secret->overlappedActive && _fd_waiting_for_overlapped_done_lock) {
      ...
      MPR_Lock(_fd_waiting_for_overlapped_done_lock);
      forWaiting->next = _fd_waiting_for_overlapped_done;
      _fd_waiting_for_overlapped_done = forWaiting;
      MPR_Unlock(_fd_waiting_for_overlapped_done_lock);
  ...
  *nd = NULL;
...
#if defined(WINNT)
  {
    ...
    if (rv < 0) {
      ...
      *nd = MPR_AllocFileDesc(newSock, MPR_GetTCPMethods());
      if (*nd == 0) {
        _PR_MD_CLOSE_SOCKET(newSock);
        /* MPR_AllocFileDesc() has invoked MPR_SetError(). */
        rv = -1;
      } else {
...
#else
  rv = MPR_EmulateSendFile(sd, sfd, flags, timeout);
#endif /* WINNT */
...
PR_IMPLEMENT(const PRIOMethods*) MPR_GetTCPMethods() { return &tcpMethods; }

PR_IMPLEMENT(const PRIOMethods*) MPR_GetUDPMethods() { return &udpMethods; }
...
PR_IMPLEMENT(PRStatus) MPR_NewTCPSocketPair(PRFileDesc* f[]) {
#ifdef XP_UNIX
  PRInt32 rv, osfd[2];
  ...
  f[0] = MPR_AllocFileDesc(osfd[0], MPR_GetTCPMethods());
  if (!f[0]) {
  ...
  f[1] = MPR_AllocFileDesc(osfd[1], MPR_GetTCPMethods());
  if (!f[1]) {
    MPR_Close(f[0]);
    _PR_MD_CLOSE_SOCKET(osfd[1]);
    /* MPR_AllocFileDesc() has invoked MPR_SetError(). */
   ...
   * because MPR_Accept would have associated the I/O completion
...
failed:
  ...
  f[0] = f[1] = NULL;
  listenSock = MPR_NewTCPSocket();
  if (listenSock == NULL) {
    goto failed;
  }
  MPR_InitializeNetAddr(PR_IpAddrLoopback, 0, &selfAddr); /* BugZilla: 35408 */
  if (MPR_Bind(listenSock, &selfAddr) == PR_FAILURE) {
    goto failed;
  }
  if (MPR_GetSockName(listenSock, &selfAddr) == PR_FAILURE) {
    goto failed;
  }
  port = ntohs(selfAddr.inet.port);
  if (MPR_Listen(listenSock, 5) == PR_FAILURE) {
    goto failed;
  }
  f[0] = MPR_NewTCPSocket();
  if (f[0] == NULL) {
    goto failed;
  }
#  ifdef _PR_CONNECT_DOES_NOT_BIND
   ...
   */
  MPR_InitializeNetAddr(PR_IpAddrLoopback, 0, &selfAddr);
  if (MPR_Bind(f[0], &selfAddr) == PR_FAILURE) {
    goto failed;
  }
#  endif
  MPR_InitializeNetAddr(PR_IpAddrLoopback, port, &selfAddr);

  /*
   * Only a thread is used to do the connect and accept.
   * I am relying on the fact that MPR_Connect returns
   * successfully as soon as the connect request is put
   * into the listen queue (but before MPR_Accept is called).
   * This is the behavior of the BSD socket code.  If
   * connect does not return until accept is called, we
   * will need to create another thread to call connect.
   */
  if (MPR_Connect(f[0], &selfAddr, PR_INTERVAL_NO_TIMEOUT) == PR_FAILURE) {
    goto failed;
  }
  /*
   * A malicious local process may connect to the listening
   ...
   */
  if (MPR_GetSockName(f[0], &selfAddr) == PR_FAILURE) {
    goto failed;
  }
  f[1] = MPR_Accept(listenSock, &peerAddr, PR_INTERVAL_NO_TIMEOUT);
  if (f[1] == NULL) {
    goto failed;
  }
...
PR_IMPLEMENT(PROsfd)
MPR_FileDesc2NativeHandle(PRFileDesc* fd) {
  if (fd) {
    fd = MPR_GetIdentitiesLayer(fd, PR_NSPR_IO_LAYER);
  }
  if (!fd) {
    MPR_SetError(PR_INVALID_ARGUMENT_ERROR, 0);
    return -1;
...
PR_IMPLEMENT(void)
MPR_ChangeFileDescNativeHandle(PRFileDesc* fd, PROsfd handle) {
  if (fd) {
...
PR_IMPLEMENT(void) MPR_FD_NSET(PROsfd fd, PR_fd_set* set) {
  PR_ASSERT(set->nsize < PR_MAX_SELECT_DESC);

  set->narray[set->nsize++] = fd;
}

PR_IMPLEMENT(void) MPR_FD_NCLR(PROsfd fd, PR_fd_set* set) {
  PRUint32 index, index2;
...
PR_IMPLEMENT(PRInt32) MPR_FD_NISSET(PROsfd fd, PR_fd_set* set) {
  PRUint32 index;
  for (index = 0; index < set->nsize; index++)
    if (set->narray[index] == fd) {
      return 1;
...
MPR_Select(PRInt32 unused, PR_fd_set* pr_rd, PR_fd_set* pr_wr, PR_fd_set* pr_ex,
  ...
  ** (there can't be any overlapping) and call MPR_Poll().
  */
  ...
  if (warning) {
    warning = _PR_Obsolete("MPR_Select()", "MPR_Poll()");
  }
  ...
  copy = poll = (PRPollDesc*)MPR_Calloc(npds + PD_INCR, sizeof(PRPollDesc));
  if (NULL == poll) {
    goto out_of_memory;
  }
  ...
  PR_ASSERT(unused > 0);
  npds = MPR_Poll(poll, unused, timeout);
