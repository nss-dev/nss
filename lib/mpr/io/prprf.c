...
MPR_sxprintf(PRStuffFunc func, void* arg, const char* fmt, ...) {
  va_list ap;
  PRUint32 rv;

  va_start(ap, fmt);
  rv = MPR_vsxprintf(func, arg, fmt, ap);
  va_end(ap);
  return rv;
}

PR_IMPLEMENT(PRUint32)
MPR_vsxprintf(PRStuffFunc func, void* arg, const char* fmt, va_list ap) {
  SprintfState ss;
  int rv;
...
*/
PR_IMPLEMENT(char*) MPR_smprintf(const char* fmt, ...) {
  va_list ap;
  char* rv;

  va_start(ap, fmt);
  rv = MPR_vsmprintf(fmt, ap);
  va_end(ap);
  return rv;
}

/*
** Free memory allocated, for the caller, by MPR_smprintf
*/
PR_IMPLEMENT(void) MPR_smprintf_free(char* mem) { PR_DELETE(mem); }

PR_IMPLEMENT(char*) MPR_vsmprintf(const char* fmt, va_list ap) {
  SprintfState ss;
  int rv;
...
*/
PR_IMPLEMENT(PRUint32)
MPR_snprintf(char* out, PRUint32 outlen, const char* fmt, ...) {
  va_list ap;
  PRUint32 rv;

  va_start(ap, fmt);
  rv = MPR_vsnprintf(out, outlen, fmt, ap);
  va_end(ap);
  return rv;
}

PR_IMPLEMENT(PRUint32)
MPR_vsnprintf(char* out, PRUint32 outlen, const char* fmt, va_list ap) {
  SprintfState ss;
  PRUint32 n;
...
PR_IMPLEMENT(char*) MPR_sprintf_append(char* last, const char* fmt, ...) {
  va_list ap;
  char* rv;

  va_start(ap, fmt);
  rv = MPR_vsprintf_append(last, fmt, ap);
  va_end(ap);
  return rv;
}

PR_IMPLEMENT(char*)
MPR_vsprintf_append(char* last, const char* fmt, va_list ap) {
  SprintfState ss;
  int rv;
