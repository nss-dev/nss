...
static PRStatus PR_CALLBACK pl_TopClose(PRFileDesc* fd) {
  ...
  if ((fd->higher) && (PR_IO_LAYER_HEAD == fd->higher->identity)) {
     ...
     */
    top = MPR_PopIOLayer(fd->higher, PR_TOP_IO_LAYER);
    top->dtor(top);
    /*
    ...
    /* old style stack */
    top = MPR_PopIOLayer(fd, PR_TOP_IO_LAYER);
    top->dtor(top);
...
static PRFileDesc* PR_CALLBACK pl_TopAccept(PRFileDesc* fd, PRNetAddr* addr,
  ...
  *newstack = *fd; /* make a copy of the accepting layer */
  ...
  /* this MPR_PushIOLayer call cannot fail */
  rv = MPR_PushIOLayer(newfd, PR_TOP_IO_LAYER, newstack);
...
static PRInt32 PR_CALLBACK pl_DefAcceptread(PRFileDesc* sd, PRFileDesc** nd,
  ...
  *newstack = *sd; /* make a copy of the accepting layer */
  ...
  /* this MPR_PushIOLayer call cannot fail */
  rv = MPR_PushIOLayer(*nd, PR_TOP_IO_LAYER, newstack);
  PR_ASSERT(PR_SUCCESS == rv);
  return nbytes;
}
...
PR_IMPLEMENT(const PRIOMethods*) MPR_GetDefaultIOMethods(void) {
  return &pl_methods;
} /* MPR_GetDefaultIOMethods */

PR_IMPLEMENT(PRFileDesc*)
MPR_CreateIOLayerStub(PRDescIdentity ident, const PRIOMethods* methods) {
  PRFileDesc* fd = NULL;
...
PR_IMPLEMENT(PRFileDesc*) MPR_CreateIOLayer(PRFileDesc* top) {
  PRFileDesc* fd = NULL;
...
MPR_PushIOLayer(PRFileDesc* stack, PRDescIdentity id, PRFileDesc* fd) {
  PRFileDesc* insert = MPR_GetIdentitiesLayer(stack, id);
...
PR_IMPLEMENT(PRFileDesc*) MPR_PopIOLayer(PRFileDesc* stack, PRDescIdentity id) {
  PRFileDesc* extract = MPR_GetIdentitiesLayer(stack, id);
...
PR_IMPLEMENT(PRDescIdentity) MPR_GetUniqueIdentity(const char* layer_name) {
  ...
  if (NULL != layer_name) {
    name = (char*)MPR_Malloc(strlen(layer_name) + 1);
    if (NULL == name) {
      MPR_SetError(PR_OUT_OF_MEMORY_ERROR, 0);
...
retry:
  ...
  return identity;
} /* MPR_GetUniqueIdentity */

PR_IMPLEMENT(const char*) MPR_GetNameForIdentity(PRDescIdentity ident) {
  const char* rv = NULL;
  if (!_pr_initialized) {
    _PR_ImplicitInitialization();
  }
  ...
  return rv;
} /* MPR_GetNameForIdentity */

PR_IMPLEMENT(PRDescIdentity) MPR_GetLayersIdentity(PRFileDesc* fd) {
  PR_ASSERT(NULL != fd);
  if (PR_IO_LAYER_HEAD == fd->identity) {
  ...
  return fd->identity;
} /* MPR_GetLayersIdentity */

PR_IMPLEMENT(PRFileDesc*)
MPR_GetIdentitiesLayer(PRFileDesc* fd, PRDescIdentity id) {
  PRFileDesc* layer = fd;
