...
void _PR_InitIO(void) {
  const PRIOMethods* methods = MPR_GetFileMethods();
...
#ifdef WIN32
  _pr_stdin = MPR_AllocFileDesc((PROsfd)GetStdHandle(STD_INPUT_HANDLE), methods);
  _pr_stdout =
      MPR_AllocFileDesc((PROsfd)GetStdHandle(STD_OUTPUT_HANDLE), methods);
  _pr_stderr =
      MPR_AllocFileDesc((PROsfd)GetStdHandle(STD_ERROR_HANDLE), methods);
#  ifdef WINNT
...
#  endif
#else
  _pr_stdin = MPR_AllocFileDesc(0, methods);
  _pr_stdout = MPR_AllocFileDesc(1, methods);
  _pr_stderr = MPR_AllocFileDesc(2, methods);
#endif
...
MPR_AllocFileDesc(PROsfd osfd, const PRIOMethods* methods) {
  PRFileDesc* fd;
