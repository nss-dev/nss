...
static void TimerManager(void* arg) {
  ...
  while (1) {
    if (PR_CLIST_IS_EMPTY(&tm_vars.timer_queue)) {
      MPR_WaitCondVar(tm_vars.new_timer, PR_INTERVAL_NO_TIMEOUT);
    } else {
      now = MPR_IntervalNow();
      ...
      if ((PRInt32)(now - timer->absolute) >= 0) {
        ...
        MPR_Unlock(tm_vars.ml);
        timer->func(timer->arg);
        MPR_Lock(tm_vars.ml);
        timer->ref_count -= 1;
        if (0 == timer->ref_count) {
          MPR_NotifyAllCondVar(tm_vars.cancel_timer);
        }
      } else {
        timeout = (PRIntervalTime)(timer->absolute - now);
        MPR_WaitCondVar(tm_vars.new_timer, timeout);
      }
    }
  }
  MPR_Unlock(tm_vars.ml);
...
static TimerEvent* CreateTimer(PRIntervalTime timeout, void (*func)(void*),
  ...
  timer->absolute = MPR_IntervalNow() + timeout;
  timer->func = func;
  timer->arg = arg;
  timer->ref_count = 2;
  MPR_Lock(tm_vars.ml);
  ...
  MPR_NotifyCondVar(tm_vars.new_timer);
  MPR_Unlock(tm_vars.ml);
  return timer;
}
...
static PRBool CancelTimer(TimerEvent* timer) {
  ...
  if (timer->links.prev == &timer->links) {
    while (timer->ref_count == 1) {
      MPR_WaitCondVar(tm_vars.cancel_timer, PR_INTERVAL_NO_TIMEOUT);
    }
  } else {
...
static PRStatus TimerInit(void) {
  tm_vars.ml = MPR_NewLock();
  if (NULL == tm_vars.ml) {
    goto failed;
  }
  tm_vars.new_timer = MPR_NewCondVar(tm_vars.ml);
  if (NULL == tm_vars.new_timer) {
    goto failed;
  }
  tm_vars.cancel_timer = MPR_NewCondVar(tm_vars.ml);
  if (NULL == tm_vars.cancel_timer) {
    goto failed;
  }
  PR_INIT_CLIST(&tm_vars.timer_queue);
  tm_vars.manager_thread =
      MPR_CreateThread(PR_SYSTEM_THREAD, TimerManager, NULL, PR_PRIORITY_NORMAL,
...
failed:
  if (NULL != tm_vars.cancel_timer) {
    MPR_DestroyCondVar(tm_vars.cancel_timer);
  }
  if (NULL != tm_vars.new_timer) {
    MPR_DestroyCondVar(tm_vars.new_timer);
  }
  if (NULL != tm_vars.ml) {
    MPR_DestroyLock(tm_vars.ml);
  }
  return PR_FAILURE;
}
...
void _PR_InitMW(void) {
  ...
  max_polling_interval = MPR_MillisecondsToInterval(MAX_POLLING_INTERVAL);
} /* _PR_InitMW */

void _PR_CleanupMW(void) {
  MPR_DestroyLock(mw_lock);
  mw_lock = NULL;
  if (mw_state->group) {
    MPR_DestroyWaitGroup(mw_state->group);
...
static PRWaitGroup* MW_Init2(void) {
  ...
  {
    group = MPR_CreateWaitGroup(_PR_DEFAULT_HASH_LENGTH);
    if (NULL == group) {
      goto failed_alloc;
    }
    MPR_Lock(mw_lock);
    ...
    MPR_Unlock(mw_lock);
    if (group != NULL) {
      (void)MPR_DestroyWaitGroup(group);
    }
...
static _PR_HashStory MW_AddHashInternal(PRRecvWait* desc, _PRWaiterHash* hash) {
  ...
  */
  ...
  while (rehash-- > 0) {
    ...
    if (desc == waiter[hidx]) {
      MPR_SetError(PR_INVALID_ARGUMENT_ERROR, 0); /* desc already in table */
      return _prmw_error;
    }
#if 0
...
#  endif
  ...
  MPR_NotifyCondVar(group->io_complete);
...
static PRStatus _MW_PollInternal(PRWaitGroup* group) {
  ...
  group->poller = MPR_GetCurrentThread();
  ...
  while (PR_TRUE) {
    ...
    while (0 == group->waiter->count) {
      PRStatus st;
      st = MPR_WaitCondVar(group->new_business, PR_INTERVAL_NO_TIMEOUT);
      if (_prmw_running != group->state) {
        MPR_SetError(PR_INVALID_STATE_ERROR, 0);
        goto aborted;
      }
      if (_MW_ABORTED(st)) {
        goto aborted;
...
#  endif
    ...
    */
    ...
    count_ready = MPR_Poll(group->polling_list, count, polling_interval);
...
static PRMWGroupState MW_TestForShutdownInternal(PRWaitGroup* group) {
  ...
  */
  if ((_prmw_stopping == rv) && (0 == group->waiting_threads)) {
    ...
    MPR_NotifyCondVar(group->mw_manage);
  }
  return rv;
...
static void NT_TimeProc(void* arg) {
  ...
  ** $$$$
  */
  bottom = MPR_GetIdentitiesLayer(desc->fd, PR_NSPR_IO_LAYER);
...
MPR_AddWaitFileDesc(PRWaitGroup* group, PRRecvWait* desc) {
  _PR_HashStory hrv;
  PRStatus rv = PR_FAILURE;
#ifdef WINNT
  ...
  */
  if (0 == group->waiter->count) {
    group->last_poll = MPR_IntervalNow();
  }
  ...
  MPR_NotifyCondVar(group->new_business); /* tell the world */
  ...
  if (desc->timeout != PR_INTERVAL_NO_TIMEOUT) {
    ...
    if (0 == overlapped->data.mw.timer) {
       ...
       */
      MPR_SetError(PR_INSUFFICIENT_RESOURCES_ERROR, GetLastError());
      return PR_FAILURE;
    }
  }
...
PR_IMPLEMENT(PRRecvWait*) MPR_WaitRecvReady(PRWaitGroup* group) {
  ...
  do {
    ...
    */
      ...
      */
      if (NULL == group->poller) {
        ...
        */
        PRStatus st;
        do {
          st = MPR_WaitCondVar(group->io_complete, PR_INTERVAL_NO_TIMEOUT);
          if (_prmw_running != group->state) {
            MPR_SetError(PR_INVALID_STATE_ERROR, 0);
            goto aborted;
          }
    ...
    io_ready = PR_LIST_HEAD(&group->io_ready);
    MPR_NotifyCondVar(group->io_taken);
    PR_ASSERT(io_ready != NULL);
    PR_REMOVE_LINK(io_ready);
  } while (NULL == io_ready);
...
PR_IMPLEMENT(PRStatus)
MPR_CancelWaitFileDesc(PRWaitGroup* group, PRRecvWait* desc) {
#if !defined(WINNT)
  ...
  if (InterlockedCompareExchange((LONG*)&desc->outcome, (LONG)PR_MW_INTERRUPT,
                                 (LONG)PR_MW_PENDING) == (LONG)PR_MW_PENDING) {
    PRFileDesc* bottom = MPR_GetIdentitiesLayer(desc->fd, PR_NSPR_IO_LAYER);
    PR_ASSERT(NULL != bottom);
    if (NULL == bottom) {
      MPR_SetError(PR_INVALID_ARGUMENT_ERROR, 0);
      goto unlock;
...
unlock:
  MPR_Unlock(group->ml);
  return rv;
} /* MPR_CancelWaitFileDesc */

PR_IMPLEMENT(PRRecvWait*) MPR_CancelWaitGroup(PRWaitGroup* group) {
  PRRecvWait** desc;
  ...
  if (_prmw_stopped != group->state) {
    ...
    if (0 == group->waiting_threads) { /* is there anybody else? */
      ...
      MPR_NotifyAllCondVar(group->new_business);
      MPR_NotifyAllCondVar(group->io_complete);
    }
    while (_prmw_stopped != group->state) {
      (void)MPR_WaitCondVar(group->mw_manage, PR_INTERVAL_NO_TIMEOUT);
    }
  }
  ...
  for (desc = &group->waiter->recv_wait; desc < end; ++desc) {
    if (NULL != *desc) {
      if (InterlockedCompareExchange(
              ...
              (LONG)PR_MW_PENDING) == (LONG)PR_MW_PENDING) {
        PRFileDesc* bottom =
            MPR_GetIdentitiesLayer((*desc)->fd, PR_NSPR_IO_LAYER);
        PR_ASSERT(NULL != bottom);
        if (NULL == bottom) {
          MPR_SetError(PR_INVALID_ARGUMENT_ERROR, 0);
          goto invalid_arg;
...
#  endif
  ...
  if (PR_CLIST_IS_EMPTY(&group->io_ready)) {
    MPR_SetError(PR_GROUP_EMPTY_ERROR, 0);
  } else {
    PRCList* head = PR_LIST_HEAD(&group->io_ready);
...
PR_IMPLEMENT(PRWaitGroup*) MPR_CreateWaitGroup(PRInt32 size /* ignored */) {
  PRWaitGroup* wg;
  ...
  wg->io_taken = MPR_NewCondVar(wg->ml);
  if (NULL == wg->io_taken) {
    goto failed_cvar0;
  }
  wg->io_complete = MPR_NewCondVar(wg->ml);
  if (NULL == wg->io_complete) {
    goto failed_cvar1;
  }
  wg->new_business = MPR_NewCondVar(wg->ml);
  if (NULL == wg->new_business) {
    goto failed_cvar2;
  }
  wg->mw_manage = MPR_NewCondVar(wg->ml);
...
failed_waiter:
  MPR_DestroyCondVar(wg->mw_manage);
failed_cvar3:
  MPR_DestroyCondVar(wg->new_business);
failed_cvar2:
  MPR_DestroyCondVar(wg->io_complete);
failed_cvar1:
  MPR_DestroyCondVar(wg->io_taken);
failed_cvar0:
  MPR_DestroyLock(wg->ml);
failed_lock:
  PR_DELETE(wg);
  wg = NULL;
...
PR_IMPLEMENT(PRStatus) MPR_DestroyWaitGroup(PRWaitGroup* group) {
  PRStatus rv = PR_SUCCESS;
  if (NULL == group) {
    group = mw_state->group;
  }
  ...
  if (NULL != group) {
    ...
    MPR_DestroyCondVar(group->mw_manage);
    MPR_DestroyCondVar(group->new_business);
    MPR_DestroyCondVar(group->io_complete);
    MPR_DestroyCondVar(group->io_taken);
    MPR_DestroyLock(group->ml);
...
PR_IMPLEMENT(PRMWaitEnumerator*) MPR_CreateMWaitEnumerator(PRWaitGroup* group) {
  ...
  return enumerator;
} /* MPR_CreateMWaitEnumerator */

PR_IMPLEMENT(PRStatus)
MPR_DestroyMWaitEnumerator(PRMWaitEnumerator* enumerator) {
  ...
  MPR_Free(enumerator);
  return PR_SUCCESS;
} /* MPR_DestroyMWaitEnumerator */

PR_IMPLEMENT(PRRecvWait*)
MPR_EnumerateWaitGroup(PRMWaitEnumerator* enumerator,
  ...
  if (NULL == previous) {
    ...
    enumerator->thread = MPR_GetCurrentThread();
    enumerator->index = 0;
  }
  /* continuing an enumeration */
    ...
    if (enumerator->p_timestamp != enumerator->group->p_timestamp) {
      return MPR_EnumerateWaitGroup(enumerator, NULL);
    }
  }
...
#else
  MPR_Unlock(enumerator->group->ml);
#endif
