  ...
  */
  if (0 == _pr_fd_cache.limit_high) {
    ...
    do {
      ...
      MPR_Lock(_pr_fd_cache.ml); /* need the lock to do this safely */
...
PR_IMPLEMENT(PRStatus) MPR_SetFDCacheSize(PRIntn low, PRIntn high) {
  /*
...
void _PR_InitFdCache(void) {
  ...
  */
  const char* low = MPR_GetEnv("NSPR_FD_CACHE_SIZE_LOW");
  const char* high = MPR_GetEnv("NSPR_FD_CACHE_SIZE_HIGH");
...
void _PR_CleanupFdCache(void) {
  ...
  MPR_DestroyLock(_pr_fd_cache.ml);
