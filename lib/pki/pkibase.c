...
nssPKIObject_Lock(nssPKIObject *object)
{
    switch (object->lockType) {
        case nssPKIMonitor:
            MPR_EnterMonitor(object->sync.mlock);
            break;
        case nssPKILock:
            MPR_Lock(object->sync.lock);
            break;
        default:
            PORT_Assert(0);
    }
...
nssPKIObject_Unlock(nssPKIObject *object)
{
    switch (object->lockType) {
        case nssPKIMonitor:
            MPR_ExitMonitor(object->sync.mlock);
            break;
        case nssPKILock:
            MPR_Unlock(object->sync.lock);
            break;
        default:
            PORT_Assert(0);
    }
...
nssPKIObject_NewLock(nssPKIObject *object, nssPKILockType lockType)
{
    ...
    switch (lockType) {
        case nssPKIMonitor:
            object->sync.mlock = MPR_NewMonitor();
            return (object->sync.mlock ? PR_SUCCESS : PR_FAILURE);
        case nssPKILock:
            object->sync.lock = MPR_NewLock();
...
nssPKIObject_DestroyLock(nssPKIObject *object)
{
    switch (object->lockType) {
        case nssPKIMonitor:
            MPR_DestroyMonitor(object->sync.mlock);
            object->sync.mlock = NULL;
            break;
        case nssPKILock:
            MPR_DestroyLock(object->sync.lock);
...
{
    return NSSTime_SetPRTime(timeOpt, MPR_Now());
}
