...
nsslowkey_CloseKeyDB(NSSLOWKEYDBHandle *handle)
{
    if (handle != NULL) {
        ...
        if (handle->lock != NULL) {
            SKIP_AFTER_FORK(MPR_DestroyLock(handle->lock));
        }
...
keydb_Close(NSSLOWKEYDBHandle *kdb)
{
    ...
    PORT_Assert(kdbLock != NULL);
    SKIP_AFTER_FORK(MPR_Lock(kdbLock));

    (*db->close)(db);

    SKIP_AFTER_FORK(MPR_Unlock(kdbLock));
